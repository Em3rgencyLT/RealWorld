...
public class GSDSplineN : MonoBehaviour
{
    ...
    public void RemoveEdgeObject(int tIndex = -1, bool bSkipUpdate = false)
    {
        ...
        if (!bSkipUpdate) SetupEdgeObjects();
    }
    ...
    public void DetectInvalidEdgeObjects()
    {
        ...
        for (var i = InvalidList.Count - 1; i >= 0; i--) RemoveEdgeObject(InvalidList[i], true);
    ...
    public int SplinatedMeshGetIndex(ref string UID)
    {
        ...
        {
            SMM = SplinatedObjects[i];
            if (string.CompareOrdinal(SMM.UID, UID) == 0) return i;
        }
    ...
    public void RemoveSplinatedObject(int tIndex = -1, bool bSkipUpdate = false)
    {
        ...
        if (!bSkipUpdate) SetupSplinatedMeshes();
    }
    ...
    public void DetectInvalidSplinatedObjects()
    {
        ...
        for (var i = InvalidList.Count - 1; i >= 0; i--) RemoveSplinatedObject(InvalidList[i], true);
...
//		}
    ...
    public Vector3 FilterMaxGradeHeight(Vector3 tPos, out float MinY, out float MaxY)
    {
        var tVect = tPos;
        tVect.y = pos.y;
        var CurrentDistance = Vector2.Distance(new Vector2(pos.x, pos.z), new Vector2(tPos.x, tPos.z));
...
//		float DifferenceFromMax = -1;
        if (tPos.y > MaxY) //			DifferenceFromMax = tPos.y - MaximumY;
            tPos.y = MaxY;
        if (tPos.y < MinY) //			DifferenceFromMax = MinimumY - tPos.y;
            tPos.y = MinY;
    ...
    public void SetupSplinationLimits()
    {
        ...
        for (var i = idOnSpline; i < mCount; i++)
        {
            tNode = GSDSpline.mNodes[i];
            if (tNode.CanSplinate())
    ...
    public void UpdateCuts()
    {
        ...
        {
            MR = RoadCut_marker.GetComponent<MeshRenderer>();
            if (MR == null || MR.sharedMaterial == null) DestroyImmediate(RoadCut_marker);
        }

        if (ShoulderCutR_marker != null)
        {
            MR = ShoulderCutR_marker.GetComponent<MeshRenderer>();
            if (MR == null || MR.sharedMaterial == null) DestroyImmediate(ShoulderCutR_marker);
        }

        if (ShoulderCutL_marker != null)
        {
            MR = ShoulderCutL_marker.GetComponent<MeshRenderer>();
            if (MR == null || MR.sharedMaterial == null) DestroyImmediate(ShoulderCutL_marker);
        }
    }
    ...
    private void BridgeStart()
    {
        ...
        {
            tNode = GSDSpline.mNodes[i];
            if (tNode.bIsIntersection) //Encountered intersection. End search.
                return;
            if (tNode.bIsBridgeStart) //Encountered another bridge. Return:
                return;
            if (tNode.bIgnore) continue;
    ...
    public bool CanBridgeStart()
    {
        ...
        {
            if (GSDSpline.mNodes[idOnSpline + 1].bIsBridgeStart) return false;
        ...
        if (GSDSpline.IsInBridge(tTime)) return false;
    ...
    private void TunnelStart()
    {
        ...
        for (var i = StartI; i < mCount; i++)
        {
            ...
            if (tNode.bIsIntersection) //Encountered intersection. End search.
                return;
            if (tNode.bIsTunnelStart) //Encountered another Tunnel. Return:
                return;
            if (tNode.bIgnore) continue;
    ...
    public bool CanTunnelStart()
    {
        ...
        if (idOnSpline < mCount - 1)
        {
            if (GSDSpline.mNodes[idOnSpline + 1].bIsTunnelStart) return false;
        ...
        if (GSDSpline.IsInTunnel(tTime)) return false;
    ...
    {
        var cCount = 0;
        if (GSDRootUtil.IsApproximately(V1.x, V2.x, 0.02f)) cCount += 1;
        if (GSDRootUtil.IsApproximately(V1.y, V2.y, 0.02f)) cCount += 1;
        if (GSDRootUtil.IsApproximately(V1.z, V2.z, 0.02f)) cCount += 1;
