...
public class GSDSplineI : MonoBehaviour
{
    ...
    {
        var iCount = GSDSpline.mNodes.Count;
        if (iCount < 2) return;
        GSDSplineIN tNode;
        GSDSplineN xNode;
        mNodes.Clear();
        var tParam = GSDSpline.GetClosestParam(MousePos, false, true);
    ...
    private void OnDrawGizmos()
    {
        ...
        var DistanceFromCam = Vector3.Distance(Camera.current.transform.position, mNodes[0].pos);
        ...
        var startI = 0f;
        var endI = 1f;
        if (ActionNode.idOnSpline > 3) startI = mNodes[ActionNode.idOnSpline - 2].tTime;
        if (ActionNode.idOnSpline < mNodes.Count - 3) endI = mNodes[ActionNode.idOnSpline + 2].tTime;
...
//		float step = 0.1f / mDistance;
        ...
        var tTime = 0f;
        for (var j = 0; j < mNodes.Count - 1; j++)
        {
            tVect2 = mNodes[j].pos;
            if (j > 0)
            {
