...
//		if(!Application.isEditor || (Application.isEditor && UnityEditor.EditorApplication.isPlaying)){ return; }
        ...
        var tNodesRawLength = tNodesRaw.Length;
        if (tNodesRawLength == 0) return;
        for (var i = 0; i < tNodesRawLength; i++)
            if (tNodesRaw[i] != null)
            {
...
//			if(tRoad.bProfiling){ UnityEngine.Profiling.Profiler.EndSample(); }
        else if (GetNodeCount() == 1) mNodes[0].tTime = 0f;
...
//                tNode.bIsEndPoint = false;
        ...
        else if (mNodesCount == 1) mNodes[0].tangent = default(Vector3);
        ...
        var MaxEffectDistance = Mathf.Max(tMaxEffects) * 2f; //Add min/max clear diff to bound checks
        var mCount1 = GetNodeCount();
...
//		Vector3 tVect1 = default(Vector3);
        ...
        var MinX = Mathf.Min(tMinMaxX) - MaxEffectDistance;
        var MaxX = Mathf.Max(tMinMaxX) + MaxEffectDistance;
        var MinZ = Mathf.Min(tMinMaxZ) - MaxEffectDistance;
        var MaxZ = Mathf.Max(tMinMaxZ) + MaxEffectDistance;
        ...
        for (i = 0; i < tNodesCount; i++)
        {
            ...
            if (i != tNodes.Count - 1)
            {
                if (tNodes[i + 1].transform.position - tNode.transform.position == Vector3.zero)
                    ...
                    rot = Quaternion.LookRotation(tNodes[i + 1].transform.position - tNode.transform.position,
                        transform.up);
            }
            else if (bClosed)
            {
            ...
            tNode.Setup(tNode.transform.position, rot, new Vector2(0f, 1f), step * (float) i,
                tNode.transform.gameObject.name);
    ...
    private void Setup_SplineLength()
    {
        ...
        var step = 0.5f / mDistance;

        //Get a slightly more accurate portrayal of the time:
        var tTime = 0f;
        for (var j = 0; j < mNodeCount - 1; j++)
        {
            tVect2 = mNodes[j].pos;
            if (j > 0)
            {
...
//		float tNanCheck = 0f;
        ...
        for (var j = 1; j < mNodeCount - 1; j++)
        {
            PrevNode = mNodes[j - 1];
            ThisNode = mNodes[j];

            if (j == 1) prevPos = GetSplineValue(PrevNode.tTime);
            tDistance = 0.00001f;
            for (var i = PrevNode.tTime; i < ThisNode.tTime; i += step)
            {
                cPos = GetSplineValue(i);
                if (!float.IsNaN(cPos.x))
                {
                    if (float.IsNaN(prevPos.x)) prevPos = cPos;
                    tDistance += Vector3.Distance(cPos, prevPos);
                    prevPos = cPos;
                }
            }
...
//		float tNanCheck = 0f;
        ...
        var prevPos = cPos;
        var tempDistanceModMax = tRoad.opt_RoadDefinition - step;
    ...
    private void GetClosestRoadDefKeys(float tX, out int lo, out int hi, out int loIndex, out int hiIndex)
    {
        ...
        if (RoadDefKeysArray[lo] == x) hi = lo;
    ...
    private void GetClosestRoadDefValues(float tX, out float loF, out float hiF, out int loIndex, out int hiIndex)
    {
        ...
        if (GSDRootUtil.IsApproximately(RoadDefValuesArray[lo], tX, 0.02f)) hi = lo;
    ...
    {
        var tIndex = GetClosestRoadDefValuesIndex(mDist, false, false);
        ...
        if (tIndex < tCount - 1)
            if (mDist > kDist)
            {
                ...
                var tDiff1 = (mDist - kDist) / (NextValue - kDist);
        ...
        if (tIndex > 0)
            if (mDist < kDist)
            {
                ...
                var tDiff1 = (mDist - PrevValue) / (kDist - PrevValue);
    ...
    {
        var tIndex = GetClosestRoadDefIndex(param, false, false);
        var tKey = TranslateInverseParamToFloat(RoadDefKeysArray[tIndex]);
        var tCount = RoadDefKeysArray.Length;
        var kDist = RoadDefValuesArray[tIndex];
        var xDiff = kDist;
        ...
        if (tIndex < tCount - 1)
            if (param > tKey)
            {
                ...
                var tDiff1 = (param - tKey) / (NextValue - tKey);
        ...
        if (tIndex > 0)
            if (param < tKey)
            {
                ...
                var tDiff1 = 1f - (param - PrevValue) / (tKey - PrevValue);
...
//		}else{
        ...
        var param = (f - mNodes[idx].tTime) / (mNodes[idx + 1].tTime - mNodes[idx].tTime);
...
//		}
    ...
    {
        var Step1 = CachedPointsSeperation / distance; //5m to 1m	
        var Step2 = Step1 * 0.2f; //20 cm		
        var Step3 = Step2 * 0.4f; //8 cm 
        var Step4 = Step3 * 0.25f; //2 cm
        var tMin = 0f;
        var tMax = 1f;
        var BestValue = -1f;
        var MaxStretch = 0.9f;
        ...
        //Step 1: 1m 
        BestValue = GetClosestPoint_Helper(ref tVect, Step1, BestValue, tMin, tMax, ref BestVect_p, ref BestVect_n,
            true);
    ...
    private float GetClosestPoint_Helper(ref Vector3 tVect, float tStep, float BestValue, float tMin, float tMax,
        ref Vector3 BestVect_p, ref Vector3 BestVect_n, bool bMeterCache = false)
    {
        var mDistance = 5000f;
        var tDistance = 0f;
        var cVect = new Vector3(0f, 0f, 0f);
        var pVect = new Vector3(0f, 0f, 0f);
        var bFirstLoopHappened = false;
        var bSetBestValue = false;
        ...
        if (bSetBestValue) BestVect_n = cVect;
    ...
    {
        var Distance1 = Vector3.Distance(tVect, BestVect_p);
        var Distance2 = Vector3.Distance(tVect, BestVect_n);

        if (Distance1 < Distance2) //tMin lean
            return true;
        else //tMax lean
            return false;
    }
    ...
    private Vector3 GetHermiteInternal(int i, double t, bool bTangent = false)
    {
        ...
        var tMaxMag = tRoad.opt_MagnitudeThreshold;
...
//		if(!bGizmoDraw){ return; }
        ...
        var DistanceFromCam = Vector3.SqrMagnitude(Camera.current.transform.position - mNodes[0].transform.position);
    ...
    public bool IsNearIntersection(ref Vector3 tPos, ref float nResult)
    {
        var mCount = GetNodeCount();
        GSDSplineN tNode;
        var MetersToCheck = 75f * (tRoad.opt_LaneWidth / 5f * (tRoad.opt_LaneWidth / 5f));
        float tDist;
        for (var i = 0; i < mCount; i++)
        {
            ...
            if (tNode.bIsIntersection)
            {
                ...
                if (bUseSQ) tDist = Vector3.SqrMagnitude(tPos - tNode.pos);
                //				else{
...
//				}
                ...
                if (tNode.GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.NoTurnLane)
                {
                    if (bUseSQ)
                    {
                        MetersToCheck = MetersToCheck_NoTurnLaneSQ *
                                        (tRoad.opt_LaneWidth / 5f * (tRoad.opt_LaneWidth / 5f));
                        ;
                    }
...
//					}
                ...
                else if (tNode.GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.TurnLane)
                {
                    if (bUseSQ)
                    {
                        MetersToCheck = MetersToCheck_TurnLaneSQ *
                                        (tRoad.opt_LaneWidth / 5f * (tRoad.opt_LaneWidth / 5f));
                        ;
                    }
...
//					}
                ...
                else if (tNode.GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                {
                    if (bUseSQ)
                    {
                        MetersToCheck = MetersToCheck_BothTurnLaneSQ *
                                        (tRoad.opt_LaneWidth / 5f * (tRoad.opt_LaneWidth / 5f));
                        ;
                    }
...
//					}
                ...
                if (tRoad.opt_Lanes == 4)
                    MetersToCheck *= 1.25f;
                else if (tRoad.opt_Lanes == 6) MetersToCheck *= 1.35f;
    ...
    public float IntersectionStrength(ref Vector3 tPos, ref float nResult, ref GSDRoadIntersection tInter,
        ref bool bIsPast, ref float p, ref GSDSplineN fNode)
    {
        var mCount = GetNodeCount();
        float tDist;
        GSDSplineN tNode;
...
//					}
                ...
                if (tDist <= MetersToCheck)
                {
                    if (tNode.GSDRI.bSameSpline)
                    {
                        ...
                        var P1 = tNode.tTime - p;
                        if (P1 < 0f) P1 *= -1f;
                        var P2 = xNode.tTime - p;
                        if (P2 < 0f) P2 *= -1f;
...
//		return false;
    ...
    private void DestroyIntersection(GSDSplineN tNode)
    {
        if (tNode != null)
        {
            if (tNode.bIsEndPoint)
            {
                ...
                {
                    DestroyImmediate(tNode.GSDSpline.mNodes[0].transform.gameObject);
                    tNode.GSDSpline.bSpecialStartControlNode = false;
                }
                else if (tNode.idOnSpline == tNode.GSDSpline.GetNodeCount() - 2 &&
                         tNode.GSDSpline.mNodes[tNode.GSDSpline.GetNodeCount() - 1].bSpecialEndNode_IsEnd)
                {
                    DestroyImmediate(tNode.GSDSpline.mNodes[tNode.GSDSpline.GetNodeCount() - 1].transform.gameObject);
        ...
        if (tNode.Intersection_OtherNode != null)
        {
            ...
            {
                if (tNode.Intersection_OtherNode.idOnSpline == 1 &&
                    tNode.Intersection_OtherNode.GSDSpline.mNodes[0].bSpecialEndNode_IsStart)
                {
                    DestroyImmediate(tNode.Intersection_OtherNode.GSDSpline.mNodes[0].transform.gameObject);
                    tNode.Intersection_OtherNode.GSDSpline.bSpecialStartControlNode = false;
                }
                else if (tNode.Intersection_OtherNode.idOnSpline ==
                    tNode.Intersection_OtherNode.GSDSpline.GetNodeCount() - 2 && tNode.Intersection_OtherNode
                        .GSDSpline.mNodes[tNode.Intersection_OtherNode.GSDSpline.GetNodeCount() - 1]
                        .bSpecialEndNode_IsEnd)
                {
                    DestroyImmediate(tNode.Intersection_OtherNode.GSDSpline
                        .mNodes[tNode.Intersection_OtherNode.GSDSpline.GetNodeCount() - 1].transform.gameObject);
    ...
    public bool IsInBridge(float p)
    {
        ...
        {
            KVP = BridgeParams[i];
            if (GSDRootUtil.IsApproximately(KVP.Key, p, 0.0001f) ||
                GSDRootUtil.IsApproximately(KVP.Value, p, 0.0001f)) return true;
            if (p > KVP.Key && p < KVP.Value) return true;
        }
    ...
    public float BridgeUpComing(float p)
    {
        ...
        for (var i = 0; i < cCount; i++)
        {
            ...
            if (p > KVP.Key && p < KVP.Value) return (KVP.Key - OrigP) / tDist;
        }
    ...
    public bool IsInBridgeTerrain(float p)
    {
        ...
        {
            KVP = BridgeParams[i];
            if (GSDRootUtil.IsApproximately(KVP.Key + 10f / distance, p, 0.0001f) ||
                GSDRootUtil.IsApproximately(KVP.Value - 10f / distance, p, 0.0001f)) return true;
            if (p > KVP.Key + 10f / distance && p < KVP.Value - 10f / distance) return true;
        }
    ...
    public float GetBridgeEnd(float p)
    {
        ...
        {
            KVP = BridgeParams[i];
            if (p >= KVP.Key && p <= KVP.Value) return KVP.Value;
        }
    ...
    private void ActivateEndNodeConnection_Do(GSDSplineN tNode1, GSDSplineN tNode2)
    {
        ...
        var xCount = xSpline.GetNodeCount();
        var mCount = GetNodeCount();
...
//			bSecondNode_End = true;
        ...
        var lWidth1 = tNode1.GSDSpline.tRoad.opt_LaneWidth;
        var lWidth2 = tNode2.GSDSpline.tRoad.opt_LaneWidth;
...
//		}
    ...
    public int GetNodeCountNonNull()
    {
        ...
        for (var i = 0; i < mCount; i++)
            ...
            {
                tCount += 1;
                if (mNodes[i].bIsIntersection && mNodes[i].GSDRI == null) DestroyIntersection(mNodes[i]);
            }
    ...
    public GSDSplineN GetLastLegitimateNode()
    {
        ...
        {
            tNode = mNodes[i];
            if (tNode.IsLegitimate()) return tNode;
        }
    ...
    public GSDSplineN GetPrevLegitimateNode(int tIndex)
    {
        ...
        {
            GSDSplineN tNode = null;
            for (var i = tIndex - 1; i >= 0; i--)
            {
                tNode = mNodes[i];
                if (tNode.IsLegitimateGrade()) return tNode;
            }
    ...
    {
        var mCount = GetNodeCount();
        for (var i = 0; i < mCount; i++) mNodes[i].ClearCuts();
    }
