...
namespace GSD.Threaded
{
    ...
    public static class GSDTerraformingT
    {
        ...
        {
            public List<Roads.GSDRoadUtil.Construction3DTri> triList;
            public Roads.GSDRoadUtil.Construction2DRect tRect;
...
//		}
        ...
        private static void GetTempHeights_Coordinates(ref Vector3 tVect, ref Roads.GSDTerraforming.TempTerrainData TTD,
            out int x, out int y)
        {
        ...
        private static void GetTempDetails_Coordinates(ref Vector3 tVect, ref Roads.GSDTerraforming.TempTerrainData TTD,
            out int x, out int y)
        {
            //Get the normalized position of this game object relative to the terrain:
            var tempCoord = tVect - TTD.TerrainPos;
        ...
        public static void DoRects(GSDSplineC tSpline, Roads.GSDTerraforming.TempTerrainData TTD)
        {
            DoRectsDo(ref tSpline, ref TTD);
        }

        private static void DoRectsDo(ref GSDSplineC tSpline, ref Roads.GSDTerraforming.TempTerrainData TTD)
        {
            var Sep = tSpline.tRoad.RoadWidth() * 0.5f;
            var HeightSep = Sep + tSpline.tRoad.opt_MatchHeightsDistance * 0.5f;
...
//			List<GSD.Roads.GSDRoadUtil.Construction3DTri> triList = new List<GSD.Roads.GSDRoadUtil.Construction3DTri>();
            var TreerectList = new List<Roads.GSDRoadUtil.Construction2DRect>();
            var tStep = tSpline.tRoad.opt_RoadDefinition / tSpline.distance;
//			tStep *= 0.5f;
...
//			float RoadConnection_FinalMax1 = FinalMax; 	//Storage of incremental end values for the road connection mesh construction at the end of this function.
            if (tSpline.bSpecialEndNode_IsStart_Delay)
                StartMin += tSpline.SpecialEndNodeDelay_Start /
                            tSpline
                                .distance; //If there's a start delay (in meters), delay the start of road construction: Due to special control nodes for road connections or 3 way intersections.
            else if (tSpline.bSpecialEndNode_IsEnd_Delay)
                FinalMax -= tSpline.SpecialEndNodeDelay_End /
                            tSpline
                                .distance; //If there's a end delay (in meters), cut early the end of road construction: Due to special control nodes for road connections or 3 way intersections.
...
//			float RoadConnection_FinalMax2 = FinalMax; 	//Storage of incremental end values for the road connection mesh construction at the end of this function.
            ...
            var fValueMod = tSpline.tRoad.opt_RoadDefinition / tSpline.distance;
...
//			bool T3Added = false;
            ...
            var TreeClearDist = tSpline.tRoad.opt_ClearTreesDistance;
            if (TreeClearDist < tSpline.tRoad.RoadWidth()) TreeClearDist = tSpline.tRoad.RoadWidth();
            Roads.GSDRoadUtil.Construction2DRect tRect = null;
            var tGrade = 0f;
            for (var i = StartMin; i < FinalMax; i += tStep)
                if (tSpline.tRoad.opt_HeightModEnabled)
                {
                    ...
                    tIntStrength = tSpline.IntersectionStrength(ref tVect1, ref tIntHeight, ref GSDRI, ref bIsPastInter,
                        ref i, ref xNode);
...
//					bFirstInterNode = false;	
                    ...
                    tIntStrength2 = tSpline.IntersectionStrength(ref tVect2, ref tIntHeight2, ref GSDRI,
                        ref bIsPastInter, ref i, ref xNode);
...
//							T2SUB -= tIntHeight2 - tVect2.y;
                        ...
                        else if (!bIntStr2_Full)
                        {
                            if (tNext + tStep < 1f)
                            {
                                ...
                                tIntStrength2 = tSpline.IntersectionStrength(ref tVect3, ref tIntHeight2, ref GSDRI,
                                    ref bIsPastInter, ref i, ref xNode);
                            }
                            else
                            {
                    ...
                    tXYs.Add(CreateTris(fValue1, fValue2, ref tVect1, ref POS1, ref tVect2, ref POS2, Sep, ref TBMList,
                        ref T1SUB, ref T2SUB, ref TTD, HeightSep));

                    //Details and trees:
                    tRect = SetDetailCoords(i, ref tVect1, ref POS1, ref tVect2, ref POS2,
                        tSpline.tRoad.opt_ClearDetailsDistance, TreeClearDist, ref TTD, ref tSpline);
                    if (tSpline.tRoad.opt_TreeModEnabled && tRect != null) TreerectList.Add(tRect);
                }
                else
                {
                    if (i > 1f) break;
            ...
            if (tSpline.tRoad.bProfiling) UnityEngine.Profiling.Profiler.BeginSample("DoRectsTree");
            if (tSpline.tRoad.opt_TreeModEnabled && TreerectList != null && TreerectList.Count > 0)
            {
                var tCount = TTD.TreeSize;
            ...
            if (!tSpline.tRoad.opt_HeightModEnabled) return;
...
//			tSpline.mNodes[22].tHMList = new List<Vector3>();
            ...
            for (var i = StartMin; i < FinalMax; i += tStep)
            {
                if (TBMList.Count > 0)
                {
                    if (TBMList[0].MaxI < i) CleanupTris(i, ref TBMList);
                }
                else
                {
                    break;
                ...
                {
                    tXY = tXYs[k];
                    tFloat = ProcessCoordinateGrabber(ref i, ref tSpline, ref TTD, ref TBMList, ref tXY, bIsBridge,
                        bIsTunnel);
        ...
        private static void CleanupTris(float CurrentI, ref List<TerrainBoundsMaker> tList)
        {
            ...
            for (var i = 0; i < mCount; i++)
                if (tList[i].MaxI < CurrentI)
                    ...
                    break;
            if (LastIndexToRemove >= 0) tList.RemoveRange(0, LastIndexToRemove);
            //			
...
//			}
        ...
        private static int[] CreateTris(float i, float i2, ref Vector3 tVect1, ref Vector3 POS1, ref Vector3 tVect2,
            ref Vector3 POS2, float Sep, ref List<TerrainBoundsMaker> tList, ref float T1SUB, ref float T2SUB,
            ref Roads.GSDTerraforming.TempTerrainData TTD, float HeightSep)
        {
            ...
            TBM.triList = new List<Roads.GSDRoadUtil.Construction3DTri>();

            TBM.triList.Add(new Roads.GSDRoadUtil.Construction3DTri(lVect1, rVect1, lVect2, i, i2));
            TBM.triList.Add(new Roads.GSDRoadUtil.Construction3DTri(lVect2, rVect1, rVect2, i, i2));
            ...
            TBM.triList.Add(new Roads.GSDRoadUtil.Construction3DTri(lVect1far, lVect1, lVect2far, i, i2));
            TBM.triList.Add(new Roads.GSDRoadUtil.Construction3DTri(lVect2far, lVect1, lVect2, i, i2));
            TBM.triList.Add(new Roads.GSDRoadUtil.Construction3DTri(rVect1, rVect1far, rVect2, i, i2));
            TBM.triList.Add(new Roads.GSDRoadUtil.Construction3DTri(rVect2, rVect1far, rVect2far, i, i2));

            TBM.tRect = new Roads.GSDRoadUtil.Construction2DRect(new Vector2(lVect1far.x, lVect1far.z),
                new Vector2(rVect1far.x, rVect1far.z), new Vector2(rVect2far.x, rVect2far.z),
                new Vector2(lVect2far.x, lVect2far.z), 0f);
//			tRect.MinI = i;
//			tRect.MaxI = i2;
        ...
        private static Roads.GSDRoadUtil.Construction2DRect SetDetailCoords(float param, ref Vector3 tVect1,
            ref Vector3 POS1, ref Vector3 tVect2, ref Vector3 POS2, float Sep, float TreeSep,
            ref Roads.GSDTerraforming.TempTerrainData TTD, ref GSDSplineC tSpline)
        {
            ...
            var bIsInBridge = tSpline.IsInBridgeTerrain(param);
            var bIsInTunnel = tSpline.IsInTunnelTerrain(param);
            int x2, y2, x3, y3;
            ...
            var tDiff1 = TTD.heights[x2, y2] * (float) TTD.TerrainSize.y - tVect1.y;
            ...
            if (x3 == -1) return null;
            if (y3 == -1) return null;
            var tDiff2 = TTD.heights[x3, y3] * (float) TTD.TerrainSize.y - tVect2.y;
            ...
            if (tSpline.tRoad.opt_TreeModEnabled)
            {
                ...
                if (bIsInBridge && !bQuit)
                {
                    ...
                    if (tDiff1 > tSpline.tRoad.opt_ClearTreesDistanceHeight) bQuit = true;
                    if (tDiff2 > tSpline.tRoad.opt_ClearTreesDistanceHeight) bQuit = true;
                }
                ...
                if (bIsInTunnel && !bQuit)
                {
                    if (tDiff1 < 0f)
                    {
                        if (tDiff1 * -1f > tSpline.tRoad.opt_ClearTreesDistanceHeight) bQuit = true;
                    }
                    else
                    {
                        if (tDiff1 > tSpline.tRoad.opt_ClearTreesDistanceHeight * 0.1f) bQuit = true;
                    }

                    if (tDiff2 < 0f)
                    {
                        if (tDiff2 * -1f > tSpline.tRoad.opt_ClearTreesDistanceHeight) bQuit = true;
                    }
                    else
                    {
                        if (tDiff2 > tSpline.tRoad.opt_ClearTreesDistanceHeight * 0.1f) bQuit = true;
                    }
                }
                ...
                if (!bQuit)
                {
                    ...
                    tRect = new Roads.GSDRoadUtil.Construction2DRect(new Vector2(lVect1far.x, lVect1far.z),
                        new Vector2(rVect1far.x, rVect1far.z), new Vector2(rVect2far.x, rVect2far.z),
                        new Vector2(lVect2far.x, lVect2far.z), 0f);
                }
            }
            ...
            if (tSpline.tRoad.opt_DetailModEnabled)
            {
                if (bIsInBridge || bIsInTunnel)
                {
                    ...
                    if (tDiff1 > tSpline.tRoad.opt_ClearDetailsDistanceHeight) bQuit = true;
                    if (tDiff2 > tSpline.tRoad.opt_ClearDetailsDistanceHeight) bQuit = true;

                    if (bQuit) return tRect;
                }
...
//							}
        ...
        private static float ProcessCoordinateGrabber(ref float param, ref GSDSplineC tSpline,
            ref Roads.GSDTerraforming.TempTerrainData TTD, ref List<TerrainBoundsMaker> tList, ref int[] tXY,
            bool bIsBridge, bool bIsTunnel)
        {
            var MinX = tXY[0];
            var MinY = tXY[1];
            var MaxX = tXY[2];
...
//			bool bOneHit = false;
            ...
            for (var k = MinY; k <= MaxY; k++)
                if (TTD.tHeights[i, k] != true)
                {
                    ...
                    AdjustedTerrainVect_Tri(ref param, out bAdjusted, out tHeight, ref xVect, ref tList, bIsBridge,
                        bIsTunnel);
...
//							bOneHit = true;
        ...
        private static Vector3 ConvertTerrainCoordToWorldVect(int x, int y, float tHeight,
            ref Roads.GSDTerraforming.TempTerrainData TTD)
        {
            //Get the normalized position of this game object relative to the terrain:
            var x1 = x / ((float) TTD.HM - 1f);
            x1 = x1 * TTD.TerrainSize.x;

            var z1 = y / ((float) TTD.HM - 1f);
            z1 = z1 * TTD.TerrainSize.z;

            var y1 = tHeight * TTD.TerrainSize.y;
        ...
        private static void AdjustedTerrainVect_Tri(ref float param, out bool bAdjusted, out float tHeight,
            ref Vector3 xVect, ref List<TerrainBoundsMaker> tList, bool bIsBridge, bool bIsTunnel)
        {
            var OrigHeight = xVect.y;
            var mCount = tList.Count;
            var tCount = 0;
            Roads.GSDRoadUtil.Construction3DTri tTri;
            ...
            {
                TBM = tList[i];
                if (param < TBM.MinI) return;
                if (param > TBM.MaxI) continue;
    ...
    public static class GSDRoadCreationT
    {
        ...
        private static void RoadJob_DoPrelim(ref GSDRoad tRoad)
        {
            ...
            var ShoulderR_OuterAngle = 0f;
            if (ShoulderR_OuterAngle < 0f)
            {
            }

            var ShoulderL_OuterAngle = 0f;
...
//			float OrigStep = 0.06f;
            var Step = tRoad.opt_RoadDefinition / tSpline.distance;
//			float AngleStep = 5f;
...
//			float[] HeightChecks = new float[5];
            ...
            var bTempbridge = false;
            if (bTempbridge == false)
            {
            }

            var bBridgeInitial = false;
            if (bBridgeInitial == false)
            {
            }

            var bBridgeLast = false;
...
//			int BridgeRIndex;
            ...
            var bTempTunnel = false;
            if (bTempTunnel == false)
            {
            }

            var bTunnelInitial = false;
            if (bTunnelInitial == false)
            {
            }

            var bTunnelLast = false;
            if (bTunnelLast == false)
            {
            }

            var TunnelUpComing = 0f;
...
//			float tIntDistCheck = 75f;
            GSDRoadIntersection GSDRI = null;
            var bIsPastInter = false;
            var bMaxIntersection = false;
            var bWasPrevMaxInter = false;
            GSDSplineN xNode = null;
            var tInterSubtract = 4f;
            var tLastInterHeight = -4f;
            var bOverrideRampR = false;
            var bOverrideRampL = false;
            var RampR_Override = default(Vector3);
            var RampL_Override = default(Vector3);
            var bFirstInterNode = false;
            var bInter_PrevWasCorner = false;
            if (bInter_PrevWasCorner == false)
            {
            }

            var bInter_CurreIsCorner = false;
            var bInter_CurreIsCornerRR = false;
            var bInter_CurreIsCornerRL = false;
            var bInter_CurreIsCornerLL = false;
            var bInter_CurreIsCornerLR = false;
            var bInter_PrevWasCornerRR = false;
            var bInter_PrevWasCornerRL = false;
            var bInter_PrevWasCornerLL = false;
            var bInter_PrevWasCornerLR = false;
            ...
            var bInterTestAddAfterR = false;
            if (bInterTestAddAfterR == false)
            {
            }

            var bInterTestAddAfterL = false;
...
//			Vector3 InterTestVect4 = default(Vector3);
            var bShoulderSkipR = false;
            var bShoulderSkipL = false;
            var bShrinkRoadB = false;
            var bShrinkRoadFNext = false;
            var bShrinkRoadF = false;
            var bIsNextInter = false;
            GSDSplineN cNode = null;
            var NodeID = -1;
            var NodeIDPrev = -1;
            var NodeCount = tSpline.GetNodeCount();
            var bDynamicCut = false;
            ...
            var bImmuneR = false;
            var bImmuneL = false;
            var bSpecAddedL = false;
            if (bSpecAddedL == false)
            {
            }

            var bSpecAddedR = false;
            if (bSpecAddedR == false)
            {
            }

            var bTriggerInterAddition = false;
            var bSpecialThreeWayIgnoreR = false;
            var bSpecialThreeWayIgnoreL = false;
//			int eCount = -1;
//			int eIndex = -1;
//			int uCount = -1;
//			int uIndex = -1;
            ...
            var tParam2 = 0f;
            var tParam1 = 0f;
            var bRecordShoulderForNormals = false;
            var bRecordShoulderLForNormals = false;

            //Unused for now, for later partial construction methods:
            var bInterseOn = tRoad.RCS.bInterseOn;
...
//			}	
            ...
            if (bInterseOn) RoadJob_Prelim_Inter(ref tRoad);
            ...
            if (tRoad.opt_bRoadCuts || tRoad.opt_bDynamicCuts) tRoad.RCS.RoadCutNodes.Add(tSpline.mNodes[0]);
            ...
            if (tSpline.bSpecialStartControlNode) //If ends in control node, end construction before the control node:
                StartMin = tSpline.mNodes[1].tTime;
            var bFinalEnd = false;
            var RoadConnection_StartMin1 =
                StartMin; //Storage of incremental start values for the road connection mesh construction at the end of this function.
            var RoadConnection_FinalMax1 =
                FinalMax; //Storage of incremental end values for the road connection mesh construction at the end of this function.
            if (tSpline.bSpecialEndNode_IsStart_Delay)
                StartMin += tSpline.SpecialEndNodeDelay_Start /
                            tSpline
                                .distance; //If there's a start delay (in meters), delay the start of road construction: Due to special control nodes for road connections or 3 way intersections.
...
//			float cDist = 0f;
            ...
            if (IsApproximately(StartMin, 0f, 0.0001f)) kSkip = false;
            if (IsApproximately(FinalMax, 1f, 0.0001f)) kSkipFinal = true;
...
//				bStartMinEnabled = true;
            ...
            while (!bFinalEnd && spamcheck1 < spamcheckmax1)
            {
                ...
                if (NodeID != NodeIDPrev && (tRoad.opt_bRoadCuts || tRoad.opt_bDynamicCuts)
                    ...
                    if (NodeID > StartMinIndex1 && NodeID < NodeCount - 1 && !cNode.bIsIntersection &&
                        !cNode.bSpecialEndNode)
                    {
                        ...
                        if (tRoad.opt_bShoulderCuts && bDynamicCut)
                        {
                            ...
                            tRoad.RCS.ShoulderCutsL.Add(tRoad.RCS.ShoulderL_Vectors
                                .Count); //Add the vector index to cut later.
                            tRoad.RCS.ShoulderCutsLNodes
                                .Add(cNode); //Store the node which was at the beginning of this cut.
                            tRoad.RCS.ShoulderCutsR.Add(tRoad.RCS.ShoulderR_Vectors
                                .Count); //Add the vector index to cut later.
                            tRoad.RCS.ShoulderCutsRNodes
                                .Add(cNode); //Store the node which was at the beginning of this cut.
                        }
                    }
                ...
                NodeIDPrev =
                    NodeID; //Store the previous node ID for the next round. Done now with road cuts as far as this function is concerned.
                ...
                if (bIsBridge)
                {
                    ...
                    if (!bTempbridge) bBridgeLast = true;
                }
                ...
                if (bIsTunnel)
                {
                    ...
                    if (!bTempTunnel) bTunnelLast = true;
                }
...
//				bTempYWasNegative = false;
                if (TempY < 0f) //					bTempYWasNegative = true;
                    TempY *= -1f;
                ...
                {
                    bIsPastInter = false; //If past intersection
                    tIntStrength = tRoad.GSDSpline.IntersectionStrength(ref tVect, ref tIntHeight, ref GSDRI,
                        ref bIsPastInter, ref i, ref xNode);
...
//					}
                    ...
                    var tInterStrNext =
                        tRoad.GSDSpline.IntersectionStrength_Next(tSpline.GetSplineValue(tParam2, false));
                    ...
                    if (bIsPastInter)
                    {
                        var bLLtoRL = bFirstInterNode;
                        var bRLtoRR = !bFirstInterNode;
                        ...
                        {
                            tempIVect = tVect;
                            if (!xNode.iConstruction.bFLane1Done && !GSDRI.Contains(ref tempIVect) &&
                                !GSDRI.ContainsLine(tempIVect, rVect))
                            {
                                if (f0LAdded)
                                {
                        ...
                        {
                            tempIVect = lVect;
                            if (!xNode.iConstruction.bFLane2Done && !GSDRI.Contains(ref tempIVect) &&
                                !GSDRI.ContainsLine(tempIVect, rVect))
                            {
                                if (f1LAdded)
                                {
                        ...
                        if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                        {
                            ...
                            if (!xNode.iConstruction.bFLane3Done && !GSDRI.Contains(ref tempIVect) &&
                                !GSDRI.ContainsLine(lVect, tempIVect))
                            {
...
//							xNode.iConstruction.bFDone = true;
                            ...
                            if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                            {
                                ...
                                if (tRoad.opt_Lanes == 2)
                                    xNode.iConstruction.iFLane1R.Add(
                                        GVC((rVect_Next - lVect_Next) * 0.475f + lVect_Next, tIntHeight));
                                else if (tRoad.opt_Lanes == 4)
                                    xNode.iConstruction.iFLane1R.Add(
                                        GVC((rVect_Next - lVect_Next) * 0.488f + lVect_Next, tIntHeight));
                                else if (tRoad.opt_Lanes == 6)
                                    xNode.iConstruction.iFLane1R.Add(
                                        GVC((rVect_Next - lVect_Next) * 0.492f + lVect_Next, tIntHeight));

                                if (tRoad.opt_Lanes == 2)
                                    xNode.iConstruction.iFLane3L.Add(GVC((rVect_Next - lVect_Next) * 0.03f + lVect_Next,
                                        tIntHeight));
                                else if (tRoad.opt_Lanes == 4)
                                    xNode.iConstruction.iFLane3L.Add(
                                        GVC((rVect_Next - lVect_Next) * 0.015f + lVect_Next, tIntHeight));
                                else if (tRoad.opt_Lanes == 6)
                                    xNode.iConstruction.iFLane3L.Add(GVC((rVect_Next - lVect_Next) * 0.01f + lVect_Next,
                                        tIntHeight));
                    ...
                    else
                    {
                        ...
                        if (xNode.iConstruction.iBLane0R.Count == 0)
                        {
                            ...
                            if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                            {
                                ...
                                xNode.iConstruction.iBLane1R.Add(
                                    tVect_Prev + new Vector3(LaneWidth * 0.05f * POS.normalized.z, 0,
...
//							goto InterSkip;
                        ...
                        bLine = false;
                        if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                            bLine = !GSDRI.ContainsLine(tempIVect,
                        ...
                        {
                            //Finalize lane 0:
                            InterFinalizeiBLane0(ref xNode, ref GSDRI, ref tIntHeight, bLRtoRR, bLLtoLR,
                                bFirstInterNode);
                        }
                        ...
                        {
                            if (b0LAdded && (xNode.iConstruction.iBLane0L.Count != 2 ||
                                             GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.NoTurnLane))
                            {
                                xNode.iConstruction.iBLane0R.Add(GVC(tempIVect, tIntHeight));
                                b0RAdded = true;
                            }
                        ...
                        {
                            //Finalize lane 1:
                            InterFinalizeiBLane1(ref xNode, ref GSDRI, ref tIntHeight, bLRtoRR, bLLtoLR,
                                bFirstInterNode, ref b0LAdded, ref b1RAdded);
                        }

                        //Line 2:
                        if (xNode.iConstruction.iBLane1R.Count == 0 &&
                            xNode.GSDRI.rType != GSDRoadIntersection.RoadTypeEnum.NoTurnLane)
                        {
                        ...
                        else
                        {
                            ...
                            {
                                InterFinalizeiBLane2(ref xNode, ref GSDRI, ref tIntHeight, bLRtoRR, bLLtoLR,
                                    bFirstInterNode, ref b2LAdded, ref b1LAdded, ref b0LAdded, ref b1RAdded);
                            }
                        }
                        ...
                        tempIVect = tVect + new Vector3((LaneWidth * 0.5f + RoadSeperation) * POS.normalized.z, 0,
                            (LaneWidth * 0.5f + RoadSeperation) * -POS.normalized.x);
                        if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.NoTurnLane) tempIVect = rVect;
                        if (!xNode.iConstruction.bBLane3Done && !GSDRI.ContainsLine(rVect, tempIVect) &&
                            !GSDRI.ContainsLine(rVect, lVect))
                        {
                        ...
                        {
                            InterFinalizeiBLane3(ref xNode, ref GSDRI, ref tIntHeight, bLRtoRR, bLLtoLR,
                                bFirstInterNode, ref b2LAdded, ref b1LAdded, ref b0LAdded, ref b1RAdded);
                        }
                    }
                }
...
//					if(!Mathf.Approximately(tIntStrength_temp,0f)){ ShoulderL_lVect.y = (tIntStrength_temp*tIntHeight) + ((1-tIntStrength_temp)*ShoulderL_lVect.y); }
                ...
                else
                {
                    RampR_R = tVect + new Vector3(RampOuterWidthR * POS.normalized.z, 0,
                        RampOuterWidthR * -POS.normalized.x) + gHeight;
                    ...
                    RampL_L = tVect + new Vector3(RampOuterWidthL * -POS.normalized.z, 0,
                        RampOuterWidthL * POS.normalized.x) + gHeight;
...
//					uIndex = -1;
                    ...
                    var tempRoadDef = Mathf.Clamp(tRoad.opt_LaneWidth, 3f, 5f);
                    ...
                    if (t2DDist < mCornerDist)
                    {
                            ...
                            if (vList != null)
                            {
                                for (var m = 0; m < vList.Count; m++)
                                    if (Vector3.SqrMagnitude(vList[m] - ShoulderR_lVect) < 0.01f)
                                        if (!(IsApproximately(vList[m].x, GSDRI.CornerRR.x, 0.01f) &&
                                              IsApproximately(vList[m].z, GSDRI.CornerRR.z, 0.01f)))
                                            eList.Add(m);
                        ...
                        RampR_Override = new Vector3(GSDRI.CornerRR_RampOuter.x, tIntHeight,
                            GSDRI.CornerRR_RampOuter.z);
                    ...
                    else
                    {
                        ...
                        if (t2DDist < mCornerDist)
                        {
                                ...
                                if (vList != null)
                                {
                                    for (var m = 0; m < vList.Count; m++)
                                        if (Vector3.SqrMagnitude(vList[m] - ShoulderL_rVect) < 0.01f)
                                            if (!(IsApproximately(vList[m].x, GSDRI.CornerRR.x) &&
                                                  IsApproximately(vList[m].z, GSDRI.CornerRR.z)))
                                                eList.Add(m);
                                    for (var m = eList.Count - 1; m >= 0; m--) vList.RemoveAt(eList[m]);
                                }
                            ...
                            RampL_Override = new Vector3(GSDRI.CornerRR_RampOuter.x, tIntHeight,
                                GSDRI.CornerRR_RampOuter.z);
                    ...
                    if (t2DDist < mCornerDist)
                    {
                        ...
                        if (bFirstInterNode)
                        {
                            ...
                            if (vList != null)
                            {
                                for (var m = 1; m < vList.Count; m++)
                                    if (Vector3.SqrMagnitude(vList[m] - ShoulderR_lVect) < 0.01f)
                                        if (!(IsApproximately(vList[m].x, GSDRI.CornerRL.x) &&
                                              IsApproximately(vList[m].z, GSDRI.CornerRL.z)))
                                            eList.Add(m);
                                for (var m = eList.Count - 1; m >= 0; m--) vList.RemoveAt(eList[m]);
                            }
                                                ...
                                                if (m == vList.Count - 1)
                                                {
                                                    if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.NoTurnLane)
                                                        ...
                                                        b2RAdded = false;
                                                             ...
                                                             GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                                                        b3RAdded = false;
                                                }
                                            }
                        ...
                        RampR_Override = new Vector3(GSDRI.CornerRL_RampOuter.x, tIntHeight,
                            GSDRI.CornerRL_RampOuter.z);
                        bRecordShoulderForNormals = true;
                    }
                    else
                    {
                        ...
                        if (t2DDist < mCornerDist)
                        {
                            ...
                            RampL_Override = new Vector3(GSDRI.CornerRL_RampOuter.x, tIntHeight,
                                GSDRI.CornerRL_RampOuter.z);
                            bRecordShoulderLForNormals = true;
                        }
                    }
                    ...
                    if (t2DDist < mCornerDist)
                    {
                        ...
                        if (!bFirstInterNode)
                        {
                            ...
                            if (vList != null)
                            {
                                for (var m = 0; m < vList.Count; m++)
                                    if (Vector3.SqrMagnitude(vList[m] - ShoulderR_lVect) < 0.01f)
                                        if (!(IsApproximately(vList[m].x, GSDRI.CornerLR.x) &&
                                              IsApproximately(vList[m].z, GSDRI.CornerLR.z)))
                                            eList.Add(m);
                                for (var m = eList.Count - 1; m >= 0; m--) vList.RemoveAt(eList[m]);
                            }
                        ...
                        RampR_Override = new Vector3(GSDRI.CornerLR_RampOuter.x, tIntHeight,
                            GSDRI.CornerLR_RampOuter.z);
                        bRecordShoulderForNormals = true;
                    }
                    else
                    {
                        ...
                        if (t2DDist < mCornerDist)
                        {
                            ...
                            RampL_Override = new Vector3(GSDRI.CornerLR_RampOuter.x, tIntHeight,
                                GSDRI.CornerLR_RampOuter.z);
                            bRecordShoulderLForNormals = true;
                        }
                    }
                    ...
                    if (t2DDist < mCornerDist)
                    {
                        ...
                        if (!bFirstInterNode)
                        {
                            ...
                            if (vList != null)
                            {
                                for (var m = 1; m < vList.Count; m++)
                                    if (Vector3.SqrMagnitude(vList[m] - ShoulderR_lVect) < 0.01f)
                                        if (!(IsApproximately(vList[m].x, GSDRI.CornerLL.x) &&
                                              IsApproximately(vList[m].z, GSDRI.CornerLL.z)))
                                            eList.Add(m);
                                for (var m = eList.Count - 1; m >= 0; m--) vList.RemoveAt(eList[m]);
                            }
                        ...
                        RampR_Override = new Vector3(GSDRI.CornerLL_RampOuter.x, tIntHeight,
                            GSDRI.CornerLL_RampOuter.z);
                        bRecordShoulderForNormals = true;
                    }
                    else
                    {
                        ...
                        if (t2DDist < mCornerDist)
                        {
                            ...
                            RampL_Override = new Vector3(GSDRI.CornerLL_RampOuter.x, tIntHeight,
                                GSDRI.CornerLL_RampOuter.z);
                            bRecordShoulderLForNormals = true;
                        }
                    }
                ...
                if (!bIsBridge && !bIsTunnel && bMaxIntersection && bWasPrevMaxInter && bInterseOn)
                {
                    ...
                    if (xNode.bIsEndPoint)
                    {
                        ...
                        {
                            if (i < xNode.tTime) bGoAhead = false;
                        }
                        else
                        {
                            if (i > xNode.tTime) bGoAhead = false;
                    ...
                    {
                        var vRect = new Roads.GSDRoadUtil.Construction2DRect(
...
//						tObj.transform.name = "temp22";
                ...
                else
                {
                    RampR_R = tVect + new Vector3(RampOuterWidthR * POS.normalized.z, 0,
                        RampOuterWidthR * -POS.normalized.x) + gHeight;
                    ;
                    if (bOverrideRampR) RampR_R = RampR_Override;
                    SetVectorHeight2(ref RampR_R, ref i, ref tSpline.HeightHistory, ref tSpline);
                    RampR_R.y -= 0.35f;
                    ...
                    RampL_L = tVect + new Vector3(RampOuterWidthL * -POS.normalized.z, 0,
                        RampOuterWidthL * POS.normalized.x) + gHeight;
                    ;
                    if (bOverrideRampL) RampL_L = RampL_Override;
                    SetVectorHeight2(ref RampL_L, ref i, ref tSpline.HeightHistory, ref tSpline);
                    RampL_L.y -= 0.35f;
                    bOverrideRampR = false;
                    bOverrideRampL = false;
                ...
                if (bMaxIntersection && bInterseOn)
                {
                    if (bFirstInterNode)
                    {
                        if (bInter_PrevWasCornerLR && bInter_CurreIsCornerLL ||
                            bInter_PrevWasCornerRR && bInter_CurreIsCornerRL) bTriggerInterAddition = true;
                    }
                    else
                    {
                        if (!GSDRI.bFlipped)
                        {
                            if (bInter_PrevWasCornerLL && bInter_CurreIsCornerRL ||
                                bInter_PrevWasCornerLR && bInter_CurreIsCornerRR ||
                                bInter_PrevWasCornerRR && bInter_CurreIsCornerLR) bTriggerInterAddition = true;
                        }
                        else
                        {
                            if (bInter_PrevWasCornerRR && bInter_CurreIsCornerLR ||
                                bInter_PrevWasCornerLR && bInter_CurreIsCornerRR ||
                                bInter_PrevWasCornerRL && bInter_CurreIsCornerLL) bTriggerInterAddition = true;
                        }
                    }

                    if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes) bTriggerInterAddition = false;
                    ...
                    if (GSDRI.IgnoreSide > -1)
                    {
                        ...
                        {
                            //RL to LL:
                            if (!bFirstInterNode && (bInter_PrevWasCornerRL && bInter_CurreIsCornerLL ||
                                                     bInter_PrevWasCornerLL && bInter_CurreIsCornerRL))
                            {
                                //bTriggerInterAddition = false;	
                                if (GSDRI.bFlipped)
                        ...
                        {
                            //LR to RR:
                            if (!bFirstInterNode && (bInter_PrevWasCornerRR && bInter_CurreIsCornerLR ||
                                                     bInter_PrevWasCornerLR && bInter_CurreIsCornerRR))
                            {
                                //bTriggerInterAddition = false;	
                                if (GSDRI.bFlipped)
...
//				i+=Step;//Master step incrementer.
            ...
            if (bInterseOn) RoadJob_Prelim_FinalizeInter(ref tRoad);
            if (tRoad.bProfiling) UnityEngine.Profiling.Profiler.EndSample();
...
//			float RampPercent = 0.2f;
            ...
            {
                var RoadConn_verts = new Vector3[4];
                var rrCount = tRoad.RCS.ShoulderR_Vectors.Count;
                ...
                RoadConn_normals[3] = -Vector3.forward;
                var RoadConn_uv = new Vector2[4];
                var tMod = RoadWidth / tSpline.SpecialEndNodeDelay_End_Result / 2f;
        ...
        private static void InterFinalizeiBLane0(ref GSDSplineN xNode, ref GSDRoadIntersection GSDRI,
            ref float tIntHeight, bool bLRtoRR, bool bLLtoLR, bool bFirstInterNode)
        {
        ...
        private static void InterFinalizeiBLane1(ref GSDSplineN xNode, ref GSDRoadIntersection GSDRI,
            ref float tIntHeight, bool bLRtoRR, bool bLLtoLR, bool bFirstInterNode, ref bool b0LAdded,
            ref bool b1RAdded)
        {
        ...
        private static void InterFinalizeiBLane2(ref GSDSplineN xNode, ref GSDRoadIntersection GSDRI,
            ref float tIntHeight, bool bLRtoRR, bool bLLtoLR, bool bFirstInterNode, ref bool b2LAdded,
            ref bool b1LAdded, ref bool b0LAdded, ref bool b1RAdded)
        {
            ...
            if (b1LAdded && !xNode.iConstruction.bBLane1Done_Final)
            {
                ...
                b1LAdded = false;
                InterFinalizeiBLane1(ref xNode, ref GSDRI, ref tIntHeight, bLRtoRR, bLLtoLR, bFirstInterNode,
                    ref b0LAdded, ref b1RAdded);
            }
            ...
            if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes ||
                GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.TurnLane)
            {
                if (GSDRI.bFlipped && !bFirstInterNode)
                {
                    if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
        ...
        private static void InterFinalizeiBLane3(ref GSDSplineN xNode, ref GSDRoadIntersection GSDRI,
            ref float tIntHeight, bool bLRtoRR, bool bLLtoLR, bool bFirstInterNode, ref bool b2LAdded,
            ref bool b1LAdded, ref bool b0LAdded, ref bool b1RAdded)
        {
            if (b2LAdded && !xNode.iConstruction.bBLane2Done_Final)
            {
                ...
                b2LAdded = false;
                InterFinalizeiBLane2(ref xNode, ref GSDRI, ref tIntHeight, bLRtoRR, bLLtoLR, bFirstInterNode,
                    ref b2LAdded, ref b1LAdded, ref b0LAdded, ref b1RAdded);
            }
        ...
        private static void RoadJob_Prelim_Inter(ref GSDRoad tRoad)
        {
            ...
            var POS = default(Vector3);
            var bIsPastInter = false;
            var bOldMethod = false;
            //bool bCancel = false; if (bTempCancel) { }
            var bTempCancel = false;
            if (bTempCancel)
            {
            }
            ...
            var mCount = tSpline.GetNodeCount();
            var PreInter_RoadWidthMod = 4.5f;
            if (!bOldMethod) PreInter_RoadWidthMod = 5.5f;
            var preInterDistance = tSpline.RoadWidth * PreInter_RoadWidthMod / tSpline.distance;
            ...
            for (var j = 0; j < mCount; j++)
            {
                ...
                if (tSpline.mNodes[j].bIsIntersection)
                {
                    ...
                    if (iNode.iConstruction == null)
                        iNode.iConstruction = new Roads.GSDIntersections.iConstructionMaker();
                    ...
                    try
                    {
                        if (!iNode.iConstruction.tempconstruction_HasProcessed_Inter1)
                        {
                            preInterDistance = iNode.GSDSpline.RoadWidth * PreInter_RoadWidthMod /
                                               iNode.GSDSpline.distance;
                            iNode.iConstruction.tempconstruction_InterStart = iNode.tTime - preInterDistance;
                            iNode.iConstruction.tempconstruction_InterEnd = iNode.tTime + preInterDistance;
                            if (iNode.iConstruction.tempconstruction_InterStart > 1f)
                                iNode.iConstruction.tempconstruction_InterStart = 1f;
                ...
                if (oNode1.bIsIntersection)
                {
                    ...
                    try
                    {
                        if (oNode1.iConstruction.tempconstruction_HasProcessed_Inter2 &&
                            oNode2.iConstruction.tempconstruction_HasProcessed_Inter2) continue;
                    }
                    catch
                    {
                        continue;
                    ...
                    if (!oNode1.iConstruction.tempconstruction_HasProcessed_Inter2)
                    {
                        ...
                        for (var i = oNode1.iConstruction.tempconstruction_InterStart;
                            i < oNode1.iConstruction.tempconstruction_InterEnd;
                            i += PreInterPrecision1)
                        {
                            ...
                            {
                                PreInterVectR = PreInterVect + new Vector3(RoadSeperation_NoTurn * POS.normalized.z, 0,
                                    RoadSeperation_NoTurn * -POS.normalized.x);
                                PreInterVectL = PreInterVect + new Vector3(RoadSeperation_NoTurn * -POS.normalized.z, 0,
                                    RoadSeperation_NoTurn * POS.normalized.x);
                            }
                            ...
                            if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                            {
                                PreInterVectR_RightTurn = PreInterVect + new Vector3(RoadSep2Lane * POS.normalized.z, 0,
                                    RoadSep2Lane * -POS.normalized.x);
                                oNode1.iConstruction.tempconstruction_R_RightTurn.Add(
                                    ConvertVect3_To_Vect2(PreInterVectR_RightTurn));

                                PreInterVectL_RightTurn = PreInterVect + new Vector3(RoadSep2Lane * -POS.normalized.z,
                                    0, RoadSep2Lane * POS.normalized.x);
                                oNode1.iConstruction.tempconstruction_L_RightTurn.Add(
                                    ConvertVect3_To_Vect2(PreInterVectL_RightTurn));
                            }
                        }
                    }
                    ...
                    if (oNode2.iConstruction == null)
                        oNode2.iConstruction = new Roads.GSDIntersections.iConstructionMaker();
                    if (!oNode2.iConstruction.tempconstruction_HasProcessed_Inter2)
                    {
                        ...
                        for (var i = oNode2.iConstruction.tempconstruction_InterStart;
                            i < oNode2.iConstruction.tempconstruction_InterEnd;
                            i += PreInterPrecision2)
                        {
                            ...
                            if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                            {
                                PreInterVectR_RightTurn = PreInterVect + new Vector3(RoadSep2Lane * POS.normalized.z, 0,
                                    RoadSep2Lane * -POS.normalized.x);
                                oNode2.iConstruction.tempconstruction_R_RightTurn.Add(
                                    ConvertVect3_To_Vect2(PreInterVectR_RightTurn));

                                PreInterVectL_RightTurn = PreInterVect + new Vector3(RoadSep2Lane * -POS.normalized.z,
                                    0, RoadSep2Lane * POS.normalized.x);
                                oNode2.iConstruction.tempconstruction_L_RightTurn.Add(
                                    ConvertVect3_To_Vect2(PreInterVectL_RightTurn));
                            }
                        }
                    }
                    ...
                    var bFlipped = false;
                    var bFlippedSet = false;
                    var hCount1 = oNode1.iConstruction.tempconstruction_R.Count;
                    var hCount2 = oNode2.iConstruction.tempconstruction_R.Count;
                    var N1RCount = oNode1.iConstruction.tempconstruction_R.Count;
                    var N1LCount = oNode1.iConstruction.tempconstruction_L.Count;
                    var N2RCount = oNode2.iConstruction.tempconstruction_R.Count;
                    var N2LCount = oNode2.iConstruction.tempconstruction_L.Count;
                    ...
                    for (var h = 0; h < hCount1; h++)
                    {
                        ...
                        {
                            if (k < N2RCount)
                                if (Vector2.Distance(oNode1.iConstruction.tempconstruction_R[h],
                                    oNode2.iConstruction.tempconstruction_R[k]) < tRoad.opt_RoadDefinition)
                                {
                                    bFlipped = false;
                                    bFlippedSet = true;
                                    break;
                            ...
                            if (k < N2LCount)
                                if (Vector2.Distance(oNode1.iConstruction.tempconstruction_R[h],
                                    oNode2.iConstruction.tempconstruction_L[k]) < tRoad.opt_RoadDefinition)
                                {
                                    bFlipped = true;
                                    bFlippedSet = true;
                                    break;
                    ...
                    if (GSDRI.bFirstSpecial_First)
                    {
                        ...
                        if (GSDRI.iStopType == GSDRoadIntersection.iStopTypeEnum.StopSign_AllWay)
                            GSDRI.IgnoreCorner = 0;
                                 ...
                                 GSDRI.iStopType == GSDRoadIntersection.iStopTypeEnum.TrafficLight2)
                            GSDRI.IgnoreCorner = 1;
                    ...
                    if (!bOldMethod)
                    {
                        ...
                        if (!bFlipped)
                        {
                            if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                                tFoundVectRR = IntersectionCornerCalc(
                                    ref oNode1.iConstruction.tempconstruction_R_RightTurn,
                                    ref oNode2.iConstruction.tempconstruction_R);
                            else
                                tFoundVectRR = IntersectionCornerCalc(ref oNode1.iConstruction.tempconstruction_R,
                                    ref oNode2.iConstruction.tempconstruction_R);
                        }
                        else
                        {
                            if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                                tFoundVectRR = IntersectionCornerCalc(
                                    ref oNode1.iConstruction.tempconstruction_R_RightTurn,
                                    ref oNode2.iConstruction.tempconstruction_L);
                            else
                                tFoundVectRR = IntersectionCornerCalc(ref oNode1.iConstruction.tempconstruction_R,
                                    ref oNode2.iConstruction.tempconstruction_L);
                        }
                        ...
                        {
                            if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                                tFoundVectRL = IntersectionCornerCalc(ref oNode1.iConstruction.tempconstruction_R,
                                    ref oNode2.iConstruction.tempconstruction_L_RightTurn);
                            else
                                tFoundVectRL = IntersectionCornerCalc(ref oNode1.iConstruction.tempconstruction_R,
                                    ref oNode2.iConstruction.tempconstruction_L);
                        }
                        else
                        {
                            if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                                tFoundVectRL = IntersectionCornerCalc(ref oNode1.iConstruction.tempconstruction_R,
                                    ref oNode2.iConstruction.tempconstruction_R_RightTurn);
                            else
                                tFoundVectRL = IntersectionCornerCalc(ref oNode1.iConstruction.tempconstruction_R,
                                    ref oNode2.iConstruction.tempconstruction_R);
                        }
                        ...
                        if (!bFlipped)
                        {
                            if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                                tFoundVectLL = IntersectionCornerCalc(
                                    ref oNode1.iConstruction.tempconstruction_L_RightTurn,
                                    ref oNode2.iConstruction.tempconstruction_L);
                            else
                                tFoundVectLL = IntersectionCornerCalc(ref oNode1.iConstruction.tempconstruction_L,
                                    ref oNode2.iConstruction.tempconstruction_L);
                        }
                        else
                        {
                        ...
                        {
                            if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                                tFoundVectLR = IntersectionCornerCalc(ref oNode1.iConstruction.tempconstruction_L,
                                    ref oNode2.iConstruction.tempconstruction_R_RightTurn);
                            else
                                tFoundVectLR = IntersectionCornerCalc(ref oNode1.iConstruction.tempconstruction_L,
                                    ref oNode2.iConstruction.tempconstruction_R);
                    ...
                    else
                    {
                        ...
                        var oDistanceLL = 0f;
                        var bHasBeen1mRR = false;
                        var bHasBeen1mRL = false;
                        var bHasBeen1mLR = false;
                        var bHasBeen1mLL = false;
                        var bHasBeen1mRR_ignore = false;
                        var bHasBeen1mRL_ignore = false;
                        var bHasBeen1mLR_ignore = false;
                        var bHasBeen1mLL_ignore = false;
                        var bHasBeen1mRR_ignore_Max = false;
                        var bHasBeen1mRL_ignore_Max = false;
                        var bHasBeen1mLR_ignore_Max = false;
                        var bHasBeen1mLL_ignore_Max = false;
                        var mMin = 0.2f;
                        var mMax = 0.5f;
                        ...
                        for (var h = 0; h < MaxHCount; h++)
                        {
                            ...
                            for (var k = 0; k < MaxCount; k++)
                                ...
                                {
                                    //RR:
                                    if (!bHasBeen1mRR_ignore_Max && !bHasBeen1mRR_ignore && h < N1RCount &&
                                        k < N2RCount)
                                    {
                                        oDistanceRR = Vector2.Distance(oNode1.iConstruction.tempconstruction_R[h],
                                            oNode2.iConstruction.tempconstruction_R[k]);
                                    ...
                                    //RL:
                                    if (!bHasBeen1mRL_ignore_Max && !bHasBeen1mRL_ignore && h < N1RCount &&
                                        k < N2LCount)
                                    {
                                        oDistanceRL = Vector2.Distance(oNode1.iConstruction.tempconstruction_R[h],
                                            oNode2.iConstruction.tempconstruction_L[k]);
                                    ...
                                    //LR:
                                    if (!bHasBeen1mLR_ignore_Max && !bHasBeen1mLR_ignore && h < N1LCount &&
                                        k < N2RCount)
                                    {
                                        oDistanceLR = Vector2.Distance(oNode1.iConstruction.tempconstruction_L[h],
                                            oNode2.iConstruction.tempconstruction_R[k]);
                                    ...
                                    //LL:
                                    if (!bHasBeen1mLL_ignore_Max && !bHasBeen1mLL_ignore && h < N1LCount &&
                                        k < N2LCount)
                                    {
                                        oDistanceLL = Vector2.Distance(oNode1.iConstruction.tempconstruction_L[h],
                                            oNode2.iConstruction.tempconstruction_L[k]);
                    ...
                    if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                    {
                        ...
                        GSDRI.fCornerLR_CornerRR[2] =
                            (tVectRR - tVectLR) * ((LanesPerSide + 1) / TotalLanesI) + tVectLR;
                        GSDRI.fCornerLR_CornerRR[3] =
                        ...
                        GSDRI.fCornerRL_CornerRR[2] =
                            (tVectRR - tVectRL) * ((LanesPerSide + 1) / TotalLanesI) + tVectRL;
                        GSDRI.fCornerRL_CornerRR[3] =
                        ...
                        GSDRI.fCornerLL_CornerRL[2] =
                            (tVectRL - tVectLL) * ((LanesPerSide + 1) / TotalLanesI) + tVectLL;
                        GSDRI.fCornerLL_CornerRL[3] =
                        ...
                        GSDRI.fCornerLL_CornerLR[2] =
                            (tVectLR - tVectLL) * ((LanesPerSide + 1) / TotalLanesI) + tVectLL;
                        GSDRI.fCornerLL_CornerLR[3] =
                    ...
                    Roads.GSDIntersectionObjects.GetFourPoints(GSDRI, out GSDRI.CornerRR_Outer,
                        out GSDRI.CornerRL_Outer, out GSDRI.CornerLL_Outer, out GSDRI.CornerLR_Outer, tShoulderWidth);
                    Roads.GSDIntersectionObjects.GetFourPoints(GSDRI, out GSDRI.CornerRR_RampOuter,
                        out GSDRI.CornerRL_RampOuter, out GSDRI.CornerLL_RampOuter, out GSDRI.CornerLR_RampOuter,
                        tRampWidth);
        ...
        {
            var PrimaryCount = PrimaryList.Count;
            var SecondaryCount = SecondaryList.Count;
            ...
            for (var i = 1; i < PrimaryCount; i++)
            {
                ...
                for (var k = 1; k < SecondaryCount; k++)
                {
                    ...
                    bDidIntersect = GSDRootUtil.Intersects2D(ref t2D_Line1Start, ref t2D_Line1End, ref t2D_Line2Start,
                        ref t2D_Line2End, out tIntersectLocation);
                    if (bDidIntersect) return tIntersectLocation;
                }
            }
        ...
        private static void AddIntersectionBounds(ref GSDRoad tRoad, ref Roads.RoadConstructorBufferMaker RCS)
        {
            var bIsBridge = false;
            var bBridgeInitial = false;
            if (bBridgeInitial == false)
...
//			}
            ...
            var FinalMax = 1f;
            var StartMin = 0f;
            if (tSpline.bSpecialEndControlNode) FinalMax = tSpline.mNodes[tSpline.GetNodeCount() - 2].tTime;
            if (tSpline.bSpecialStartControlNode) StartMin = tSpline.mNodes[1].tTime;
...
//			float cDist = 0f;
            ...
            while (!bFinalEnd && spamcheck1 < spamcheckmax1)
            {
                ...
                tSpline.GetSplineValue_Both(i, out tVect, out POS);
                bIsPastInter = false;
                tIntStrength = tSpline.IntersectionStrength(ref tVect, ref tIntHeight, ref GSDRI, ref bIsPastInter,
                    ref i, ref xNode);
                if (IsApproximately(tIntStrength, 1f, 0.001f) || tIntStrength > 1f)
                    bMaxIntersection = true;
                else
                ...
                if (bMaxIntersection)
                {
                    ...
                    else
                    {
                        ...
                        tIntStrength_temp = tRoad.GSDSpline.IntersectionStrength(ref rVect, ref tIntHeight, ref GSDRI,
                            ref bIsPastInter, ref i, ref xNode);
                    ...
                    //Add bounds for later removal:
                    Roads.GSDRoadUtil.Construction2DRect vRect = null;
                    if (!bIsBridge && !bIsTunnel && bMaxIntersection && bWasPrevMaxInter)
                    {
                        ...
                        {
                            if (GSDRI.bFlipped && !bFirstInterNode)
                                vRect = new Roads.GSDRoadUtil.Construction2DRect(
                                    new Vector2(rVect.x, rVect.z),
                                    new Vector2(lVect.x, lVect.z),
                                    new Vector2(rVect_Prev.x, rVect_Prev.z),
                                    new Vector2(lVect_Prev.x, lVect_Prev.z),
...
//				i+=Step; 
        ...
        {
            var mCount = tRoad.GSDSpline.GetNodeCount();
            GSDSplineN tNode;
            for (var i = 0; i < mCount; i++)
            {
        ...
        private static bool Inter_OrganizeVerticesMatchEdges(ref List<Vector3> tList1, ref List<Vector3> tList2,
            bool bSkip1 = false, bool bSkipFirstListOne = false, bool bSkipBoth = false)
        {
            ...
            if (bSkip1)
            {
                if (bSkipBoth)
                {
                    ...
                    for (var i = 1; i < tList1.Count; i++) tList1New.Add(tList1[i]);
                    for (var i = 1; i < tList2.Count; i++) tList2New.Add(tList2[i]);
                }
                else
                {
            ...
            for (var i = 0; i < PrimaryList.Count; i++) SecondaryList.Add(PrimaryList[i]);
        ...
        private static void Inter_OrganizeVerticesMatchShoulder(ref List<Vector3> tShoulderList,
            ref List<Vector3> tToMatch, int StartI, ref Vector3 StartVec, ref Vector3 EndVect, float tHeight,
            bool bIsF = false)
        {
//			return;
            var BackupList = new List<Vector3>();
            for (var i = 0; i < tToMatch.Count; i++) BackupList.Add(tToMatch[i]);
            ...
            for (var i = StartI; i < tShoulderList.Count; i++)
            {
...
//				}
            ...
            {
                if (bIsF)
                    for (var i = RealStartID; i > 0; i -= 8)
                    {
                        t2D = ConvertVect3_To_Vect2(tShoulderList[i]);
                        tToMatch.Add(tShoulderList[i]);
                        if (t2D == t2D_End) //if(tShoulderList[i] == EndVect){
                            break;
                        spamcounter += 1;
                        if (spamcounter > 100)
                        {
                    ...
                    for (var i = RealStartID; i < tShoulderList.Count; i += 8)
                    {
                        t2D = ConvertVect3_To_Vect2(tShoulderList[i]);
                        tToMatch.Add(tShoulderList[i]);
                        if (t2D == t2D_End) //if(tShoulderList[i] == EndVect){
...
//			}
        ...
        private static void Inter_OrganizeVertices(ref GSDSplineN tNode, ref GSDRoad tRoad)
        {
            ...
            //Skipping (3 ways):
            var bSkipF = false;
            if (iCon.iFLane0L.Count == 0) bSkipF = true;
            var bSkipB = false;
            ...
            if (tNode.idOnSpline == 0 && string.CompareOrdinal(GSDRI.Node1UID, tNode.UID) == 0) bSkipB = true;
            //Is primary node and is last node on a spline, meaning t junction: It does not have a F:
            if (tNode.idOnSpline == tNode.GSDSpline.GetNodeCount() - 1 &&
                string.CompareOrdinal(GSDRI.Node1UID, tNode.UID) == 0) bSkipF = true;
            ...
            if (!bSkipB)
            {
                Inter_OrganizeVerticesMatchShoulder(ref tRoad.RCS.ShoulderL_Vectors, ref iCon.iBLane0L,
                    iCon.ShoulderBL_StartIndex, ref iCon.ShoulderBL_Start, ref iCon.ShoulderBL_End, GSDRI.Height);
                if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                    Inter_OrganizeVerticesMatchShoulder(ref tRoad.RCS.ShoulderR_Vectors, ref iCon.iBLane3R,
                        iCon.ShoulderBR_StartIndex, ref iCon.ShoulderBR_Start, ref iCon.ShoulderBR_End, GSDRI.Height);
                else if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.TurnLane)
                    Inter_OrganizeVerticesMatchShoulder(ref tRoad.RCS.ShoulderR_Vectors, ref iCon.iBLane2R,
                        iCon.ShoulderBR_StartIndex, ref iCon.ShoulderBR_Start, ref iCon.ShoulderBR_End, GSDRI.Height);
                else
                    Inter_OrganizeVerticesMatchShoulder(ref tRoad.RCS.ShoulderR_Vectors, ref iCon.iBLane1R,
                        iCon.ShoulderBR_StartIndex, ref iCon.ShoulderBR_Start, ref iCon.ShoulderBR_End, GSDRI.Height);
            }

            if (!bSkipF)
            {
                Inter_OrganizeVerticesMatchShoulder(ref tRoad.RCS.ShoulderR_Vectors, ref iCon.iFLane0L,
                    iCon.ShoulderFR_StartIndex, ref iCon.ShoulderFR_Start, ref iCon.ShoulderFR_End, GSDRI.Height, true);
                if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                    Inter_OrganizeVerticesMatchShoulder(ref tRoad.RCS.ShoulderL_Vectors, ref iCon.iFLane3R,
                        iCon.ShoulderFL_StartIndex, ref iCon.ShoulderFL_Start, ref iCon.ShoulderFL_End, GSDRI.Height,
                        true);
                else if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.TurnLane)
                    Inter_OrganizeVerticesMatchShoulder(ref tRoad.RCS.ShoulderL_Vectors, ref iCon.iFLane2R,
                        iCon.ShoulderFL_StartIndex, ref iCon.ShoulderFL_Start, ref iCon.ShoulderFL_End, GSDRI.Height,
                        true);
                else
                    Inter_OrganizeVerticesMatchShoulder(ref tRoad.RCS.ShoulderL_Vectors, ref iCon.iFLane1R,
                        iCon.ShoulderFL_StartIndex, ref iCon.ShoulderFL_Start, ref iCon.ShoulderFL_End, GSDRI.Height,
                        true);
            }

            var bError = false;
            var tWarning = "Intersection " + GSDRI.tName + " in road " + tRoad.tName +
...
//			float mDistance = 0.05f;
            ...
            mCount = tRoad.RCS.RoadVectors.Count;
            var cCount = tRoad.GSDSpline.GetNodeCount();
            var tStartI = 0;
            var tEndI = mCount;
            ...
            if (cCount > 2)
                if (!tRoad.GSDSpline.mNodes[0].bIsIntersection && !tRoad.GSDSpline.mNodes[1].bIsIntersection)
                    for (var i = 2; i < cCount; i++)
                        ...
                        {
                            if (i - 2 >= 1) tStartI = (int) (tRoad.GSDSpline.mNodes[i - 2].tTime * mCount);
                            break;
                        }

            if (cCount > 3)
                if (!tRoad.GSDSpline.mNodes[cCount - 1].bIsIntersection &&
                    !tRoad.GSDSpline.mNodes[cCount - 2].bIsIntersection)
                    for (var i = cCount - 3; i >= 0; i--)
                        if (tRoad.GSDSpline.mNodes[i].bIsIntersection)
                        {
                            if (i + 2 < cCount) tEndI = (int) (tRoad.GSDSpline.mNodes[i + 2].tTime * mCount);
                            break;
                        }
...
//			}
            ...
            if (!bSkipB) iBMainPlate = InterVertices(iCon.iBMainPlateL, iCon.iBMainPlateR, tNode.GSDRI.Height);
            if (!bSkipF) iFMainPlate = InterVertices(iCon.iFMainPlateL, iCon.iFMainPlateR, tNode.GSDRI.Height);
            //			//Marker plates:
...
//			tRoad.RCS.IntersectionTypes.Add((int)tNode.GSDRI.rType);
        ...
        private static List<Vector3> InterVertices(List<Vector3> tL, List<Vector3> tR, float tHeight)
        {
            ...
            var tCountL = tL.Count;
            var tCountR = tR.Count;
            var spamcheck = 0;
        ...
        /// </param>/
        public static void RoadJob1(ref Roads.RoadConstructorBufferMaker RCS)
        {
...
//			if(RCS.tRoad.bProfiling){ UnityEngine.Profiling.Profiler.BeginSample("ProcessRoad_IntersectionCleanup"); }
        ...
        /// The road construction buffer, by reference.
        /// </param>
        public static void RoadJob2(ref Roads.RoadConstructorBufferMaker RCS)
        {
            ...
            if (RCS.bRoadOn)
            {
                ...
                if (!RCS.tMeshSkip)
                {
                    var vCount = RCS.RoadVectors.Count;
            ...
            if (RCS.tRoad.opt_bRoadCuts || RCS.tRoad.opt_bDynamicCuts)
            {
                ...
                var cCount = RCS.cut_RoadVectors.Count;
                for (var i = 0; i < cCount; i++)
                {
                    RCS.cut_tangents.Add(GSDRootUtil.ProcessTangents(RCS.cut_tris[i], RCS.cut_normals[i], RCS.cut_uv[i],
                        RCS.cut_RoadVectors[i].ToArray()));
                    RCS.cut_tangents_world.Add(GSDRootUtil.ProcessTangents(RCS.cut_tris[i], RCS.cut_normals[i],
                        RCS.cut_uv_world[i], RCS.cut_RoadVectors[i].ToArray()));
                }
            }

            if (RCS.tRoad.opt_bShoulderCuts || RCS.tRoad.opt_bDynamicCuts)
            {
                var rCount = RCS.cut_ShoulderR_Vectors.Count;
                for (var i = 0; i < rCount; i++)
                {
                    ProcessRoad_UVs_ShoulderCut(ref RCS, false, i);
                    RCS.cut_tangents_SR.Add(GSDRootUtil.ProcessTangents(RCS.cut_tris_ShoulderR[i],
                        RCS.cut_normals_ShoulderR[i], RCS.cut_uv_SR[i], RCS.cut_ShoulderR_Vectors[i].ToArray()));
                    RCS.cut_tangents_SR_world.Add(GSDRootUtil.ProcessTangents(RCS.cut_tris_ShoulderR[i],
                        RCS.cut_normals_ShoulderR[i], RCS.cut_uv_SR_world[i], RCS.cut_ShoulderR_Vectors[i].ToArray()));
                }

                var lCount = RCS.cut_ShoulderL_Vectors.Count;
                for (var i = 0; i < lCount; i++)
                {
                    ProcessRoad_UVs_ShoulderCut(ref RCS, true, i);
                    RCS.cut_tangents_SL.Add(GSDRootUtil.ProcessTangents(RCS.cut_tris_ShoulderL[i],
                        RCS.cut_normals_ShoulderL[i], RCS.cut_uv_SL[i], RCS.cut_ShoulderL_Vectors[i].ToArray()));
                    RCS.cut_tangents_SL_world.Add(GSDRootUtil.ProcessTangents(RCS.cut_tris_ShoulderL[i],
                        RCS.cut_normals_ShoulderL[i], RCS.cut_uv_SL_world[i], RCS.cut_ShoulderL_Vectors[i].ToArray()));
                }
            }

            if (RCS.bInterseOn) ProcessRoad_UVs_Intersections(ref RCS);
            ...
            if (RCS.bRoadOn)
            {
                if (!RCS.tMeshSkip)
                    RCS.tangents =
                        GSDRootUtil.ProcessTangents(RCS.tris, RCS.normals, RCS.uv, RCS.RoadVectors.ToArray());
                if (!RCS.tMeshSkip)
                    RCS.tangents2 =
                        GSDRootUtil.ProcessTangents(RCS.tris, RCS.normals, RCS.uv2, RCS.RoadVectors.ToArray());
                if (!RCS.tMesh_SRSkip)
                    RCS.tangents_SR = GSDRootUtil.ProcessTangents(RCS.tris_ShoulderR, RCS.normals_ShoulderR, RCS.uv_SR,
                        RCS.ShoulderR_Vectors.ToArray());
                if (!RCS.tMesh_SLSkip)
                    RCS.tangents_SL = GSDRootUtil.ProcessTangents(RCS.tris_ShoulderL, RCS.normals_ShoulderL, RCS.uv_SL,
                        RCS.ShoulderL_Vectors.ToArray());
                for (var i = 0; i < RCS.tMesh_RoadConnections.Count; i++)
                    RCS.RoadConnections_tangents.Add(GSDRootUtil.ProcessTangents(RCS.RoadConnections_tris[i],
                        RCS.RoadConnections_normals[i], RCS.RoadConnections_uv[i], RCS.RoadConnections_verts[i]));
            }

            if (RCS.bInterseOn)
            {
                //Back lanes:
                var vCount = RCS.iBLane0s.Count;
                for (var i = 0; i < vCount; i++)
                    RCS.iBLane0s_tangents.Add(GSDRootUtil.ProcessTangents(RCS.iBLane0s_tris[i], RCS.iBLane0s_normals[i],
                        RCS.iBLane0s_uv[i], RCS.iBLane0s[i]));
                vCount = RCS.iBLane1s.Count;
                for (var i = 0; i < vCount; i++)
                    RCS.iBLane1s_tangents.Add(GSDRootUtil.ProcessTangents(RCS.iBLane1s_tris[i], RCS.iBLane1s_normals[i],
                        RCS.iBLane1s_uv[i], RCS.iBLane1s[i]));
                vCount = RCS.iBLane2s.Count;
                for (var i = 0; i < vCount; i++)
                    RCS.iBLane2s_tangents.Add(GSDRootUtil.ProcessTangents(RCS.iBLane2s_tris[i], RCS.iBLane2s_normals[i],
                        RCS.iBLane2s_uv[i], RCS.iBLane2s[i]));
                vCount = RCS.iBLane3s.Count;
                for (var i = 0; i < vCount; i++)
                    RCS.iBLane3s_tangents.Add(GSDRootUtil.ProcessTangents(RCS.iBLane3s_tris[i], RCS.iBLane3s_normals[i],
                        RCS.iBLane3s_uv[i], RCS.iBLane3s[i]));
                //Front lanes:
                vCount = RCS.iFLane0s.Count;
                for (var i = 0; i < vCount; i++)
                    RCS.iFLane0s_tangents.Add(GSDRootUtil.ProcessTangents(RCS.iFLane0s_tris[i], RCS.iFLane0s_normals[i],
                        RCS.iFLane0s_uv[i], RCS.iFLane0s[i]));
                vCount = RCS.iFLane1s.Count;
                for (var i = 0; i < vCount; i++)
                    RCS.iFLane1s_tangents.Add(GSDRootUtil.ProcessTangents(RCS.iFLane1s_tris[i], RCS.iFLane1s_normals[i],
                        RCS.iFLane1s_uv[i], RCS.iFLane1s[i]));
                vCount = RCS.iFLane2s.Count;
                for (var i = 0; i < vCount; i++)
                    RCS.iFLane2s_tangents.Add(GSDRootUtil.ProcessTangents(RCS.iFLane2s_tris[i], RCS.iFLane2s_normals[i],
                        RCS.iFLane2s_uv[i], RCS.iFLane2s[i]));
                vCount = RCS.iFLane3s.Count;
                for (var i = 0; i < vCount; i++)
                    RCS.iFLane3s_tangents.Add(GSDRootUtil.ProcessTangents(RCS.iFLane3s_tris[i], RCS.iFLane3s_normals[i],
                        RCS.iFLane3s_uv[i], RCS.iFLane3s[i]));
                //Main plates:
                vCount = RCS.iBMainPlates.Count;
                for (var i = 0; i < vCount; i++)
                    RCS.iBMainPlates_tangents.Add(GSDRootUtil.ProcessTangents(RCS.iBMainPlates_tris[i],
                        RCS.iBMainPlates_normals[i], RCS.iBMainPlates_uv[i], RCS.iBMainPlates[i]));
                vCount = RCS.iBMainPlates.Count;
                for (var i = 0; i < vCount; i++)
                    RCS.iBMainPlates_tangents2.Add(GSDRootUtil.ProcessTangents(RCS.iBMainPlates_tris[i],
                        RCS.iBMainPlates_normals[i], RCS.iBMainPlates_uv2[i], RCS.iBMainPlates[i]));
                vCount = RCS.iFMainPlates.Count;
                for (var i = 0; i < vCount; i++)
                    RCS.iFMainPlates_tangents.Add(GSDRootUtil.ProcessTangents(RCS.iFMainPlates_tris[i],
                        RCS.iFMainPlates_normals[i], RCS.iFMainPlates_uv[i], RCS.iFMainPlates[i]));
                vCount = RCS.iFMainPlates.Count;
                for (var i = 0; i < vCount; i++)
                    RCS.iFMainPlates_tangents2.Add(GSDRootUtil.ProcessTangents(RCS.iFMainPlates_tris[i],
                        RCS.iFMainPlates_normals[i], RCS.iFMainPlates_uv2[i], RCS.iFMainPlates[i]));
            }
        }

        #region "Intersection Cleanup"

        private static void ProcessRoad_IntersectionCleanup(ref Roads.RoadConstructorBufferMaker RCS)
        {
            var tList = RCS.tIntersectionBounds;
            var mCount = tList.Count;
            RCS.ShoulderR_Vectors =
                ProcessRoad_IntersectionCleanup_Helper(ref RCS.ShoulderR_Vectors, ref tList, mCount,
                    ref RCS.ImmuneVects);
            RCS.ShoulderL_Vectors =
                ProcessRoad_IntersectionCleanup_Helper(ref RCS.ShoulderL_Vectors, ref tList, mCount,
                    ref RCS.ImmuneVects);
        }

        private static List<Vector3> ProcessRoad_IntersectionCleanup_Helper(ref List<Vector3> tVects,
            ref List<Roads.GSDRoadUtil.Construction2DRect> tList, int mCount, ref HashSet<Vector3> ImmuneVects)
        {
            Roads.GSDRoadUtil.Construction2DRect tRect = null;
            var MVL = tVects.Count;
...
//			Debug.Log ("Fcount: " + fCount);
        ...
        private static void ProcessRoad_Tris_Bulk(ref Roads.RoadConstructorBufferMaker RCS)
        {
            ...
            if (RCS.tRoad.opt_bRoadCuts || RCS.tRoad.opt_bDynamicCuts) ProcessRoad_Tris_RoadCuts(ref RCS);
...
//			if(xCount < 0){ xCount = 0; }
        ...
        private static void ProcessRoad_Tris_RoadCuts(ref Roads.RoadConstructorBufferMaker RCS)
        {
            //Road cuts aren't working right for the special nodes on cuts
            var cCount = RCS.RoadCuts.Count;
            ...
            var mMax = RCS.RoadVectors.Count;
            tVectListSingle = new List<Vector3>();
            RCS.cut_RoadVectorsHome.Add(RCS.RoadVectors[PrevRoadCutIndex]);
            xVect = RCS.RoadVectors[PrevRoadCutIndex];
            for (var i = PrevRoadCutIndex; i < mMax; i++) tVectListSingle.Add(RCS.RoadVectors[i] - xVect);
            tVects.Add(tVectListSingle);
        ...
        private static void ProcessRoad_Tris_ShoulderCutsR(ref Roads.RoadConstructorBufferMaker RCS)
        {
            var cCount = RCS.ShoulderCutsR.Count;
            var PrevRoadCutIndex = 0;
            var CurrentRoadCutIndex = 0;
            var tVects = new List<List<Vector3>>();
            List<Vector3> tVectListSingle = null;
            ...
            var mMax = RCS.ShoulderR_Vectors.Count;
            tVectListSingle = new List<Vector3>();
            RCS.cut_ShoulderR_VectorsHome.Add(RCS.ShoulderR_Vectors[PrevRoadCutIndex]);
            xVect = RCS.ShoulderR_Vectors[PrevRoadCutIndex];
            for (var i = PrevRoadCutIndex; i < mMax; i++) tVectListSingle.Add(RCS.ShoulderR_Vectors[i] - xVect);
            tVects.Add(tVectListSingle);
        ...
        private static void ProcessRoad_Tris_ShoulderCutsL(ref Roads.RoadConstructorBufferMaker RCS)
        {
            var cCount = RCS.ShoulderCutsL.Count;
            var PrevRoadCutIndex = 0;
            var CurrentRoadCutIndex = 0;
            var tVects = new List<List<Vector3>>();
            List<Vector3> tVectListSingle = null;
            ...
            var mMax = RCS.ShoulderL_Vectors.Count;
            tVectListSingle = new List<Vector3>();
            RCS.cut_ShoulderL_VectorsHome.Add(RCS.ShoulderL_Vectors[PrevRoadCutIndex]);
            xVect = RCS.ShoulderL_Vectors[PrevRoadCutIndex];
            for (var i = PrevRoadCutIndex; i < mMax; i++) tVectListSingle.Add(RCS.ShoulderL_Vectors[i] - xVect);
            tVects.Add(tVectListSingle);
        ...
        private static int[] ProcessRoad_Tris_Shoulder(int MVL)
        {
            var TriCount = 0;
            int x1, x2, x3;
            var xCount = (int) (MVL / 2 * 0.25f * 6) - 6;
            if (xCount < 0) xCount = 0;
            xCount = xCount * 2;

            var tri = new int[xCount];
            for (var i = 0; i < MVL; i += 8)
            {
                if (i + 8 == MVL) break;
        ...
        private static void ProcessRoad_Tris_iProcessor(ref List<int[]> TriList, ref List<Vector3[]> VertexList)
        {
            ...
            var vListCount = VertexList.Count;
            int[] tris;
        ...
        private static void ProcessRoad_Normals_Bulk(ref Roads.RoadConstructorBufferMaker RCS)
        {
...
//			}
            ...
            //Road cuts normals:
            if (RCS.tRoad.opt_bRoadCuts || RCS.tRoad.opt_bDynamicCuts) ProcessRoad_Normals_RoadCuts(ref RCS);
            if (RCS.tRoad.opt_bShoulderCuts || RCS.tRoad.opt_bDynamicCuts)
            {
        ...
        private static void ProcessRoad_Normals_RoadCuts(ref Roads.RoadConstructorBufferMaker RCS)
        {
            var cCount = RCS.cut_RoadVectors.Count;
            for (var j = 0; j < cCount; j++)
            {
                var MVL = RCS.cut_RoadVectors[j].Count;
                var normals = new Vector3[MVL];
//				Vector3 tVect = -Vector3.forward;
//				for(int i=0;i<MVL;i++){
//					normals[i] = tVect;
//				}
        ...
        private static void ProcessRoad_Normals_ShoulderCutsR(ref Roads.RoadConstructorBufferMaker RCS)
        {
            var cCount = RCS.cut_ShoulderR_Vectors.Count;
            for (var j = 0; j < cCount; j++)
            {
                var MVL = RCS.cut_ShoulderR_Vectors[j].Count;
                var normals = new Vector3[MVL];
//				Vector3 tVect = -Vector3.forward;
//				for(int i=0;i<MVL;i++){
//					normals[i] = tVect;
//				}
        ...
        private static void ProcessRoad_Normals_ShoulderCutsL(ref Roads.RoadConstructorBufferMaker RCS)
        {
            var cCount = RCS.cut_ShoulderL_Vectors.Count;
            for (var j = 0; j < cCount; j++)
            {
                var MVL = RCS.cut_ShoulderL_Vectors[j].Count;
                var normals = new Vector3[MVL];
//				Vector3 tVect = -Vector3.forward;
//				for(int i=0;i<MVL;i++){
//					normals[i] = tVect;
//				}
        ...
        private static void ProcessRoad_Normals_Shoulders(ref Roads.RoadConstructorBufferMaker RCS)
        {
            //A mesh with just the vertices and triangles set up will be visible in the editor but will not look very convincing since it is not correctly shaded without the normals. 
            //The normals for the flat plane are very simple - they are all identical and point in the negative Z direction in the plane's local space. 
            //With the normals added, the plane will be correctly shaded but remember that you need a light in the scene to see the effect. 
            var MVL = RCS.ShoulderL_Vectors.Count;
            var normals = new Vector3[MVL];
//			Vector3 tVect = -Vector3.forward;
//			for(int i=0;i<MVL;i++){
//				normals[i] = tVect;
...
//			}
        ...
        //For intersection parts:
        private static void ProcessRoad_Normals_iProcessor(ref List<Vector3[]> NormalList,
            ref List<Vector3[]> VertexList)
        {
...
//				}
        ...
        private static void ProcessRoad_UVs_RoadCuts(ref Roads.RoadConstructorBufferMaker RCS)
        {
            //Finally, adding texture coordinates to the mesh will enable it to display a material correctly. 
            //Assuming we want to show the whole image across the plane, the UV values will all be 0 or 1, corresponding to the corners of the texture. 
...
//			float fDistance = Vector3.Distance(tVerts[0],tVerts[2]);
            ...
            for (i = 0; i < MVL; i++) uv[i] = new Vector2(tVerts[i].x * 0.2f, tVerts[i].z * 0.2f);
            return uv;
...
//			return uv;
        ...
        private static void ProcessRoad_UVs_ShoulderCut(ref Roads.RoadConstructorBufferMaker RCS, bool bIsLeft, int j)
        {
            var i = 0;
            Vector3[] tVerts;
            if (bIsLeft)
            ...
            var xDistance = 0f;
            i = 0;
            var TheOne = RCS.tRoad.opt_ShoulderWidth / RCS.tRoad.opt_RoadDefinition;
            while (i + 8 < MVL)
            {
        ...
        private static void ProcessRoad_UVs_Intersections(ref Roads.RoadConstructorBufferMaker RCS)
        {
            var tCount = -1;
            ...
            tCount = RCS.iBMainPlates.Count;
            for (var i = 0; i < tCount; i++)
                RCS.iBMainPlates_uv2.Add(
                    ProcessRoad_UVs_Intersection_MainPlate2(ref RCS, RCS.iBMainPlates[i], RCS.iBMainPlates_tID[i]));
            tCount = RCS.iFMainPlates.Count;
            for (var i = 0; i < tCount; i++)
                RCS.iFMainPlates_uv2.Add(
                    ProcessRoad_UVs_Intersection_MainPlate2(ref RCS, RCS.iFMainPlates[i], RCS.iFMainPlates_tID[i]));
        ...
        private static Vector2[] ProcessRoad_UVs_Intersection_FullLane(ref Roads.RoadConstructorBufferMaker RCS,
            Vector3[] tVerts)
        {
            //Finally, adding texture coordinates to the mesh will enable it to display a material correctly. 
            //Assuming we want to show the whole image across the plane, the UV values will all be 0 or 1, corresponding to the corners of the texture. 
        ...
        private static Vector2[] ProcessRoad_UVs_Intersection_Lane4(ref Roads.RoadConstructorBufferMaker RCS,
            Vector3[] tVerts)
        {
            //Finally, adding texture coordinates to the mesh will enable it to display a material correctly. 
            //Assuming we want to show the whole image across the plane, the UV values will all be 0 or 1, corresponding to the corners of the texture. 
        ...
        private static Vector2[] ProcessRoad_UVs_Intersection_MiddleLane(ref Roads.RoadConstructorBufferMaker RCS,
            Vector3[] tVerts)
        {
            //Finally, adding texture coordinates to the mesh will enable it to display a material correctly. 
            //Assuming we want to show the whole image across the plane, the UV values will all be 0 or 1, corresponding to the corners of the texture. 
        ...
        private static Vector2[] ProcessRoad_UVs_Intersection_Lane0(ref Roads.RoadConstructorBufferMaker RCS,
            Vector3[] tVerts)
        {
            //Finally, adding texture coordinates to the mesh will enable it to display a material correctly. 
            //Assuming we want to show the whole image across the plane, the UV values will all be 0 or 1, corresponding to the corners of the texture. 
...
//				}
        ...
        private static Vector2[] ProcessRoad_UVs_Intersection_MarkerPlate(ref Roads.RoadConstructorBufferMaker RCS,
            Vector3[] tVerts)
        {
            //Finally, adding texture coordinates to the mesh will enable it to display a material correctly. 
            //Assuming we want to show the whole image across the plane, the UV values will all be 0 or 1, corresponding to the corners of the texture. 
            ...
            var mDistanceL = Vector3.Distance(tVerts[i], tVerts[tVerts.Length - 3]);
            var mDistanceR = Vector3.Distance(tVerts[i + 2], tVerts[tVerts.Length - 1]);
        ...
        private static Vector2[] ProcessRoad_UVs_Intersection_MainPlate(ref Roads.RoadConstructorBufferMaker RCS,
            Vector3[] tVerts)
        {
            //Finally, adding texture coordinates to the mesh will enable it to display a material correctly. 
            //Assuming we want to show the whole image across the plane, the UV values will all be 0 or 1, corresponding to the corners of the texture. 
...
//			return uv;
        ...
        private static Vector2[] ProcessRoad_UVs_Intersection_MainPlate2(ref Roads.RoadConstructorBufferMaker RCS,
            Vector3[] tVerts, GSDRoadIntersection GSDRI)
        {
            //Finally, adding texture coordinates to the mesh will enable it to display a material correctly. 
            //Assuming we want to show the whole image across the plane, the UV values will all be 0 or 1, corresponding to the corners of the texture. 
            //int MVL = tMesh.vertices.Length;
...
//			float DistRepresent = 5f;
            ...
            var mDistanceL = Vector3.Distance(tVerts[i + 4], tVerts[tVerts.Length - 3]);
            var mDistanceR = Vector3.Distance(tVerts[i + 6], tVerts[tVerts.Length - 1]);
...
//			int bHitMaxR = 0;
            ...
            var RoadWidth = RCS.tRoad.RoadWidth();
            var LaneWidth = RCS.tRoad.opt_LaneWidth;
            float iWidth = -1;
            if (GSDRI.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
...
//			uv[MVL-4].y = 1f;
        ...
        private static void SetVectorHeight2(ref Vector3 tWorldVect, ref float p,
            ref List<KeyValuePair<float, float>> tList, ref GSDSplineC tSpline)
        {
            var mCount = tList.Count;
            var i = 0;
            ...
            for (i = 0; i < mCount - 1; i++)
                if (p >= tList[i].Key && p < tList[i + 1].Key)
                {
                    ...
                    if (i > 3)
                    {
                        if (tList[i - 1].Value < cValue) cValue = tList[i - 1].Value;
                        if (tList[i - 2].Value < cValue) cValue = tList[i - 2].Value;
                        if (tList[i - 3].Value < cValue) cValue = tList[i - 3].Value;
                    }
            ...
            //Zero protection: 
            if (GSDRootUtil.IsApproximately(cValue, 0f, 0.001f) && tWorldVect.y > 0f) cValue = tWorldVect.y - 0.35f;
    ...
    public class TerrainCalcs : GSDThreadedJob
    {
        ...
        private List<Roads.GSDTerraforming.TempTerrainData> TTDList;
        private GSDSplineC tSpline;
        private GSDRoad tRoad;

        public void Setup(ref List<Roads.GSDTerraforming.TempTerrainData> _TTDList, GSDSplineC _tSpline, GSDRoad _tRoad)
        {
            TTDList = _TTDList;
            tSpline = _tSpline;
            tRoad = _tRoad;
        ...
        protected override void ThreadFunction()
        {
            var Step = tRoad.opt_RoadDefinition * 0.4f / tSpline.distance;
            if (Step > 2f) Step = 2f;
            if (Step < 1f) Step = 1f;
            //			float tDistance = tRoad.RoadWidth()*2f;

//			Vector3 tVect,POS;
            foreach (var TTD in TTDList) //				float PrevHeight = 0f;
//				float FinalMax = 1f;
//				float StartMin = 0f;
//				if(tSpline.bSpecialEndControlNode){
//					FinalMax = tSpline.mNodes[tSpline.GetNodeCount()-2].tTime;
...
//				}
    ...
    {
        public static void RunMe(ref List<Roads.GSDTerraforming.TempTerrainData> TTDList, GSDSplineC tSpline,
            GSDRoad tRoad)
        {
            var Step = tRoad.opt_RoadDefinition * 0.4f / tSpline.distance;
            if (Step > 2f) Step = 2f;
...
//				if(tRoad.opt_MatchTerrain){
                if (tRoad.bProfiling) UnityEngine.Profiling.Profiler.BeginSample("DoRects");
                GSDTerraformingT.DoRects(tSpline, TTD);
...
//				}
    ...
    {
        private object GSDm_Handle = new object();
        private Roads.RoadConstructorBufferMaker RCS;
        private GSDRoad tRoad;

        public void Setup(ref Roads.RoadConstructorBufferMaker _RCS, ref GSDRoad _tRoad)
        {
            RCS = _RCS;
            tRoad = _tRoad;
        }
        ...
        public Roads.RoadConstructorBufferMaker GetRCS()
        {
            Roads.RoadConstructorBufferMaker tRCS;
            lock (GSDm_Handle)
            {
                tRCS = RCS;
            }
    ...
    {
        public static void RunMe(ref Roads.RoadConstructorBufferMaker RCS)
        {
            GSDRoadCreationT.RoadJob1(ref RCS);
        }
    }
    ...
    public class RoadCalcs2 : GSDThreadedJob
    {
        ...
        public void Setup(ref Roads.RoadConstructorBufferMaker _RCS)
        {
            RCS = _RCS;
        }
