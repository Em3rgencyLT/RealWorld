...
//			bNeedsUpdate = true;
        ...
        public class SplinatedMeshLibraryMaker
        {
            ...
            public void LoadToSMM(SplinatedMeshMaker SMM)
            {
                ...
                if (CurrentSplinationCap1String != null && CurrentSplinationCap1String.Length > 1)
                    SMM.CurrentSplinationCap1 =
                        (GameObject) UnityEditor.AssetDatabase.LoadAssetAtPath(CurrentSplinationCap1String,
                            typeof(GameObject));

                SMM.CurrentSplinationCap2String = CurrentSplinationCap2String;
                if (CurrentSplinationCap2String != null && CurrentSplinationCap2String.Length > 1)
                    SMM.CurrentSplinationCap2 =
                        (GameObject) UnityEditor.AssetDatabase.LoadAssetAtPath(CurrentSplinationCap2String,
                            typeof(GameObject));
                ...
                if (bMaterialOverride)
                {
                    if (SplinatedMaterial1String != null && SplinatedMaterial1String.Length > 0)
                        SMM.SplinatedMaterial1 =
                            (Material) UnityEditor.AssetDatabase.LoadAssetAtPath(SplinatedMaterial1String,
                                typeof(Material));
                    if (SplinatedMaterial2String != null && SplinatedMaterial2String.Length > 0)
                        SMM.SplinatedMaterial2 =
                            (Material) UnityEditor.AssetDatabase.LoadAssetAtPath(SplinatedMaterial2String,
                                typeof(Material));
                }
                ...
                if (EndCapStartString != null && EndCapStartString.Length > 0)
                    SMM.EndCapStart =
                        (GameObject) UnityEditor.AssetDatabase.LoadAssetAtPath(EndCapStartString, typeof(GameObject));
                if (EndCapEndString != null && EndCapEndString.Length > 0)
                    SMM.EndCapEnd =
                        (GameObject) UnityEditor.AssetDatabase.LoadAssetAtPath(EndCapEndString, typeof(GameObject));
                SMM.bEndCapCustomMatchStart = bEndCapCustomMatchStart;
                SMM.EndCapCustomOffsetStart = EndCapCustomOffsetStart;
                SMM.EndCapCustomOffsetEnd = EndCapCustomOffsetEnd;
                SMM.EndCapCustomRotOffsetStart = EndCapCustomRotOffsetStart;
...
#if UNITY_EDITOR
                SMM.CurrentSplination =
                    CurrentSplination; // (GameObject)UnityEditor.AssetDatabase.LoadAssetAtPath(CurrentSplinationString,typeof(GameObject));
                SMM.CurrentSplinationCap1 =
                    CurrentSplinationCap1; // (GameObject)UnityEditor.AssetDatabase.LoadAssetAtPath(CurrentSplinationCap1String,typeof(GameObject));
                SMM.CurrentSplinationCap2 =
                    CurrentSplinationCap2; // (GameObject)UnityEditor.AssetDatabase.LoadAssetAtPath(CurrentSplinationCap2String,typeof(GameObject));
        ...
        {
            var tCount = tVects.Length;
            var mVect = default(Vector3);
            for (var i = 0; i < tCount; i++) mVect += tVects[i];
            mVect /= tCount;
            return mVect;
        }
        ...
        private static bool FloatsNear(float tNear, float tVal1, float tVal2)
        {
            ...
            if (tVal1 < tVal2 + tNear && tVal1 > tVal2 - tNear) return true;
            if (tVal2 < tVal1 + tNear && tVal2 > tVal1 - tNear) return true;
            return false;
        }
        ...
        {
            var cCount = 0;
            if (GSDRootUtil.IsApproximately(V1.x, V2.x, Precision)) cCount += 1;
            if (GSDRootUtil.IsApproximately(V1.y, V2.y, Precision)) cCount += 1;
            if (GSDRootUtil.IsApproximately(V1.z, V2.z, Precision)) cCount += 1;
        ...
        {
            if (!GSDRootUtil.IsApproximately(V1.x, 0f, 0.0001f)) return false;
            if (!GSDRootUtil.IsApproximately(V1.y, 0f, 0.0001f)) return false;
            if (!GSDRootUtil.IsApproximately(V1.z, 0f, 0.0001f)) return false;
            return true;
        }

        private static bool V3EqualNormal(Vector3 V1, Vector3 V2)
        {
            if (!GSDRootUtil.IsApproximately(V1.x, V2.x, 0.01f)) return false;
            if (!GSDRootUtil.IsApproximately(V1.y, V2.y, 0.01f)) return false;
            if (!GSDRootUtil.IsApproximately(V1.z, V2.z, 0.01f)) return false;
            return true;
        }

        private static bool IsApproxExtruded(ref Vector3 V1, ref Vector3 V2, bool bIsZAxis)
        {
            if (!GSDRootUtil.IsApproximately(V1.y, V2.y, 0.02f)) return false;

            if (bIsZAxis)
            {
                if (!GSDRootUtil.IsApproximately(V1.x, V2.x, 0.02f)) return false;
            }
            else
            {
                if (!GSDRootUtil.IsApproximately(V1.z, V2.z, 0.02f)) return false;
            }
...
//			}
            ...
            var tObj = (GameObject) Object.Instantiate(CurrentSplination);
            ErrortObj[0] = tObj;
            ...
            if (EndCapStart != null)
            {
                EndCapStartObj = (GameObject) Object.Instantiate(EndCapStart);
                ErrortObj[1] = EndCapStartObj;
            }

            if (EndCapEnd != null)
            {
                EndCapEndObj = (GameObject) Object.Instantiate(EndCapEnd);
            ...
            if (bFlipRotation)
            {
                if (CurrentSplinationCap2 != null)
                {
                    Cap1 = (GameObject) Object.Instantiate(CurrentSplinationCap2);
                    ErrortObj[3] = Cap1;
                }

                if (CurrentSplinationCap1 != null)
                {
                    Cap2 = (GameObject) Object.Instantiate(CurrentSplinationCap1);
                    ErrortObj[4] = Cap2;
                }
            }
            else
            {
                if (CurrentSplinationCap1 != null)
                {
                    Cap1 = (GameObject) Object.Instantiate(CurrentSplinationCap1);
                    ErrortObj[3] = Cap1;
                }

                if (CurrentSplinationCap2 != null)
                {
                    Cap2 = (GameObject) Object.Instantiate(CurrentSplinationCap2);
                    ErrortObj[4] = Cap2;
                }
            }
...
//				float oMinZ = Mathf.Min(oMinMaxZ);
                ...
                for (var i = 0; i < CapOrigMVL1; i++)
                    ...
                    {
                        if (GSDRootUtil.IsApproximately(CapOrigVerts1[i].z, oMaxZ, MinMaxMod)) tCapMatchIndices1.Add(i);
                    }
                    else
                    {
                        if (GSDRootUtil.IsApproximately(CapOrigVerts1[i].x, oMaxX, MinMaxMod)) tCapMatchIndices1.Add(i);
                    }
...
//				float oMaxZ = Mathf.Max(oMinMaxZ);
                ...
                for (var i = 0; i < CapOrigMVL2; i++)
                    if (Axis == AxisTypeEnum.Z)
                    {
                        if (GSDRootUtil.IsApproximately(CapOrigVerts2[i].z, oMinZ, MinMaxMod)) tCapMatchIndices2.Add(i);
                    }
                    else
                    {
                        if (GSDRootUtil.IsApproximately(CapOrigVerts2[i].x, oMinX, MinMaxMod)) tCapMatchIndices2.Add(i);
                    }
...
//			}
            ...
            if (CapMesh1 != null)
            {
                ...
                foreach (var KVP in MatchingIndices_Min)
                {
                    ...
                    for (var i = 0; i < CapOrigMVL1; i++)
                        if (tCapMatchIndices1.Contains(i) &&
                            IsApproxTwoThirds(ref tVect1, CapOrigVerts1[i], VertexMatchingPrecision))
                        {
                            tList.Add(i);
                            bDidAdd = true;
                        }
            ...
            if (CapMesh2 != null)
            {
                ...
                foreach (var KVP in MatchingIndices)
                {
                    ...
                    tVect1 = OrigVerts[KVP.Key];
                    for (var i = 0; i < CapOrigMVL2; i++)
                        if (tCapMatchIndices2.Contains(i) &&
                            IsApproxTwoThirds(ref tVect1, CapOrigVerts2[i], VertexMatchingPrecision))
                        {
                            tList.Add(i);
                            bDidAdd = true;
                        }
            ...
            {
                var RoadDefStart = tSpline.tRoad.opt_RoadDefinition / 2f * -1;
                var UVChange = tSpline.tRoad.opt_RoadDefinition / mMaxDiff;
                foreach (var KVP in MatchingIndices)
                {
                    //Vertex change:
                    if (Axis == AxisTypeEnum.X)
...
//			Vector3 tEndPos = tSpline.GetSplineValue(EndTime);
            ...
            if (bTrimStart)
                tTimes.RemoveAt(0);
            else if (bTrimEnd) tTimes.RemoveAt(tTimes.Count - 1);
...
//			float cDiff = 0f;
            ...
            var jDistance = 0f;
            var jStartDistance = tSpline.TranslateParamToDist(tStartTime);
            var jEndDistance = tSpline.TranslateParamToDist(tEndTime);
                    ...
                    if (bVerticalCutoff)
                        if (MiddleVectorIndicies.Contains(i))
                        {
                            ...
                            if (bVerticalCutoffDownwards)
                            {
                                ...
                                {
                                    if (tFloat5 < tOrigHeightBuffer) tVerts[vManuver + i].y = tOrigHeightBuffer;
                                }
...
//					}
                ...
                //Triangles:
                for (var i = 0; i < OrigTriCount; i++) tTris[i + TriManuver] = OrigTris[i] + vManuver;
                    ...
                    if (bVerticalMeshCutoff_OppositeDir)
                    {
                        ...
                        for (var i = 0; i < MiddleCount; i++)
                        {
                            ...
                            if (!bVerticalCutoffDownwards)
                            {
                                ...
                                if (VertCutBuffer3.y < tBuffer) tVerts[vManuver + MiddleVectorIndicies[i]].y = tBuffer;
                            }
                            else
                            {
                                tBuffer = GetVHeightAtXY(ref VertCutBuffer1, ref VertCutBuffer2, ref VertCutBuffer3) -
                ...
                if (bStartDown)
                {
                    ...
                    if (bStartTypeDownOverride) tFloat1 = StartTypeDownOverride;
                    if (j == 0)
                    {
                        for (var i = 0; i < MinCount; i++)
                        {
                ...
                {
                    tFloat1 = mMaxHeight * 1.05f;
                    if (bEndTypeDownOverride) tFloat1 = EndTypeDownOverride;
                    if (j == MeshCount - 1)
                    {
                        for (var i = 0; i < MaxCount; i++)
                        {
...
//									tHitIndex=0;
                                ...
                                {
                                    if (tRayHit[g].point.y > tHitY) tHitY = tRayHit[g].point.y;
//										tHitIndex = g;
                                }
...
//										tHitIndex = g;
            ...
            if (CapMesh1 != null)
            {
                ...
                foreach (var KVP in MatchingIndices_Min_Cap)
                {
                    ...
                    {
                        if (bcapstart) tVect1 = cap1_verts[KVP.Value[i]] - tVerts[KVP.Key];
                        cap1_verts[KVP.Value[i]] = tVerts[KVP.Key];
                        cap1_hit[KVP.Value[i]] = true;
                        if (bcapstart)
                        {
                            tHeight = tSpline.GetSplineValue(tSpline.GetClosestParam(cap1_verts[KVP.Value[i]]), false)
                                .y;
                            bcapstart = false;
                        }
                    }
                ...
                for (var i = CapTriCount1; i < CapTriCount1 + OldTriCount; i++) nTris[i] += CapOrigMVL1;
            ...
            if (CapMesh2 != null)
            {
                ...
                foreach (var KVP in MatchingIndices_Max_Cap)
                {
                    ...
                    for (var i = 0; i < wCount; i++)
                    {
                        if (bcapstart) tVect1 = cap2_verts[KVP.Value[i]] - tVerts[vManuver + KVP.Key + CapOrigMVL1];
                        cap2_verts[KVP.Value[i]] = tVerts[vManuver + KVP.Key + CapOrigMVL1];
                        cap2_hit[KVP.Value[i]] = true;

                        if (bcapstart)
                        {
                            tHeight = tSpline.GetSplineValue(tSpline.GetClosestParam(cap2_verts[KVP.Value[i]]), false)
                                .y;
                            bcapstart = false;
                        }
                    }
                ...
                for (var i = OldTriCount; i < nTris.Length; i++) nTris[i] += OldMVL;
            ...
            for (var i = 0; i < tVertCount; i++) tVerts[i] -= tNode.pos;
            if (cVerts != null)
            {
                var cVertCount = cVerts.Length;
                for (var i = 0; i < cVertCount; i++) cVerts[i] -= tNode.pos;
            }
            ...
            if (tName == null || tName.Length < 1) tName = "ExtrudedMesh";
            ...
            if (tNode.GSDSpline.tRoad.GSDRS.opt_bSaveMeshes) SaveMesh(ref xMesh, false);
...
//				}
                ...
                if (bBCFlipX) BCCenter.z *= -1f;
                if (bBCFlipZ) BCCenter.x *= -1f;
            ...
            if (bCollect) tNode.GSDSpline.tRoad.bTriggerGC = true;
#endif
        }
        ...
        private void SaveMesh(ref Mesh tMesh, bool bIsCollider)
        {
            ...
            if (bIsCollider) FinalName = tFolderName + tSceneName + "-" + tRoadName + "-" + tName + "-collider.asset";
        ...
        private void DoStretch(ref Vector3[] OrigVerts, ref Vector2[] OrigUV, ref int[] OrigTris,
            ref List<int> MaxVectorIndices, ref List<int> MinVectorIndices, float mMaxDiff, out Vector3[] tVerts,
            out Vector2[] tUV, out Vector3[] tNormals, out int[] tTris)
        {
...
//			}
            ...
            for (var i = 0; i < OrigMVL; i++)
            {
                ...
                if (RepeatUVType == RepeatUVTypeEnum.X)
                {
                    if (OrigUV[i].x > Stretch_UVThreshold) tUV[i].x = OrigUV[i].x * UVMod;
                }
                else
                {
                    if (OrigUV[i].y > Stretch_UVThreshold) tUV[i].y = OrigUV[i].y * UVMod;
                }
            }
        }
        ...
        private Vector3 GetAverageNormalToGround(GameObject tObj)
        {
            ...
            for (var i = 0; i < xVects.Count; i++) tHitNormal += xVects[i];
