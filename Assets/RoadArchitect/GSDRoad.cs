...
public class GSDRoad : MonoBehaviour
{
    ...
    [System.NonSerialized] public string EditorTitleString = "";
...
//	private bool bMostRecentCheck = false;
    ...
    [SerializeField] public List<GSDTerrainHistoryMaker> TerrainHistory;
    public string TerrainHistoryByteSize = "";

    [System.NonSerialized] public bool bUpdateSpline = false;
    ...
    [System.NonSerialized] public GSD.Threaded.TerrainCalcs TerrainCalcsJob;
    [System.NonSerialized] public GSD.Threaded.RoadCalcs1 RoadCalcsJob1;
    [System.NonSerialized] public GSD.Threaded.RoadCalcs2 RoadCalcsJob2;
    [System.NonSerialized] public RoadConstructorBufferMaker RCS;

    public string tName = "";
    public bool bProfiling = false;
    public bool bSkipStore = true;
    [System.NonSerialized] public float EditorConstructionStartTime = 0f;
    ...
    private void OnEnable()
    {
        ...
        EditorApplication.hierarchyChanged += delegate { hWindowChanged(); };
#else
    ...
    [System.NonSerialized] public int EditorProgress = 0;
    ...
    {
        if (!Application.isEditor) EditorApplication.update -= delegate { EditorUpdate(); };
        ...
        else if (bEditorProgressBar) RoadUpdateProgressBar();

        if (!Application.isPlaying && bUpdateSpline && !EditorApplication.isPlaying)
        {
            EditorTimerSpline += 1;
    ...
    [System.NonSerialized] public bool bEditorCameraMoving = false;

    [System.NonSerialized] public float EditorCameraPos = 0f;
...
//	float EditorCameraPos_Full = 0f;
    ...
    [System.NonSerialized] public Vector3 EditorCameraOffset = new Vector3(0f, 5f, 0f);
    [System.NonSerialized] public Camera EditorPlayCamera = null;
...
//		EditorCameraPos_Full = 0f;
    ...
    private void hWindowChanged()
    {
        ...
        if (Application.isEditor && EditorApplication.isPlayingOrWillChangePlaymode) return;

        var tCount = 0;
        if (GSDSpline != null && GSDSpline.mNodes != null) tCount = GSDSpline.GetNodeCountNonNull();
        if (tCount != MostRecentNodeCount) bUpdateSpline = true;
    }
    ...
    public void UpdateRoad(RoadUpdateTypeEnum tUpdateType = RoadUpdateTypeEnum.Full)
    {
        ...
        if (Editor_bIsConstructing) return;
        ...
        if (opt_SaveTerrainHistoryOnDisk) ConstructRoad_LoadTerrainHistory();
...
//		if(Application.isEditor && UnityEditor.EditorApplication.isPlayingOrWillChangePlaymode){ return; }
        ...
        var mCount = GSDSpline.GetNodeCount();
...
//				}
                ...
                if (tNode.bIsIntersection && tNode.GSDRI != null)
                {
                    ...
                    tNode.GSDRI.Node1.iConstruction = new GSDIntersections.iConstructionMaker();
                    tNode.GSDRI.Node2.iConstruction = new GSDIntersections.iConstructionMaker();
                }
...
//		if(bProfiling){ UnityEngine.Profiling.Profiler.EndSample(); }
        ...
        if (opt_UseDefaultMaterials) SetDefaultMats();
        ...
        for (var i = tObjs.Count - 1; i >= 0; i--)
        {
            tMainMeshes = tObjs[i];
            DestroyImmediate(tMainMeshes);
        }
        ...
        if (GSDRS == null) GSDRS = transform.parent.GetComponent<GSDRoadSystem>();
        ...
        var tTerrain = GSDRoadUtil.GetTerrain(GSDSpline.mNodes[0].pos);
        var bSameTerrain = true;
        for (var i = 1; i < mCount; i++)
            if (tTerrain != GSDRoadUtil.GetTerrain(GSDSpline.mNodes[0].pos))
            {
                bSameTerrain = false;
                break;
            }
    ...
    public void ConstructRoad_StoreTerrainHistory(bool bDiskOnly = false)
    {
        ...
        else
        {
            ...
            {
                var tSize = 0;
                for (var i = 0; i < TerrainHistory.Count; i++) tSize += TerrainHistory[i].GetSize();
    ...
    public void ConstructRoad_LoadTerrainHistory(bool bForce = false)
    {
        ...
        if (bForce) GSDGeneralEditor.TerrainHistory_Delete(this);
    }
    ...
    private void UpdateRoad_NoMultiThreading()
    {
        ...
        EditorProgress = 50;
        var tRoad = this;
        if (bProfiling) UnityEngine.Profiling.Profiler.BeginSample("RoadCon_RoadPrelim");
    ...
    private void Construction_Cleanup()
    {
        ...
        if (GSDRS.opt_bSaveMeshes) AssetDatabase.SaveAssets();
        bEditorProgressBar = false;
        EditorUtility.ClearProgressBar();
        ...
        if (PiggyBacks != null && PiggyBacks.Length > 0)
        {
            ...
            if (PiggyBacks != null)
            {
                ...
                if (NewPiggys != null) tPiggy.tRoad.PiggyBacks = NewPiggys;
                NewPiggys = null;
    ...
    private void Update()
    {
        ...
        if (bEditorTestTimer)
        {
            if (transform.name == "Road1")
            {
                ...
                if (EditorTestTimer > 2f) //					UpdateRoad(RoadUpdateTypeEnum.Full);
...
//				int xTemp = (int)tSubject[j];
    ...
    public bool DetectInvalidDefaultMatsForUndo()
    {
        ...
        var tMat2 = GSDRoadUtilityEditor.GiveMaterial(
            "Assets/RoadArchitect/Materials/Markers/GSDWhiteYellowDouble.mat");
        var tMat4 = GSDRoadUtilityEditor.GiveMaterial(
            "Assets/RoadArchitect/Materials/Markers/GSDWhiteYellowDouble-4L.mat");
        var tMat6 = GSDRoadUtilityEditor.GiveMaterial(
            "Assets/RoadArchitect/Materials/Markers/GSDWhiteYellowDouble-6L.mat");
            ...
            if (tNameLower.Contains("marker"))
            {
                if (opt_Lanes == 2)
                {
                    if (MR.sharedMaterials[0] == tMat4)
                        tCounter += 1;
                    else if (MR.sharedMaterials[0] == tMat6) tCounter += 1;
                }
                else if (opt_Lanes == 4)
                {
                ...
                else if (opt_Lanes == 6)
                {
                    if (MR.sharedMaterials[0] == tMat2)
                        tCounter += 1;
                    else if (MR.sharedMaterials[0] == tMat4) tCounter += 1;
                }
            }

            if (tCounter > 1) return true;
        }
    ...
    public void SetAllCutsToCurrentMaterials()
    {
        ...
        foreach (var MR in MRs)
        {
            ...
            if (tNameLower.Contains("marker"))
            {
                if (tMats_Marker != null) MR.sharedMaterials = tMats_Marker;
            }
            else if (tNameLower.Contains("cut"))
            {
                if (tMats_World != null) MR.sharedMaterials = tMats_World;
            }
        }
    ...
    public Material[] GetMaterials_RoadWorld()
    {
        ...
        if (RoadMaterial1 != null)
        {
            ...
            if (RoadMaterial2 != null)
            {
                ...
                {
                    mCounter += 1;
                    if (RoadMaterial4 != null) mCounter += 1;
                }
            }
        }
            ...
            if (RoadMaterial1 != null)
            {
                ...
                if (RoadMaterial2 != null)
                {
                    ...
                    if (RoadMaterial3 != null)
                    {
                        ...
                        if (RoadMaterial4 != null) tMats[3] = RoadMaterial4;
                    }
                }
            }
    ...
    public Material[] GetMaterials_RoadMarker()
    {
        ...
        if (RoadMaterialMarker1 != null)
        {
            ...
            if (RoadMaterialMarker2 != null)
            {
                ...
                {
                    mCounter += 1;
                    if (RoadMaterialMarker4 != null) mCounter += 1;
                }
            }
        }
            ...
            if (RoadMaterialMarker1 != null)
            {
                ...
                if (RoadMaterialMarker2 != null)
                {
                    ...
                    if (RoadMaterialMarker3 != null)
                    {
                        ...
                        if (RoadMaterialMarker4 != null) tMats[3] = RoadMaterialMarker4;
                    }
                }
            }
    ...
    {
        if (!opt_bShouldersEnabled) return null;
        ...
        if (ShoulderMaterial1 != null)
        {
            ...
            if (ShoulderMaterial2 != null)
            {
                ...
                {
                    mCounter += 1;
                    if (ShoulderMaterial4 != null) mCounter += 1;
                }
            }
        }
            ...
            if (ShoulderMaterial1 != null)
            {
                ...
                if (ShoulderMaterial2 != null)
                {
                    ...
                    if (ShoulderMaterial3 != null)
                    {
                        ...
                        if (ShoulderMaterial4 != null) tMats[3] = ShoulderMaterial4;
                    }
                }
            }
    ...
    public Material[] GetMaterials_ShoulderMarker()
    {
        ...
        if (ShoulderMaterialMarker1 != null)
        {
            ...
            if (ShoulderMaterialMarker2 != null)
            {
                ...
                {
                    mCounter += 1;
                    if (ShoulderMaterialMarker4 != null) mCounter += 1;
                }
            }
        }
            ...
            if (ShoulderMaterialMarker1 != null)
            {
                ...
                if (ShoulderMaterialMarker2 != null)
                {
                    ...
                    if (ShoulderMaterialMarker3 != null)
                    {
                        ...
                        if (ShoulderMaterialMarker4 != null) tMats[3] = ShoulderMaterialMarker4;
                    }
                }
            }
    ...
    {
        if (!opt_UseDefaultMaterials) return;

        if (!RoadMaterial1)
        {
            RoadMaterial1 = GSDRoadUtilityEditor.GiveMaterial("Assets/RoadArchitect/Materials/GSDRoad1.mat");
            RoadMaterial2 =
                GSDRoadUtilityEditor.GiveMaterial("Assets/RoadArchitect/Materials/Markers/GSDRoadDetailOverlay1.mat");
        }
        ...
        if (!RoadMaterialMarker1)
        {
            if (opt_Lanes == 2)
                RoadMaterialMarker1 =
                    GSDRoadUtilityEditor.GiveMaterial(
                        "Assets/RoadArchitect/Materials/Markers/GSDWhiteYellowDouble.mat");
            else if (opt_Lanes == 4)
                RoadMaterialMarker1 =
                    GSDRoadUtilityEditor.GiveMaterial(
            ...
            if (opt_Lanes == 2)
                RoadMaterialMarker2 =
                    GSDRoadUtilityEditor.GiveMaterial("Assets/RoadArchitect/Materials/Markers/GSDTireMarks.mat");
            else if (opt_Lanes == 4)
                RoadMaterialMarker2 =
                    GSDRoadUtilityEditor.GiveMaterial("Assets/RoadArchitect/Materials/Markers/GSDTireMarks-4L.mat");
            else if (opt_Lanes == 6)
                RoadMaterialMarker2 =
                    GSDRoadUtilityEditor.GiveMaterial("Assets/RoadArchitect/Materials/Markers/GSDTireMarks-6L.mat");
            else
                RoadMaterialMarker2 =
                    GSDRoadUtilityEditor.GiveMaterial("Assets/RoadArchitect/Materials/Markers/GSDTireMarks.mat");
        }
        ...
        {
            ShoulderMaterial1 = GSDRoadUtilityEditor.GiveMaterial("Assets/RoadArchitect/Materials/GSDShoulder1.mat");
            ShoulderMaterial2 =
                GSDRoadUtilityEditor.GiveMaterial("Assets/RoadArchitect/Materials/Markers/GSDRoadDetailOverlay1.mat");
        }
        ...
        if (opt_bShouldersEnabled && !RoadPhysicMaterial)
            RoadPhysicMaterial =
                GSDRoadUtilityEditor.GivePhysicsMaterial("Assets/RoadArchitect/Physics/GSDPavement.physicMaterial");
        if (opt_bShouldersEnabled && !ShoulderPhysicMaterial)
            ShoulderPhysicMaterial =
                GSDRoadUtilityEditor.GivePhysicsMaterial("Assets/RoadArchitect/Physics/GSDDirt.physicMaterial");
    }
    ...
    public void SetDefaultMats()
    {
        ...
        else if (opt_tRoadMaterialDropdown == RoadMaterialDropdownEnum.Dirt)
        {
            ...
            RoadMaterial3 = null;
            RoadMaterial4 = null;
            RoadMaterialMarker1 = GSDRoadUtilityEditor.GiveMaterial("Assets/RoadArchitect/Materials/GSDDirtRoad.mat");
        ...
        else if (opt_tRoadMaterialDropdown == RoadMaterialDropdownEnum.Brick)
        {
            ...
            RoadMaterial3 = null;
            RoadMaterial4 = null;
            RoadMaterialMarker1 = GSDRoadUtilityEditor.GiveMaterial("Assets/RoadArchitect/Materials/GSDBrickRoad.mat");
            RoadMaterialMarker2 = null;
            RoadMaterialMarker3 = null;
            RoadMaterialMarker4 = null;
        }
        else if (opt_tRoadMaterialDropdown == RoadMaterialDropdownEnum.Cobblestone)
        {
            ...
            RoadMaterial4 = null;
            RoadMaterialMarker1 =
                GSDRoadUtilityEditor.GiveMaterial("Assets/RoadArchitect/Materials/GSDCobblestoneRoad.mat");
            RoadMaterialMarker2 = null;
            RoadMaterialMarker3 = null;
            RoadMaterialMarker4 = null;
        }
        ...
        var mCount = GSDSpline.GetNodeCount();
        for (var i = 0; i < mCount; i++)
            if (GSDSpline.mNodes[i] && GSDSpline.mNodes[i].bIsIntersection && GSDSpline.mNodes[i].GSDRI != null &&
                GSDSpline.mNodes[i].GSDRI.bUseDefaultMaterials)
    ...
    {
        var tCount = tMRs.Length;
        for (var i = 0; i < tCount; i++) //EditorUtility.SetSelectedWireframeHidden(tMRs[i], !opt_GizmosEnabled);
            EditorUtility.SetSelectedRenderState(tMRs[i],
                opt_GizmosEnabled ? EditorSelectedRenderState.Wireframe : EditorSelectedRenderState.Hidden);
    }
    ...
    {
#if UNITY_EDITOR
        if (Application.isPlaying) CleanRunTime();
#else
			this.enabled = false;
#endif
    }
    ...
    public void UpdateGizmoOptions()
    {
        ...
        {
            tNode = GSDSpline.mNodes[i];
            if (tNode != null) tNode.opt_GizmosEnabled = opt_GizmosEnabled;
        }
    }
    ...
    public void DuplicateRoad()
    {
        ...
        Undo.RegisterCreatedObjectUndo(tRoadObj, "Duplicate");
    ...
    private void FixZ_Win()
    {
        ...
        //Intersections (all):
        tMarkerObjs = FindObjectsOfType<MeshRenderer>();
        foreach (MeshRenderer MR in tMarkerObjs)
