...
public class GSDTrafficLightController
{
    ...
    public GSDTrafficLightController(ref GameObject _LightLeft, ref GameObject _LightRight, ref GameObject[] _Lights,
        ref MeshRenderer _MR_Left, ref MeshRenderer _MR_Right, ref MeshRenderer[] MR_Mains)
    {
        ...
        if (LightLeftObj != null)
        {
            ...
            {
                if (tLight.transform.name.ToLower().Contains("redlight")) LightLeft_R = tLight;
                if (tLight.transform.name.ToLower().Contains("yellowlight")) LightLeft_Y = tLight;
                if (tLight.transform.name.ToLower().Contains("greenl")) LightLeft_G = tLight;
            }
        }
        ...
        if (LightRightObj != null)
        {
            ...
            foreach (var tLight in tLights)
            {
                if (tLight.transform.name.ToLower().Contains("redlight")) LightRight_R = tLight;
                if (tLight.transform.name.ToLower().Contains("yellowlight")) LightRight_Y = tLight;
                if (tLight.transform.name.ToLower().Contains("greenl")) LightRight_G = tLight;
            }
        }

        var mCount = LightsObj.Length;
        ...
        for (var i = 0; i < mCount; i++)
        {
            ...
            foreach (var tLight in tLights)
            {
                if (tLight.transform.name.ToLower().Contains("redlight")) Lights_R[i] = tLight;
                if (tLight.transform.name.ToLower().Contains("yellowlight")) Lights_Y[i] = tLight;
                if (tLight.transform.name.ToLower().Contains("greenl")) Lights_G[i] = tLight;
            }
        }
    }
    ...
    private void MRChange(ref MeshRenderer MR, iLightSubStatusEnum iLSSE)
    {
        if (bUseSharedMaterial)
        {
            ...
            else if (iLSSE == iLightSubStatusEnum.Red) MR.sharedMaterial.mainTextureOffset = new Vector2(0f, 0f);
        }
        else
        {
            if (iLSSE == iLightSubStatusEnum.Green)
            ...
            else if (iLSSE == iLightSubStatusEnum.Red) MR.material.mainTextureOffset = new Vector2(0f, 0f);
        }
    }
    ...
    private void LightChange(int tIndex, iLightSubStatusEnum iLSSE)
    {
        if (!bLightsEnabled)
        {
            var mCount = MR_MainsStorage.Length;
            for (var i = 0; i < mCount; i++)
            {
...
public class GSDTrafficLightSequence
{
    ...
    public GSDTrafficLightController.iLightControllerEnum iLightController =
        GSDTrafficLightController.iLightControllerEnum.Regular;

    public GSDTrafficLightController.iLightSubStatusEnum iLightSubcontroller =
        GSDTrafficLightController.iLightSubStatusEnum.Green;

    public float tTime = 10f;

    public GSDTrafficLightSequence(bool bPath1, GSDTrafficLightController.iLightControllerEnum tLightController,
        GSDTrafficLightController.iLightSubStatusEnum tLightSubcontroller, float xTime)
    {
    ...
    {
        return "Path1: " + bLightMasterPath1 + " iLightController: " + iLightController.ToString() +
               " iLightSubcontroller: " + iLightSubcontroller.ToString() + " tTime: " + tTime.ToString("0F");
    }
}