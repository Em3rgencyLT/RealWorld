...
public class GSDRoadEditor : Editor
{
    ...
    protected GSDRoad RS => (GSDRoad) target;
    ...
    private void Init()
    {
        ...
        if (WarningLabelBG == null)
            WarningLabelBG =
                (Texture2D) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/WarningLabelBG.png",
        ...
        if (btnRefreshTextReal == null)
            btnRefreshTextReal =
                (Texture) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/refresh.png",
                    typeof(Texture)) as Texture;
        if (LoadBtnBG == null)
            LoadBtnBG = (Texture2D) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/otherbg.png",
                typeof(Texture2D)) as Texture2D;
        if (LoadBtnBGGlow == null)
            LoadBtnBGGlow =
                (Texture2D) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/otherbg2.png",
                    typeof(Texture2D)) as Texture2D;
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Update road", GSDLoadButton)) RS.EditorUpdateMe = true;
        ...
        else if (tLanesEnum == tempEnum.Six) t_opt_Lanes.intValue = 6;

        //Option: Lane and road width:
        EditorGUILayout.BeginHorizontal();
        t_opt_LaneWidth.floatValue = EditorGUILayout.FloatField("Lane width:", RS.opt_LaneWidth);
        if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f))) t_opt_LaneWidth.floatValue = 5f;
        EditorGUILayout.EndHorizontal();
        ...
        t_opt_UseDefaultMaterials.boolValue =
            EditorGUILayout.Toggle("Use default materials:", RS.opt_UseDefaultMaterials);

        //Dropdown:
        if (RS.opt_UseDefaultMaterials)
        {
            var Old = (int) RS.opt_tRoadMaterialDropdown;
            t_opt_tRoadMaterialDropdown.enumValueIndex = (int) EditorGUILayout.Popup("Road material: ",
                (int) RS.opt_tRoadMaterialDropdown, RoadMaterialDropdownEnumDesc, GUILayout.Width(250f));
        ...
        //Mesh colliders:
        if (RS.GSDRS != null)
            t_opt_bUseMeshColliders.boolValue =
                EditorGUILayout.Toggle("Use mesh colliders: ", RS.opt_bUseMeshColliders);
        ...
        //Static:
        if (RS.GSDRS != null) t_opt_bIsStatic.boolValue = EditorGUILayout.Toggle("Static: ", RS.opt_bIsStatic);
        ...
        if (GUILayout.Button("Duplicate road", EditorStyles.miniButton, GUILayout.Width(120f))) RS.DuplicateRoad();
        ...
        t_opt_TerrainSubtract_Match.floatValue =
            EditorGUILayout.Slider("Terrain subtraction: ", RS.opt_TerrainSubtract_Match, 0.01f, 1f);
        if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f)))
        ...
        EditorGUILayout.BeginHorizontal();
        t_opt_MagnitudeThreshold.floatValue =
            EditorGUILayout.Slider("Spline magnitude limit: ", RS.opt_MagnitudeThreshold, 128f, 8192f);
        if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f)))
        ...
        {
            EditorGUILayout.BeginHorizontal();
            t_opt_MatchHeightsDistance.floatValue =
                EditorGUILayout.Slider("Heights match width: ", RS.opt_MatchHeightsDistance, 0.01f, 512f);
            if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f)))
        ...
        {
            EditorGUILayout.BeginHorizontal();
            t_opt_ClearDetailsDistance.floatValue =
                EditorGUILayout.Slider("Details clear width: ", RS.opt_ClearDetailsDistance, 0.01f, 512f);
            if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f)))
            ...
            EditorGUILayout.BeginHorizontal();
            t_opt_ClearDetailsDistanceHeight.floatValue = EditorGUILayout.Slider("Details clear height: ",
                RS.opt_ClearDetailsDistanceHeight, 0.01f, 512f);
            if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f)))
        ...
        {
            EditorGUILayout.BeginHorizontal();
            t_opt_ClearTreesDistance.floatValue =
                EditorGUILayout.Slider("Trees clear width: ", RS.opt_ClearTreesDistance, 0.01f, 512f);
            if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f)))
                t_opt_ClearTreesDistance.floatValue = 30f;
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.BeginHorizontal();
            t_opt_ClearTreesDistanceHeight.floatValue = EditorGUILayout.Slider("Trees clear height: ",
                RS.opt_ClearTreesDistanceHeight, 0.01f, 512f);
            if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f)))
        ...
        t_opt_SaveTerrainHistoryOnDisk.boolValue =
            EditorGUILayout.Toggle(RS.opt_SaveTerrainHistoryOnDisk, GUILayout.Width(50f));
        EditorGUILayout.EndHorizontal();
        ...
        t_EditorCameraMetersPerSecond.floatValue =
            EditorGUILayout.Slider("Camera meters/sec:", RS.EditorCameraMetersPerSecond, 1f, 512f);
        if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f)))
        ...
        if (RS.EditorPlayCamera == null) RS.EditorCameraSetSingle();
        RS.EditorPlayCamera =
            (Camera) EditorGUILayout.ObjectField("Editor play camera:", RS.EditorPlayCamera, typeof(Camera), true);
        EditorGUILayout.BeginHorizontal();
        ...
        {
            if (GUILayout.Button("Pause", GUILayout.Width(70f))) RS.bEditorCameraMoving = false;
        }
        else
        {
            if (GUILayout.Button("Play", GUILayout.Width(70f))) RS.bEditorCameraMoving = true;
        }
        ...
        bSetDefaultMats = false;
        if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f))) bSetDefaultMats = true;
        EditorGUILayout.EndHorizontal();
...
//			if(RS.ShoulderMaterial3 != null){EditorGUILayout.PropertyField (t_ShoulderMaterial4, new GUIContent ("  Mat #4: ")); }
            ...
            if (RS.ShoulderMaterial1 != null)
            {
                ...
                if (RS.ShoulderMaterial2 != null &&
                    GUILayout.Button(btnDeleteText, GSDImageButton, GUILayout.Width(16f))) RS.ShoulderMaterial2 = null;
                EditorGUILayout.EndHorizontal();
            }
            ...
            if (RS.ShoulderMaterial1 != null && RS.ShoulderMaterial2 != null)
            {
                ...
                if (RS.ShoulderMaterial3 != null &&
                    GUILayout.Button(btnDeleteText, GSDImageButton, GUILayout.Width(16f))) RS.ShoulderMaterial3 = null;
                EditorGUILayout.EndHorizontal();
            }
            ...
            if (RS.ShoulderMaterial1 != null && RS.ShoulderMaterial2 != null && RS.ShoulderMaterial3 != null)
            {
                ...
                if (RS.ShoulderMaterial4 != null &&
                    GUILayout.Button(btnDeleteText, GSDImageButton, GUILayout.Width(16f))) RS.ShoulderMaterial4 = null;
                EditorGUILayout.EndHorizontal();
            }
        }
...
//		//Shoulder marker material defaults:
        if (RS.opt_bShouldersEnabled)
        {
            ...
            if (RS.ShoulderMaterialMarker1 != null &&
                GUILayout.Button(btnDeleteText, GSDImageButton, GUILayout.Width(16f)))
            ...
            if (RS.ShoulderMaterialMarker1 != null)
            {
                ...
                if (RS.ShoulderMaterialMarker2 != null &&
                    GUILayout.Button(btnDeleteText, GSDImageButton, GUILayout.Width(16f)))
            ...
            if (RS.ShoulderMaterialMarker1 != null && RS.ShoulderMaterialMarker2 != null)
            {
                ...
                if (RS.ShoulderMaterialMarker3 != null &&
                    GUILayout.Button(btnDeleteText, GSDImageButton, GUILayout.Width(16f)))
                ...
                RS.ShoulderMaterialMarker3 != null)
            {
                ...
                if (RS.ShoulderMaterialMarker4 != null &&
                    GUILayout.Button(btnDeleteText, GSDImageButton, GUILayout.Width(16f)))
...
//		t_ShoulderPhysicMaterial.serializedObject = (PhysicMaterial)EditorGUILayout.ObjectField("  Shoulder mat: ",RS.ShoulderPhysicMaterial,typeof(PhysicMaterial),false);
        ...
        var bGizmoChange = false;
        var bLaneChange = false;
        var bMultithreadChange = false;
        var bSaveMeshChange = false;
        var bTerrainHistoryChange = false;
        var bEditorCameraSpeedChange = false;
        ...
        if (GUI.changed)
        {
            ...
            if (!GSDRootUtil.IsApproximately(t_EditorCameraMetersPerSecond.floatValue, RS.EditorCameraMetersPerSecond,
                0.001f)) bEditorCameraSpeedChange = true;
            ...
            if (bMultithreadChange) RS.GSDRS.UpdateAllRoads_MultiThreadOptions();

            //Option: Save meshes as unity assets options:
            if (bSaveMeshChange) RS.GSDRS.UpdateAllRoads_SaveMeshesAsAssetsOptions();
            ...
            //Option: Editor camera meters per sec
            if (bEditorCameraSpeedChange) RS.ChangeEditorCameraMetersPerSec();

            //Update road:
            if (bNeedRoadUpdate) RS.GSDSpline.Setup_Trigger();
    ...
    public void OnSceneGUI()
    {
        ...
        if (Selection.Contains(RS.transform.gameObject) && Selection.objects.Length > 1) SetSelectionToRoad();
        ...
        if (Selection.activeGameObject == RS.transform.gameObject)
        {
            ...
            if (Event.current.type == EventType.MouseMove || Event.current.type == EventType.MouseDrag)
            {
                if (current.control)
                {
                    ...
                    if (Physics.Raycast(worldRay, out hitInfo))
                    {
                        if (hitInfo.collider.transform.GetComponent<Terrain>() != null ||
                            hitInfo.collider.transform.name.ToLower().Contains("terrain"))
                        {
                            ...
                            if (RS.GSDSpline && RS.GSDSpline.PreviewSpline)
                            {
                                ...
                                if (RS.GSDSpline.PreviewSpline.mNodes == null ||
                                    RS.GSDSpline.PreviewSpline.mNodes.Count < 1) RS.GSDSpline.Setup();
                    ...
                    if (Physics.Raycast(worldRay, out hitInfo))
                    {
                        if (hitInfo.collider.transform.GetComponent<Terrain>() != null)
                        {
                            ...
                            if (RS.GSDSpline && RS.GSDSpline.PreviewSplineInsert)
                            {
                                ...
                                if (RS.GSDSpline.PreviewSplineInsert.mNodes == null ||
                                    RS.GSDSpline.PreviewSplineInsert.mNodes.Count < 1)
                ...
                else
                {
                    ...
                    if (RS.GSDSpline && RS.GSDSpline.PreviewSpline) //Debug.Log("not drawing new node");
                        RS.GSDSpline.PreviewSpline.bGizmoDraw = false;
                    if (RS.GSDSpline && RS.GSDSpline.PreviewSplineInsert) //Debug.Log("not drawing insert node");
        ...
        else
        {
            ...
            if (RS.GSDSpline.PreviewSpline) RS.GSDSpline.PreviewSpline.bGizmoDraw = false;
        }
        ...
        if (current.control)
        {
            if (Event.current.type == EventType.MouseDown) //Left click:
...
//						}
        ...
        if (GUI.changed) EditorUtility.SetDirty(RS);
    }


    private void TriggerRoadUpdate()
    {
        if (RS != null) RS.EditorUpdateMe = true;
    }
