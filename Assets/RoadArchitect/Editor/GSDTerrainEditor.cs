...
{
    protected GSDTerrain GSDT => (GSDTerrain) target;

    //Serialized properties:
    private SerializedProperty tSplatImageWidth;
    private SerializedProperty tSplatImageHeight;
    private SerializedProperty tSplatBackgroundColor;
    private SerializedProperty tSplatForegroundColor;
    private SerializedProperty tSplatWidth;
    private SerializedProperty tSkipBridges;
    private SerializedProperty tSkipTunnels;
    private SerializedProperty tSplatSingleRoad;
    private SerializedProperty tSplatSingleChoiceIndex;
    private SerializedProperty tRoadSingleChoiceUID;
    ...
    private SplatImageResoMatchingEnum tSplatReso = SplatImageResoMatchingEnum.None;
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.BeginHorizontal();
        tSplatReso =
            (SplatImageResoMatchingEnum) EditorGUILayout.Popup("Match resolutions:", (int) tSplatReso,
                TheSplatResoOptions);
        ...
        //Default button:
        if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f))) tSplatWidth.floatValue = 30f;
        EditorGUILayout.EndHorizontal();
        ...
        if (GSDT.SplatSingleRoad)
        {
            ...
            tSplatSingleChoiceIndex.intValue =
                EditorGUILayout.Popup(GSDT.SplatSingleChoiceIndex, tRoadsString, GUILayout.Width(150f));
...
//			EditorUtility.SetDirty(target); //Necessary?
    ...
    private void InitNullChecks()
    {
        ...
        if (LoadBtnBG == null)
            LoadBtnBG = (Texture2D) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/FlexBG.png",
                typeof(Texture2D)) as Texture2D;
        if (LoadBtnBGGlow == null)
            LoadBtnBGGlow =
                (Texture2D) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/FlexBG.png",
                    typeof(Texture2D)) as Texture2D;
    ...
    private void LoadSplatSingleChoice()
    {
        tRoads = null;
        tRoadsString = null;
        var xRoads = FindObjectsOfType(typeof(GSDRoad));
        var xRoadsCount = xRoads.Length;
    ...
    private void GenerateSplatMap()
    {
        ...
        if (GSDT.SplatSingleRoad && GSDT.RoadSingleChoiceUID != "")
            tBytes = GSD.Roads.GSDRoadUtil.MakeSplatMap(GSDT.tTerrain, GSDT.SplatBackground, GSDT.SplatForeground,
                GSDT.SplatResoWidth, GSDT.SplatResoHeight, GSDT.SplatWidth, GSDT.SplatSkipBridges,
                GSDT.SplatSkipTunnels, GSDT.RoadSingleChoiceUID);
        else
            tBytes = GSD.Roads.GSDRoadUtil.MakeSplatMap(GSDT.tTerrain, GSDT.SplatBackground, GSDT.SplatForeground,
                GSDT.SplatResoWidth, GSDT.SplatResoHeight, GSDT.SplatWidth, GSDT.SplatSkipBridges,
                GSDT.SplatSkipTunnels);

        if (tBytes != null && tBytes.Length > 3)
        {
            var tPath = EditorUtility.SaveFilePanel("Save splat map", Application.dataPath, "Splat", "png");
            if (tPath != null && tPath.Length > 3) System.IO.File.WriteAllBytes(tPath, tBytes);
            tBytes = null;
        }
    }
