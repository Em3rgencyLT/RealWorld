...
public class GSDSplineNEditor : Editor
{
    ...
    protected GSDSplineN tNode => (GSDSplineN) target;
...
//	bool t_opt_GizmosEnabled = false;
    ...
    private void Init()
    {
        ...
        if (btnDeleteText == null)
            btnDeleteText =
                (Texture) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/delete.png", typeof(Texture))
                as Texture;
        if (btnCopyText == null)
            btnCopyText =
                (Texture) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/copy.png", typeof(Texture))
                as Texture;
        if (btnLoadText == null)
            btnLoadText =
                (Texture) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/load.png", typeof(Texture))
                as Texture;
        if (btnSaveText == null)
            btnSaveText =
                (Texture) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/save.png", typeof(Texture))
                as Texture;
        if (btnExtrudeText == null)
            btnExtrudeText =
                (Texture) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/extrude.png",
                    typeof(Texture)) as Texture;
        if (btnEdgeText == null)
            btnEdgeText =
                (Texture) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/edge.png", typeof(Texture))
                as Texture;
        if (btnHelpText == null)
            btnHelpText =
                (Texture) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/help.png", typeof(Texture))
                as Texture;
        if (GSDTextAreaBG == null)
            GSDTextAreaBG =
                (Texture2D) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/popupbg.png",
                    typeof(Texture2D)) as Texture2D;
        if (LoadBtnBG == null)
            LoadBtnBG = (Texture2D) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/loadbg.png",
                typeof(Texture2D)) as Texture2D;
        if (LoadBtnBGGlow == null)
            LoadBtnBGGlow =
                (Texture2D) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/loadbgglow.png",
                    typeof(Texture2D)) as Texture2D;
        if (ManualBG == null)
            ManualBG = (Texture2D) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/manualbg.png",
                typeof(Texture2D)) as Texture2D;
        if (btnRefreshText == null)
            btnRefreshText =
                (Texture) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/refresh.png",
                    typeof(Texture)) as Texture;
        if (btnDefaultText == null)
            btnDefaultText =
                (Texture) AssetDatabase.LoadAssetAtPath("Assets/RoadArchitect/Editor/Icons/refresh2.png",
                    typeof(Texture)) as Texture;
        ...
        var tRoadWidthHalf = tNode.GSDSpline.tRoad.RoadWidth() * 0.5f;
        ...
        if (tNode.GSDSpline.tRoad.opt_bShouldersEnabled)
        {
            HorizMatchSubTypeDescriptions[3] = "Match shoulder left edge: -" +
                                               (tRoadWidthHalf + tNode.GSDSpline.tRoad.opt_ShoulderWidth)
                                               .ToString("F1") + " meters";
        ...
        else
        {
            HorizMatchSubTypeDescriptions[2] =
                "Match shoulder left edge: -" + tRoadWidthHalf.ToString("F1") + " meters";
            HorizMatchSubTypeDescriptions[4] =
                "Match shoulder right edge: " + tRoadWidthHalf.ToString("F1") + " meters";
        }
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (tNode.idOnSpline > 0 && tNode.idOnSpline < tNode.GSDSpline.GetNodeCount() - 1 && !tNode.bIsIntersection &&
            !tNode.bSpecialEndNode)
        {
        ...
        if (tNode.bSpecialRoadConnPrimary)
        {
            ...
            if (GUILayout.Button("Break road connection")) tNode.SpecialNodeCounterpart.BreakConnection();
            EditorGUILayout.EndHorizontal();
            if (tNode.SpecialNodeCounterpart != null)
                EditorGUILayout.LabelField(tNode.SpecialNodeCounterpart.GSDSpline.tRoad.transform.name + " to " +
                                           tNode.SpecialNodeCounterpart.SpecialNodeCounterpart.GSDSpline.tRoad.transform
                                               .name);
        ...
        if (GUI.changed)
        {
            ...
            //Option: Manual cut:
            if (tNode.idOnSpline > 0 && tNode.idOnSpline < tNode.GSDSpline.GetNodeCount() - 1 &&
                !tNode.bIsIntersection &&
                !tNode.bSpecialEndNode) // && !cNode.bIsBridge_PreNode && !cNode.bIsBridge_PostNode){
                if (tNode.GSDSpline.tRoad.opt_bDynamicCuts)
    ...
    private void DoExtAndEdgeOverview()
    {
        ...
        if (GUILayout.Button("Save group", EditorStyles.miniButton, GUILayout.Width(108f)) ||
            GUILayout.Button(btnSaveText, GSDImageButton, GUILayout.Width(16f)))
        {
        ...
        EditorGUILayout.BeginHorizontal();
        EditorGUILayout.LabelField("");
        if (GUILayout.Button("Add custom extrusion object", EditorStyles.miniButton)) tNode.AddSplinatedObject();
        EditorGUILayout.EndHorizontal();
        GUILayout.Space(4f);
        EditorGUILayout.BeginHorizontal();
        EditorGUILayout.LabelField("");
        if (GUILayout.Button("Add custom edge object", EditorStyles.miniButton)) tNode.AddEdgeObject();
        EditorGUILayout.EndHorizontal();
    ...
    public void DoSplineObjects()
    {
        ...
        for (var i = 0; i < tNode.SplinatedObjects.Count; i++)
        {
            currentCount += 1;
            SMM = tNode.SplinatedObjects[i];
            if (SMM.EM == null) SMM.EM = new GSD.Roads.Splination.SplinatedMeshMaker.SplinatedMeshEditorMaker();
            SMM.EM.Setup(SMM);
            ...
            if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f))) SMM.Setup();
            if (GUILayout.Button(btnSaveText, GSDImageButton, GUILayout.Width(16f)))
            {
                var tSave = EditorWindow.GetWindow<GSDSaveWindow>();
            ...
            //Game object (prefab):
            SMM.EM.CurrentSplination =
                (GameObject) EditorGUILayout.ObjectField("Prefab:", SMM.CurrentSplination, typeof(GameObject), false);

            //Game object (prefab start cap):
            SMM.EM.CurrentSplinationCap1 = (GameObject) EditorGUILayout.ObjectField("Prefab start cap:",
                SMM.CurrentSplinationCap1, typeof(GameObject), false);
            ...
            SMM.EM.CurrentSplinationCap2 = (GameObject) EditorGUILayout.ObjectField("Prefab end cap:",
                SMM.CurrentSplinationCap2, typeof(GameObject), false);
            ...
            {
                SMM.EM.SplinatedMaterial1 = (Material) EditorGUILayout.ObjectField("Override mat #1: ",
                    SMM.SplinatedMaterial1, typeof(Material), false);
                SMM.EM.SplinatedMaterial2 = (Material) EditorGUILayout.ObjectField("Override mat #2: ",
                    SMM.SplinatedMaterial2, typeof(Material), false);
            }

            //Axis:
            SMM.EM.Axis = (GSD.Roads.Splination.AxisTypeEnum) EditorGUILayout.Popup("Extrusion axis: ", (int) SMM.Axis,
                TheAxisDescriptions_Spline, GUILayout.Width(250f));
            ...
            EditorGUILayout.BeginHorizontal();
            SMM.EM.StartTime = EditorGUILayout.Slider("Start param: ", SMM.StartTime, tNode.MinSplination,
                tNode.MaxSplination - 0.01f);
            ...
            if (tNode.IsStraight())
            {
                ...
                else
                {
                    ...
                    //Stretch_UVThreshold:
                    SMM.EM.Stretch_UVThreshold =
                        EditorGUILayout.Slider("UV stretch threshold:", SMM.Stretch_UVThreshold, 0.01f, 0.5f);

                    //UV repeats:
                    SMM.EM.RepeatUVType = (GSD.Roads.Splination.RepeatUVTypeEnum) EditorGUILayout.Popup(
                        "UV stretch axis: ", (int) SMM.RepeatUVType, RepeatUVTypeDescriptions_Spline,
...
//			SMM.EM.bMatchRoadIncrements = EditorGUILayout.Toggle("Match road increments: ",SMM.bMatchRoadIncrements); 
            ...
            if (GUILayout.Button(btnDefaultText, GSDImageButton, GUILayout.Width(16f))) SMM.EM.MinMaxMod = 0.05f;
            EditorGUILayout.EndHorizontal();

            //Vertex matching precision:
            EditorGUILayout.BeginHorizontal();
            SMM.EM.VertexMatchingPrecision =
            ...
            //UV repeats:
            if (!SMM.bIsStretch)
                SMM.EM.RepeatUVType = (GSD.Roads.Splination.RepeatUVTypeEnum) EditorGUILayout.Popup("UV repeat axis: ",
                    (int) SMM.RepeatUVType, RepeatUVTypeDescriptions_Spline, GUILayout.Width(250f));

            if (SMM.bMatchRoadDefinition)
            {
                EditorGUILayout.BeginVertical("TextArea");
                EditorGUILayout.BeginHorizontal();
                SMM.EM.bMatchRoadDefinition =
                    EditorGUILayout.Toggle("Match road definition: ", SMM.bMatchRoadDefinition);
                if (GUILayout.Button(btnDefaultText, GSDImageButton, GUILayout.Width(16f)))
            ...
            if (GUILayout.Button(btnDefaultText, GSDImageButton, GUILayout.Width(16f))) SMM.EM.VerticalRaise = 0f;
            EditorGUILayout.EndHorizontal();
            ...
            tHorizMatching = (HorizMatchingDefaultsEnum) EditorGUILayout.Popup((int) tHorizMatching,
            ...
            if (tHorizMatching != HorizMatchingDefaultsEnum.None)
            {
                if (tHorizMatching == HorizMatchingDefaultsEnum.MatchCenter)
                    ...
                    SMM.EM.HorizontalSep =
                        (tNode.GSDSpline.tRoad.RoadWidth() / 2 + tNode.GSDSpline.tRoad.opt_ShoulderWidth) * -1;
                    ...
                    SMM.EM.HorizontalSep =
                        tNode.GSDSpline.tRoad.RoadWidth() / 2 + tNode.GSDSpline.tRoad.opt_ShoulderWidth;
                tHorizMatching = HorizMatchingDefaultsEnum.None;
            }

            EditorGUILayout.BeginHorizontal();
            SMM.EM.HorizontalSep = EditorGUILayout.Slider("Horiz offset magnitude:", SMM.HorizontalSep,
                -1f * HorizRoadMax, HorizRoadMax);
            if (GUILayout.Button(btnDefaultText, GSDImageButton, GUILayout.Width(16f))) SMM.EM.HorizontalSep = 0f;
            EditorGUILayout.EndHorizontal();
            ...
            {
                SMM.EM.bVerticalCutoff_MatchZero =
                    EditorGUILayout.Toggle("Match spline height:", SMM.bVerticalCutoff_MatchZero);
                SMM.EM.bVerticalCutoffDownwards =
                    EditorGUILayout.Toggle("Cut direction toggle:", SMM.bVerticalCutoffDownwards);
                SMM.EM.VerticalCutoff = EditorGUILayout.Slider("Height cut offset: ", SMM.VerticalCutoff, -50f, 50f);
                SMM.EM.bVerticalMeshCutoff_OppositeDir =
                    EditorGUILayout.Toggle("Opposite dir mesh cut:", SMM.bVerticalMeshCutoff_OppositeDir);
                SMM.EM.VerticalMeshCutoffOffset =
            ...
            if (SMM.bStartDown)
            {
                SMM.EM.bStartTypeDownOverride =
                    EditorGUILayout.Toggle("Override start down value: ", SMM.bStartTypeDownOverride);
                if (SMM.bStartTypeDownOverride)
                    SMM.EM.StartTypeDownOverride =
                        EditorGUILayout.Slider("Downward movement: ", SMM.StartTypeDownOverride, -10f, 10f);
            }

            if (SMM.bEndDown)
            {
                SMM.EM.bEndTypeDownOverride =
                    EditorGUILayout.Toggle("Override end down value: ", SMM.bEndTypeDownOverride);
                if (SMM.bEndTypeDownOverride)
                    SMM.EM.EndTypeDownOverride =
            ...
            SMM.EM.bEndCapCustomMatchStart =
                EditorGUILayout.Toggle("Match objects to ends:", SMM.bEndCapCustomMatchStart);

            //End objects match ground:
            SMM.EM.bEndObjectsMatchGround =
                EditorGUILayout.Toggle("Force origins to ground:", SMM.bEndObjectsMatchGround);
            ...
            tEndObjectAdd =
                (EndObjectsDefaultsEnum) EditorGUILayout.Popup((int) tEndObjectAdd, EndObjectsDefaultsEnumDesc);
            ...
            EditorGUILayout.BeginVertical("box");
            SMM.EM.CollisionType = (GSD.Roads.Splination.CollisionTypeEnum) EditorGUILayout.Popup("Collision type: ",
                (int) SMM.CollisionType, TheCollisionTypeEnumDescSpline, GUILayout.Width(320f));
            //Mesh collison convex option
            if (SMM.CollisionType != GSD.Roads.Splination.CollisionTypeEnum.None &&
            ...
            if (SMM.CollisionType == GSD.Roads.Splination.CollisionTypeEnum.SimpleMeshTriangle ||
                SMM.CollisionType == GSD.Roads.Splination.CollisionTypeEnum.SimpleMeshTrapezoid)
                SMM.EM.bSimpleCollisionAutomatic =
                    EditorGUILayout.Toggle(" Automatic simple collision: ", SMM.bSimpleCollisionAutomatic);
            ...
            if (!SMM.bSimpleCollisionAutomatic)
            {
                if (SMM.CollisionType == GSD.Roads.Splination.CollisionTypeEnum.SimpleMeshTriangle)
                {
                    ...
                    SMM.EM.CollisionTriBL.x = EditorGUILayout.Slider(" x-axis: ", SMM.CollisionTriBL.x, SMM.mMinX - 5f,
                        SMM.mMaxX + 5f);
                    SMM.EM.CollisionTriBL.y = EditorGUILayout.Slider(" y-axis: ", SMM.CollisionTriBL.y, SMM.mMinY - 5f,
                        SMM.mMaxY + 5f);
                    SMM.EM.CollisionTriBL.z = EditorGUILayout.Slider(" z-axis: ", SMM.CollisionTriBL.z, SMM.mMinZ - 5f,
                        SMM.mMaxZ + 5f);

                    EditorGUILayout.LabelField("Bottom right:");
                    SMM.EM.CollisionTriBR.x = EditorGUILayout.Slider(" x-axis: ", SMM.CollisionTriBR.x, SMM.mMinX - 5f,
                        SMM.mMaxX + 5f);
                    SMM.EM.CollisionTriBR.y = EditorGUILayout.Slider(" y-axis: ", SMM.CollisionTriBR.y, SMM.mMinY - 5f,
                        SMM.mMaxY + 5f);
                    SMM.EM.CollisionTriBR.z = EditorGUILayout.Slider(" z-axis: ", SMM.CollisionTriBR.z, SMM.mMinZ - 5f,
                        SMM.mMaxZ + 5f);

                    EditorGUILayout.LabelField("Top:");
                    SMM.EM.CollisionTriT.x = EditorGUILayout.Slider(" x-axis: ", SMM.CollisionTriT.x, SMM.mMinX - 5f,
                        SMM.mMaxX + 5f);
                    SMM.EM.CollisionTriT.y = EditorGUILayout.Slider(" y-axis: ", SMM.CollisionTriT.y, SMM.mMinY - 5f,
                        SMM.mMaxY + 5f);
                    SMM.EM.CollisionTriT.z = EditorGUILayout.Slider(" z-axis: ", SMM.CollisionTriT.z, SMM.mMinZ - 5f,
                        SMM.mMaxZ + 5f);
                }
            ...
            {
                SMM.EM.StretchBC_LocOffset =
                    EditorGUILayout.Vector3Field("Box collider center offset:", SMM.StretchBC_LocOffset);
...
//					Debug.Log ("Setup SMM");
    ...
    public void UpdateSplineObjects_OnUndo()
    {
        ...
        for (var i = tNode.transform.childCount - 1; i >= 0; i--)
            DestroyImmediate(tNode.transform.GetChild(i).gameObject);
    ...
    public void DoEdgeObjects()
    {
        ...
        if (tNode.EdgeObjects == null) tNode.EdgeObjects = new List<GSD.Roads.EdgeObjects.EdgeObjectMaker>();
        eCount = tNode.EdgeObjects.Count;

        EOM = null;

        for (var i = 0; i < tNode.EdgeObjects.Count; i++)
        {
            EOM = tNode.EdgeObjects[i];
            if (EOM.EM == null) EOM.EM = new GSD.Roads.EdgeObjects.EdgeObjectMaker.EdgeObjectEditorMaker();
            EOM.EM.Setup(EOM);
            ...
            if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f))) EOM.Setup();
            if (GUILayout.Button(btnSaveText, GSDImageButton, GUILayout.Width(16f)))
            {
                var tSave = EditorWindow.GetWindow<GSDSaveWindow>();
            ...
            if (EOM.bMaterialOverride)
            {
                if (EOM.bEdgeSignLabel) EditorGUILayout.TextField("Material search term: ", EOM.EdgeSignLabel);

                EOM.EM.EdgeMaterial1 = (Material) EditorGUILayout.ObjectField("Override mat #1: ", EOM.EdgeMaterial1,
                    typeof(Material), false);
                EOM.EM.EdgeMaterial2 = (Material) EditorGUILayout.ObjectField("Override mat #2: ", EOM.EdgeMaterial2,
                    typeof(Material), false);
            }
            ...
            else
            {
                ...
                if (EOM.bCombineMesh)
                    EOM.EM.bCombineMeshCollider =
                        EditorGUILayout.Toggle("Combined mesh collider: ", EOM.bCombineMeshCollider);
            }
...
//				EOM.EM.EndPos = tNode.GSDSpline.GetSplineValue(EOM.EM.EndTime,false);
                ...
                if (EOM.EM.bSingle) EOM.EM.bCombineMesh = false;
            }

            if (EOM.bSingle)
            {
                EOM.EM.SinglePosition =
                    EditorGUILayout.Slider("Single location: ", EOM.SinglePosition, tNode.tTime, 1f);

                if (tNode.bIsBridgeStart && tNode.bIsBridgeMatched)
                {
                    EOM.EM.SingleOnlyBridgePercent =
                        EditorGUILayout.Slider("Bridge %: ", EOM.SingleOnlyBridgePercent, 0f, 1f);
                    if (!GSDRootUtil.IsApproximately(EOM.SingleOnlyBridgePercent, EOM.EM.SingleOnlyBridgePercent,
                        0.001f))
                    {
                        EOM.EM.SingleOnlyBridgePercent = Mathf.Clamp(EOM.EM.SingleOnlyBridgePercent, 0f, 1f);
                        var tDist = EOM.EM.SingleOnlyBridgePercent * (tNode.BridgeCounterpartNode.tDist - tNode.tDist) +
                                    tNode.tDist;
            ...
            EOM.EM.bStartMatchRoadDefinition =
                EditorGUILayout.Toggle("Match road definition: ", EOM.bStartMatchRoadDefinition);
            if (EOM.bStartMatchRoadDefinition)
            {
                EOM.EM.StartMatchRoadDef =
            ...
            if (!EOM.bSingle)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                EOM.EM.StartTime =
                    EditorGUILayout.Slider("Start param: ", EOM.StartTime, tNode.MinSplination, EOM.EndTime);
                if (EOM.EM.EndTime < EOM.EM.StartTime) EOM.EM.EndTime = Mathf.Clamp(EOM.StartTime + 0.01f, 0f, 1f);
                if (GUILayout.Button("match node", EditorStyles.miniButton, GUILayout.Width(80f)))
                ...
                if (EOM.EM.StartTime > EOM.EM.EndTime) EOM.EM.StartTime = Mathf.Clamp(EOM.EndTime - 0.01f, 0f, 1f);
                if (GUILayout.Button("match next", EditorStyles.miniButton, GUILayout.Width(80f)))
                    EOM.EM.EndTime = tNode.NextTime;
                EditorGUILayout.EndHorizontal();
            }
            ...
            if (GUILayout.Button(btnDefaultText, GSDImageButton, GUILayout.Width(16f))) EOM.EM.VerticalRaise = 0f;
            EditorGUILayout.EndHorizontal();
            ...
            if (tHorizMatching != HorizMatchingDefaultsEnum.None)
            {
                ...
                else if (tHorizMatching == HorizMatchingDefaultsEnum.MatchShoulderLeft)
                {
                    if (tNode.GSDSpline.tRoad.opt_bShouldersEnabled)
                        EOM.EM.HorizontalSep = (tNode.GSDSpline.tRoad.RoadWidth() * 0.5f +
                                                tNode.GSDSpline.tRoad.opt_ShoulderWidth) * -1;
                    else
                        EOM.EM.HorizontalSep = tNode.GSDSpline.tRoad.RoadWidth() * 0.5f * -1;
                }
                else if (tHorizMatching == HorizMatchingDefaultsEnum.MatchRoadRight)
                ...
                else if (tHorizMatching == HorizMatchingDefaultsEnum.MatchShoulderRight)
                {
                    if (tNode.GSDSpline.tRoad.opt_bShouldersEnabled)
                        EOM.EM.HorizontalSep = tNode.GSDSpline.tRoad.RoadWidth() * 0.5f +
                                               tNode.GSDSpline.tRoad.opt_ShoulderWidth;
                    else
                        EOM.EM.HorizontalSep = tNode.GSDSpline.tRoad.RoadWidth() * 0.5f;
                }
            ...
            EditorGUILayout.BeginHorizontal();
            EOM.EM.HorizontalSep = EditorGUILayout.Slider("Horiz offset magnitude:", EOM.EM.HorizontalSep,
                -1f * HorizRoadMax, HorizRoadMax);
            if (GUILayout.Button(btnDefaultText, GSDImageButton, GUILayout.Width(16f))) EOM.EM.HorizontalSep = 0f;
            if (!GSDRootUtil.IsApproximately(EOM.EM.HorizontalSep, EOM.HorizontalSep))
                EOM.EM.HorizontalSep = Mathf.Clamp(EOM.EM.HorizontalSep, -1f * HorizRoadMax, HorizRoadMax);
            EditorGUILayout.EndHorizontal();
            ...
            if (EOM.HorizontalSep < 0f)
                EOM.EM.bOncomingRotation =
                    EditorGUILayout.Toggle("Auto rotate oncoming objects: ", EOM.bOncomingRotation);
...
//					Debug.Log ("Setup EOM"); 
    ...
    private void BridgeAdd_TopBase(float tHorizSep = 0f, float tVertRaise = -0.01f,
        string tMat = "Assets/RoadArchitect/Materials/GSDConcrete2.mat", bool bOverridePrefab = false,
        string OverridePrefab = "")
    {
        ...
        SMM.tName = tName;
        SMM.CurrentSplination = (GameObject) AssetDatabase.LoadAssetAtPath(tBridgeTopBaseToAdd, typeof(GameObject));
    ...
    private void BridgeAdd_BottomBase(float tHorizSep = 0f, float tVertRaise = -1.01f,
        string tMat = "Assets/RoadArchitect/Materials/GSDConcrete2.mat", bool bOverridePrefab = false,
        string OverridePrefab = "")
    {
        SMM = tNode.AddSplinatedObject();
        ...
        SMM.CurrentSplination = (GameObject) AssetDatabase.LoadAssetAtPath(tBridgeBottomBaseToAdd, typeof(GameObject));
        SMM.HorizontalSep = tHorizSep;
        SMM.VerticalRaise = tVertRaise;
        SMM.bMaterialOverride = true;
        SMM.tName = tName;
        ...
        if (tBridgeBottomBaseQuickAdd == BridgeBottomBaseDefaultsEnum.BridgeBase2)
            ...
            SMM.SplinatedMaterial1 =
                GSD.Roads.GSDRoadUtilityEditor.GiveMaterial("Assets/RoadArchitect/Materials/GSDSteel7.mat");
        else if (tBridgeBottomBaseQuickAdd == BridgeBottomBaseDefaultsEnum.BridgeSteel)
            SMM.SplinatedMaterial1 =
                GSD.Roads.GSDRoadUtilityEditor.GiveMaterial("Assets/RoadArchitect/Materials/GSDSteel7.mat");
    ...
    private void ExtrusionQuickAdd(bool bHorizOverride = false, float tHorizSep = 0f, bool bVertOverride = false,
        float tVertRaise = 0f)
    {
        try
        {
    ...
    {
        if (tSMMQuickAdd == SMMDefaultsEnum.KRail) tNode.SplinatedObjectQuickAdd("KRail");
    }

    private void ExtrudeHelper(string tPath, string tName, float DefaultHoriz,
        GSD.Roads.Splination.AxisTypeEnum tAxis = GSD.Roads.Splination.AxisTypeEnum.Z, bool bHorizOverride = false,
        float tHorizSep = 0f, bool bVertOverride = false, float tVertRaise = 0f, bool bFlipRot = false)
    {
        SMM = tNode.AddSplinatedObject();
        SMM.CurrentSplination = (GameObject) AssetDatabase.LoadAssetAtPath(tPath, typeof(GameObject));
    ...
    public void OnSceneGUI()
    {
        ...
        if (!tNode.bEditorSelected) tNode.bEditorSelected = true;
        ...
        {
            var xNodeObjects = FindObjectsOfType(typeof(GSDSplineN));
            ...
            if (!bMouseDragHasProcessed)
            {
                ...
                if (tNode.IsLegitimate() && tNode.GSDSpline.tRoad.opt_bMaxGradeEnabled) tNode.EnsureGradeValidity();
                TriggerRoadUpdate();
                bUsed = true;
            }
...
//			}
        ...
        if (GUI.changed) EditorUtility.SetDirty(tNode);
    }

    private bool VectorDiff(Vector3 tVect1, Vector3 tVect2)
    {
        if (!GSDRootUtil.IsApproximately(tVect1.x, tVect2.x, 0.0001f)) return true;
        if (!GSDRootUtil.IsApproximately(tVect1.y, tVect2.y, 0.0001f)) return true;
        if (!GSDRootUtil.IsApproximately(tVect1.z, tVect2.z, 0.0001f)) return true;
        return false;
    }
    ...
    {
        if (tNode != null) tNode.GSDSpline.tRoad.EditorUpdateMe = true;
    }
    ...
    {
        if (!GSDRootUtil.IsApproximately(V1.x, V2.x, 0.001f)) return false;
        if (!GSDRootUtil.IsApproximately(V1.y, V2.y, 0.001f)) return false;
        if (!GSDRootUtil.IsApproximately(V1.z, V2.z, 0.001f)) return false;
        return true;
    }
    ...
    private GameObject GetEndObjectQuickAdd()
    {
        ...
        return (GameObject) AssetDatabase.LoadAssetAtPath(tPath, typeof(GameObject)) as GameObject;
    }
}