...
public class GSDWizard : EditorWindow
{
    ...
    private void DoGUI()
    {
        ...
        {
            xWindowType = (WindowTypeEnum) EditorGUILayout.Popup("Category: ", (int) tWindowType, WindowTypeDescBridge,
                GUILayout.Width(312f));
        }
        else
        {
            ...
            SxWindowType = (WindowTypeEnumShort) EditorGUILayout.Popup("Category: ", (int) StWindowType, WindowTypeDesc,
                GUILayout.Width(312f));
        ...
        var oCount = oList.Count;

        var WidthSpacing = 160;
        var HeightSpacing = 200;
        var HeightOffset = 30;
        var ScrollHeightOffset = 25;

        var xCount = 0;
        var yCount = 0;
        var yMod = Mathf.FloorToInt((float) position.width / 142f) - 1;
        ...
        if (yMax * HeightSpacing + 25 > position.height)
        {
            scrollPos = GUI.BeginScrollView(new Rect(0, 25, position.width - 10, position.height - 30), scrollPos,
                new Rect(0, 0, yMod * WidthSpacing + 25, yMax * HeightSpacing + 50));
            bScrolling = true;
        ...
        for (var i = 0; i < oCount; i++)
        {
            ...
            if (bClicked)
            {
                if (tWindowType == WindowTypeEnum.Extrusion)
                {
                    ...
                    SMM.SetDefaultTimes(tNode.bIsEndPoint, tNode.tTime, tNode.NextTime, tNode.idOnSpline,
                        tNode.GSDSpline.distance);
                ...
                else if (tWindowType == WindowTypeEnum.Edge)
                {
                    ...
                    EOM.SetDefaultTimes(tNode.bIsEndPoint, tNode.tTime, tNode.NextTime, tNode.idOnSpline,
                        tNode.GSDSpline.distance);
                ...
                oList = null;
                EditorGUILayout.EndHorizontal();
                if (bScrolling) GUI.EndScrollView();
                bNoGUI = true;
                Close();
                return;
            }
    ...
    private bool DoItem(int x1, int y1, int i)
    {
        ...
        {
            if (GUI.Button(new Rect(x1, y1, 132f, 132f), oList[i].Thumb)) return true;
        }
        else
        {
            if (GUI.Button(new Rect(x1, y1, 132f, 132f), "No image")) return true;
        }
...
//		string ThumbString = "";
        for (var i = 0; i < tCount; i++)
        {
            ...
            if (tO.bIsDefault && bIsBridge)
            {
                ...
                else if (tO.DisplayName.Contains("Causeway1"))
                {
                    ...
                    tO.Desc =
                        "Standard federal highway bridge style. Good for any length depending on triangle counts.";
                    tO.sortID = 5;
                }
            ...
            if (tO.bIsDefault && !bIsBridge)
            {
                ...
                else if (tO.DisplayName.Contains("GSDGroup-KRailLights"))
                {
                    ...
                    tO.Desc =
                        "Center divider k-rail with double sided lights. Best used on mostly straight highway type roads.";
                }
    ...
    {
        if (xPath.Length < 5) xPath = GSDRootUtil.Dir_GetLibrary();

        var LaneText = "-2L";
        if (Lanes == 4)
            LaneText = "-4L";
        else if (Lanes == 6) LaneText = "-6L";
        ...
        foreach (var tInfo in fileInfo)
            if (tInfo.Extension.ToLower().Contains("gsd"))
            {
                ...
                {
                    if (tInfo.Name.Contains(LaneText)) tCount += 1;
                }
            }
    ...
    private void LoadObjs(ref string[] tNames, ref string[] tPaths, bool bIsDefault = false)
    {
        ...
        for (var i = 0; i < tCount; i++)
        {
            ...
            if (tWindowType == WindowTypeEnum.Extrusion)
            {
                var SLM = (SplinatedMeshMaker.SplinatedMeshLibraryMaker) GSDRootUtil
                    .LoadXML<SplinatedMeshMaker.SplinatedMeshLibraryMaker>(ref tPath);
            ...
            {
                var ELM = (EdgeObjectMaker.EdgeObjectLibraryMaker) GSDRootUtil
                    .LoadXML<EdgeObjectMaker.EdgeObjectLibraryMaker>(ref tPath);
            ...
            if (tO.Thumb == null)
                try
                {
                    var xObj = (GameObject) AssetDatabase.LoadAssetAtPath(tStringPath,
            ...
            if (bIsDefault && tWindowType == WindowTypeEnum.Edge)
            {
                ...
                else if (tO.DisplayName.Contains("GSDRoadBarrel3Static"))
                {
                    ...
                    tO.Desc =
                        "Three static sand barrels in a line. Best results when placed in front of railings or bridges.";
                }
                else if (tO.DisplayName.Contains("GSDRoadBarrel3Rigid"))
                {
                    tO.DisplayName = "Sand Barrels Rigid 3";
                    tO.Desc =
                        "Three rigid sand barrels in a line. Best results when placed in front of railings or bridges.";
                }
                else if (tO.DisplayName.Contains("GSDRoadBarrel7Static"))
                {
                    tO.DisplayName = "Sand Barrels Static 7";
                    tO.Desc =
                        "Seven static sand barrels in standard formation. Best results when placed in front of railings or bridges.";
                }
                else if (tO.DisplayName.Contains("GSDRoadBarrel7Rigid"))
                {
                    tO.DisplayName = "Sand Barrel Rigid 7";
                    tO.Desc =
                        "Seven rigid sand barrels in standard formation. Best results when placed in front of railings or bridges.";
                }
            ...
            if (bIsDefault && tWindowType == WindowTypeEnum.Extrusion)
            {
                ...
                else if (tO.DisplayName.Contains("GSDWBeam1R"))
                {
                    ...
                    tO.Desc =
                        "Federal spec wooden pole WBeam railing. Best used as outer shoulder railing. Right shoulder.";
                }
                else if (tO.DisplayName.Contains("GSDWBeam1L"))
                {
                    tO.DisplayName = "WBeam Wood Left";
                    tO.Desc =
                        "Federal spec wooden pole WBeam railing. Best used as outer shoulder railing. Left shoulder.";
                }
                else if (tO.DisplayName.Contains("GSDWBeam2R"))
                {
                    tO.DisplayName = "WBeam Metal Right";
                    tO.Desc =
                        "Federal spec metal pole WBeam railing. Best used as outer shoulder railing. Right shoulder.";
                }
                else if (tO.DisplayName.Contains("GSDWBeam2L"))
                {
                    tO.DisplayName = "WBeam Metal Left";
                    tO.Desc =
                        "Federal spec metal pole WBeam railing. Best used as outer shoulder railing. Left shoulder.";
                }
                ...
                else if (tO.DisplayName.Contains("GSDRailing7"))
                {
                    ...
                    tO.Desc =
                        "Rock-decorated concrete railing with pylons and double strand rusted look metal railing.";
                }
                ...
                else if (tO.DisplayName.Contains("GSDRailing4-LightR"))
                {
                    ...
                    tO.Desc =
                        "Three bar angled pole railing. Right side of road. Light version with fewer triangle count.";
                }
                else if (tO.DisplayName.Contains("GSDRailing4-LightL"))
                {
                    tO.DisplayName = "Railing #4 Light Left";
                    tO.Desc =
                        "Three bar angled pole railing. Left side of road. Light version with fewer triangle count.";
                }
                ...
                else if (tO.DisplayName.Contains("GSDCableBarrier-Light"))
                {
                    ...
                    tO.Desc =
                        "Cable barrier 10m light triangle version. Best used as center divider or as railing barriers.";
                    tO.sortID = 20;
                }
