...
//	bool bIsDefault = false;
    ...
    private void OnGUI()
    {
        ...
        temp2D_2 = (Texture2D) EditorGUILayout.ObjectField("Square thumb (optional):", temp2D, typeof(Texture2D),
            false);
        if (temp2D_2 != temp2D)
        {
            temp2D = temp2D_2;
...
//		bIsDefault = EditorGUILayout.Toggle("Is GSD:",bIsDefault);
        ...
        if (tWindowType == WindowTypeEnum.Extrusion)
            ...
            DoEdgeObject();
        else if (tWindowType == WindowTypeEnum.BridgeWizard) DoBridge();
    ...
    public void Initialize(ref Rect tRect, WindowTypeEnum _tWindowType, GSDSplineN tNode,
        GSD.Roads.Splination.SplinatedMeshMaker SMM = null, GSD.Roads.EdgeObjects.EdgeObjectMaker EOM = null)
    {
        var Rheight = 300;
        var Rwidth = 360;
        var Rx = (float) tRect.width / 2f - (float) Rwidth / 2f + tRect.x;
        var Ry = (float) tRect.height / 2f - (float) Rheight / 2f + tRect.y;
