...
{
    protected GSDRoadIntersection tInter => (GSDRoadIntersection) target;
...
//		"Traffic lights #2"
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Update intersection", GSDLoadButton)) TriggerRoadUpdate(true);
        EditorGUILayout.EndHorizontal();

        //Option: Auto update:
        t_opt_AutoUpdateIntersections.boolValue =
            EditorGUILayout.Toggle("Auto-update:", tInter.opt_AutoUpdateIntersections);
        ...
        {
            t_bLeftTurnYieldOnGreen.boolValue =
                EditorGUILayout.Toggle("Left yield on green: ", tInter.bLeftTurnYieldOnGreen);
        ...
        t_iStopType.enumValueIndex = (int) EditorGUILayout.Popup("Intersection stop type:", (int) tInter.iStopType,
            iStopTypeEnumDescriptions);


        if (tInter.iStopType == GSDRoadIntersection.iStopTypeEnum.TrafficLight1 ||
            tInter.iStopType == GSDRoadIntersection.iStopTypeEnum.TrafficLight2)
        {
            //Option: Traffic light timing type:
            t_lType.enumValueIndex = (int) EditorGUILayout.Popup("Traffic light timing:", (int) tInter.lType,
            ...
            if (tInter.lType == GSDRoadIntersection.LightTypeEnum.Timed)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                t_opt_FixedTime_RegularLightLength.floatValue = EditorGUILayout.Slider("Green length: ",
                    tInter.opt_FixedTime_RegularLightLength, 0.1f, 180f);
                ...
                {
                    EditorGUILayout.BeginHorizontal();
                    t_opt_FixedTime_LeftTurnLightLength.floatValue = EditorGUILayout.Slider("Left turn only length: ",
                        tInter.opt_FixedTime_LeftTurnLightLength, 0.1f, 180f);
                    if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f)))
                ...
                EditorGUILayout.BeginHorizontal();
                t_opt_FixedTime_AllRedLightLength.floatValue = EditorGUILayout.Slider("All red length: ",
                    tInter.opt_FixedTime_AllRedLightLength, 0.1f, 180f);
                if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f)))
                    t_opt_FixedTime_AllRedLightLength.floatValue = 1f;
                EditorGUILayout.EndHorizontal();

                EditorGUILayout.BeginHorizontal();
                t_opt_FixedTime_YellowLightLength.floatValue = EditorGUILayout.Slider("Yellow light length: ",
                    tInter.opt_FixedTime_YellowLightLength, 0.1f, 180f);
                if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f)))
            ...
            tInter.iStopType == GSDRoadIntersection.iStopTypeEnum.TrafficLight2)
        {
            ...
            t_bTrafficPoleStreetLight.boolValue =
                EditorGUILayout.Toggle("Street lights: ", tInter.bTrafficPoleStreetLight);
            ...
            t_bRegularPoleAlignment.boolValue =
                EditorGUILayout.Toggle("Normal pole alignment: ", tInter.bRegularPoleAlignment);
            ...
            if (tInter.bTrafficPoleStreetLight)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                t_StreetLight_Range.floatValue =
                    EditorGUILayout.Slider("  Street light range: ", tInter.StreetLight_Range, 1f, 128f);
                if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f)))
                ...
                t_StreetLight_Intensity.floatValue = EditorGUILayout.Slider("  Street light intensity: ",
                    tInter.StreetLight_Intensity, 0f, 8f);
                if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f)))
                ...
                EditorGUILayout.BeginHorizontal();
                t_StreetLight_Color.colorValue =
                    EditorGUILayout.ColorField("  Street light color: ", tInter.StreetLight_Color);
                if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f)))
                {
        ...
        if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f))) t_GradeMod.floatValue = 0.375f;
        EditorGUILayout.EndHorizontal();
        EditorGUILayout.BeginHorizontal();
        ...
        Line();
        t_bUseDefaultMaterials.boolValue =
            EditorGUILayout.Toggle("Use default materials:", tInter.bUseDefaultMaterials);
        ...
        if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f))) tInter.ResetMaterials_Center();
        EditorGUILayout.EndHorizontal();
        GUILayout.Space(4f);
        ...
        if (tInter.MarkerExt_Stretch1 != null)
        {
            ...
            if (tInter.MarkerExt_Stretch2 != null &&
                GUILayout.Button(btnDeleteText, GSDImageButton, GUILayout.Width(16f))) tInter.MarkerExt_Stretch2 = null;
            EditorGUILayout.EndHorizontal();
        }
        ...
        if (tInter.MarkerExt_Stretch2 != null && tInter.MarkerExt_Stretch1 != null)
        {
            ...
            if (tInter.MarkerExt_Stretch3 != null &&
                GUILayout.Button(btnDeleteText, GSDImageButton, GUILayout.Width(16f))) tInter.MarkerExt_Stretch3 = null;
            EditorGUILayout.EndHorizontal();
        }
        ...
        if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f))) tInter.ResetMaterials_Ext_Tiled();
        EditorGUILayout.EndHorizontal();
        GUILayout.Space(4f);
        ...
        if (GUILayout.Button(btnRefreshText, GSDImageButton, GUILayout.Width(16f))) tInter.ResetMaterials_Lanes();
        EditorGUILayout.EndHorizontal();
        GUILayout.Space(4f);
        ...
        if (tInter.rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes ||
            tInter.rType == GSDRoadIntersection.RoadTypeEnum.TurnLane)
        {
            EditorGUILayout.BeginHorizontal();
...
//		}
        ...
        //Set change bools:
        var bToggleTrafficLightPoleColor = false;
        var bTogglePointLights = false;
        var bGizmoChange = false;
        var bMatChange = false;
        var bRoadUpdate = false;

        if (GUI.changed)
        {
            //Option: Gizmo:
            if (t_bDrawGizmo.boolValue != tInter.bDrawGizmo) bGizmoChange = true;
            ...
            //Option: Left yield on green:
            if (t_bLeftTurnYieldOnGreen.boolValue != tInter.bLeftTurnYieldOnGreen) bRoadUpdate = true;

            //Option: Intersection stop type:
            if (t_iStopType.enumValueIndex != (int) tInter.iStopType) bRoadUpdate = true;

            //Option: Traffic light poles:
            if (t_bTrafficPoleStreetLight.boolValue != tInter.bTrafficPoleStreetLight) bRoadUpdate = true;

            //Option: Traffic light pole gray color:
            if (t_bTrafficLightGray.boolValue != tInter.bTrafficLightGray) bToggleTrafficLightPoleColor = true;

            //Option: Normal pole alignment:
            if (t_bRegularPoleAlignment.boolValue != tInter.bRegularPoleAlignment) bRoadUpdate = true;

            //Option: Point lights enabled:
            if (t_bLightsEnabled.boolValue != tInter.bLightsEnabled) bTogglePointLights = true;
            ...
            //Option: Street light color:
            if (t_StreetLight_Color.colorValue != tInter.StreetLight_Color) bTogglePointLights = true;

            //Option: Use default materials:
            if (t_bUseDefaultMaterials.boolValue != tInter.bUseDefaultMaterials) bMatChange = true;
            ...
            if (tInter.MarkerCenter3 != null && t_MarkerCenter3.objectReferenceValue != tInter.MarkerCenter3)
                bMatChange = true;
            if (tInter.MarkerExt_Stretch1 != null &&
                t_MarkerExt_Stretch1.objectReferenceValue != tInter.MarkerExt_Stretch1) bMatChange = true;
            if (tInter.MarkerExt_Stretch2 != null &&
                t_MarkerExt_Stretch2.objectReferenceValue != tInter.MarkerExt_Stretch2) bMatChange = true;
            if (tInter.MarkerExt_Stretch3 != null &&
                t_MarkerExt_Stretch3.objectReferenceValue != tInter.MarkerExt_Stretch3) bMatChange = true;
            ...
            if (bGizmoChange)
            {
                ...
                for (var i = 0;
                    i < tCount;
                    i++) //EditorUtility.SetSelectedWireframeHidden(tMRs[i], !tInter.bDrawGizmo);
                    EditorUtility.SetSelectedRenderState(tMRs[i],
                        tInter.bDrawGizmo ? EditorSelectedRenderState.Wireframe : EditorSelectedRenderState.Hidden);
                SceneView.RepaintAll();
            }

            if (bTogglePointLights) tInter.TogglePointLights(tInter.bLightsEnabled);

            if (bToggleTrafficLightPoleColor) tInter.ToggleTrafficLightPoleColor();

            if (bMatChange)
            {
                tInter.UpdateMaterials();
                if (tInter.bUseDefaultMaterials) tInter.ResetMaterials_All();
            }

            if (bRoadUpdate) TriggerRoadUpdate();
...
//		}
    ...
    private void TriggerRoadUpdate(bool bForce = false)
    {
        ...
        if (tInter != null && tInter.Node1 != null && tInter.Node2 != null)
            if (!tInter.Node1.GSDSpline.tRoad.Editor_bIsConstructing &&
                !tInter.Node2.GSDSpline.tRoad.Editor_bIsConstructing)
            {
                if (!tInter.bSameSpline)
                {
