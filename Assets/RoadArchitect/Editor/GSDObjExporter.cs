        ...
        using (var sw = new StreamWriter(folder + "/" + filename + ".mtl"))
        {
            foreach (var kvp in materialList)
            {
                ...
                if (kvp.Value.textureName != null)
                {
                    ...
                    var stripIndex = destinationFile.LastIndexOf('/'); //FIXME: Should be Path.PathSeparator;
...
//					Debug.Log("Copying texture from " + kvp.Value.textureName + " to " + destinationFile);
    ...
    private static void MeshesToFile(MeshFilter[] mf, string folder, string filename)
    {
        ...
        using (var sw = new StreamWriter(folder + "/" + filename + ".obj"))
        {
            ...
            for (var i = 0; i < mf.Length; i++) sw.Write(MeshToString(mf[i], materialList));
        }
    ...
    private static void ExportWholeSelectionToSingle()
    {
        ...
        if (exportedObjects > 0)
        {
            ...
            for (var i = 0; i < mfList.Count; i++) mf[i] = (MeshFilter) mfList[i];
            ...
            var stripIndex = filename.LastIndexOf('/'); //FIXME: Should be Path.PathSeparator
