...
namespace GSD.Roads.EdgeObjects
{
    ...
    public class EdgeObjectMaker
    {
...
#if UNITY_EDITOR
            EOM.EdgeObject =
                (GameObject) UnityEditor.AssetDatabase.LoadAssetAtPath(EdgeObjectString, typeof(GameObject));
#endif
            EOM.bIsGSD = bIsGSD;
        ...
        public void LoadFromLibrary(string xName, bool bIsQuickAdd = false)
        {
            ...
            if (bIsQuickAdd) tPath = xPath + "Q/EOM" + xName + ".gsd";
...
//				GSD.Roads.SignPlacementSubTypeEnum SubType = EOM.SubType;
            ...
            public void LoadTo(EdgeObjectMaker EOM)
            {
                ...
                if (EdgeMaterial1String.Length > 0)
                    EOM.EdgeMaterial1 =
                        (Material) UnityEditor.AssetDatabase.LoadAssetAtPath(EdgeMaterial1String, typeof(Material));
                if (EdgeMaterial2String.Length > 0)
                    EOM.EdgeMaterial2 =
                        (Material) UnityEditor.AssetDatabase.LoadAssetAtPath(EdgeMaterial2String, typeof(Material));
#endif
...
//			Quaternion OrigRot = Quaternion.identity;
            ...
            if (EdgeObject != null)
            {
                ...
                for (var j = 0; j < lCount; j++)
                {
                    ...
                    {
                        tObj = (GameObject) Object.Instantiate(EdgeObject);
                        tErrorObjs.Add(tObj);
                        tObj.transform.position = EdgeObjectLocations[j];
                    }
                    else
                    {
                        tObj = (GameObject) Object.Instantiate(EdgeObject, EdgeObjectLocations[j],
                            Quaternion.LookRotation(EdgeObjectRotations[j]));
                        tErrorObjs.Add(tObj);
                    }

//					OrigRot = tObj.transform.rotation;
                    tObj.transform.rotation *= xRot;
                    if (bOncomingRotation && SubType == SignPlacementSubTypeEnum.Left)
                    {
...
//				tMesh = null;
        ...
        private void SetupLocations()
        {
            ...
            {
                var tIndex = tNode.GSDSpline.GetClosestRoadDefIndex(StartTime, false, true);
                var jTime1 = tNode.GSDSpline.TranslateInverseParamToFloat(tNode.GSDSpline.RoadDefKeysArray[tIndex]);
                var jTime2 = jTime1;
...
//			float fTimeMax = -1f;
            var mCount = tNode.GSDSpline.GetNodeCount();
...
//			tStep = MeterSep/tNode.GSDSpline.distance;
            ...
            var bIsCenter = GSDRootUtil.IsApproximately(HorizontalSep, 0f, 0.02f);
...
//			Vector2 temp2DVect = default(Vector2);
            ...
            if (bSingle)
            {
                ...
                //Horizontal offset:
                if (!bIsCenter) //					if(HorizontalSep > 0f){
...
//					}
        ...
        private Mesh GSDCombineMeshes(ref List<Vector3[]> hVerts, ref List<int[]> hTris, ref List<Vector2[]> hUV,
            int OrigMVL, int OrigTriCount)
        {
            var mCount = hVerts.Count;
            var NewMVL = OrigMVL * mCount;
        ...
        public void ClearEOM()
        {
            if (EdgeObjects != null)
            {
                var hCount = EdgeObjects.Count;
                for (var h = hCount - 1; h >= 0; h--)
                    if (EdgeObjects[h] != null)
                        Object.DestroyImmediate(EdgeObjects[h].transform.gameObject);
            }

            if (MasterObj != null) Object.DestroyImmediate(MasterObj);
        }
