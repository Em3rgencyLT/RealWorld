...
namespace GSD.Roads
{
    ...
    public static class GSDConstruction
    {
        ...
        public static GSDSplineN CreateNode(GSDRoad RS, bool bSpecialEndNode = false,
            Vector3 vSpecialLoc = default(Vector3), bool bInterNode = false)
        {
            ...
            if (!bInterNode) UnityEditor.Undo.RegisterCreatedObjectUndo(tNodeObj, "Created node");
            ...
            else
            {
                ...
                var mCount = RS.GSDSpline.GetNodeCount();
            ...
            if (RS.opt_bMaxGradeEnabled && !bSpecialEndNode) tNode.EnsureGradeValidity(-1, true);

            if (!bInterNode && !bSpecialEndNode) RS.UpdateRoad();
            return tNode;
        }
        ...
        //Setup spline
        public static GSDSplineN InsertNode(GSDRoad RS, bool bForcedLoc = false, Vector3 ForcedLoc = default(Vector3),
            bool bIsPreNode = false, int InsertIndex = -1, bool bSpecialEndNode = false, bool bInterNode = false)
        {
            GameObject tNodeObj;
            var tWorldNodeCount = Object.FindObjectsOfType(typeof(GSDSplineN));
            if (!bForcedLoc)
                ...
                tNodeObj = new GameObject("Node" + tWorldNodeCount.Length.ToString());
            if (!bInterNode) UnityEditor.Undo.RegisterCreatedObjectUndo(tNodeObj, "Inserted node");
...
//				tNode.bIsBridge_PostNode = !bIsPreNode;	
            ...
            if (!bForcedLoc && !bSpecialEndNode && RS.opt_bMaxGradeEnabled) tNode.EnsureGradeValidity(iStart);
...
//				DetailsI = null;
        ...
        {
            var tTerrains = Object.FindObjectsOfType(typeof(Terrain));
            GSDTerrain TID;
            GameObject tObj;
            foreach (Terrain tTerrain in tTerrains)
            {
                ...
                if (TID == null) TID = tObj.AddComponent<GSDTerrain>();
                TID.CheckID();
            }
        }
        ...
        private static void ProcessRoad_Terrain_Hook1_Do(ref GSDSplineC tSpline, ref GSDRoad tRoad, bool bMultithreaded)
        {
            ...
            var heightDistance = tRoad.opt_MatchHeightsDistance;
//			float treeDistance = tRoad.opt_ClearTreesDistance;
            var detailDistance = tRoad.opt_ClearDetailsDistance;
...
//			GSDRoadUtil.Construction2DRect rRect = null;
            foreach (Terrain tTerrain in tTerrains)
            {
                ...
                {
                    var mCount3 = tRoad.GSDSpline.GetNodeCount();
                    var tVect2D_321 = default(Vector2);
                    for (var i = 0; i < mCount3; i++)
                    {
                    ...
                    if (!bContains)
                    {
                        var tDef = 5f / tSpline.distance;
                        var x2D = default(Vector2);
                        var x3D = default(Vector3);
                        for (var i = 0f; i <= 1f; i += tDef)
                        {
...
//				rRect = new GSD.Roads.GSDRoadUtil.Construction2DRect(tSpline.RoadV0,tSpline.RoadV1,tSpline.RoadV2,tSpline.RoadV3);
                ...
                if (bContains && !TempTerrainDict.ContainsKey(tTerrain))
                {
                    ...
                    TTD.heights = tTerrain.terrainData.GetHeights(0, 0, tTerrain.terrainData.heightmapResolution,
                        tTerrain.terrainData.heightmapResolution);
                    ...
                    var DetailRatio = tTerrain.terrainData.detailResolution / tTerrain.terrainData.size.x;
                    ...
                    if (tRoad.opt_HeightModEnabled)
                    {
                        ...
                        TTD.tHeights = new bool[tTerrain.terrainData.heightmapResolution,
                            tTerrain.terrainData.heightmapResolution];
...
//						TTD.DetailHasProcessed = new Dictionary<int, bool[,]>();
                        ...
                        TTD.DetailToHeightRatio = (float) (float) tTerrain.terrainData.detailResolution /
                                                  (float) tTerrain.terrainData.heightmapResolution;
...
//						TTD.DetailHasProcessed = new List<List<bool>>();
                        ...
                        for (var i = 0; i < TTD.DetailLayersCount; i++)
                        {
...
//						TTD.TreesCurrent = tTerrain.terrainData.treeInstances;
            ...
            if (bMultithreaded)
            {
                var tJob = new Threaded.TerrainCalcs();
            ...
            {
                Threaded.TerrainCalcs_Static.RunMe(ref EditorTTDList, tSpline, tRoad);
            }
        }

        public static GSDRoadUtil.Construction2DRect GetTerrainBounds(Terrain tTerrain)
        {
            var terrainWidth = tTerrain.terrainData.size.x;
            var terrainLength = tTerrain.terrainData.size.z;
//			Vector3 tPos = tTerrain.transform.TransformPoint(tTerrain.transform.position);
        ...
        {
            if (!tSpline.tRoad.opt_TreeModEnabled && !tSpline.tRoad.opt_HeightModEnabled &&
                !tSpline.tRoad.opt_DetailModEnabled) //Exit if no mod taking place.
                return;
            var TIDs = Object.FindObjectsOfType(typeof(GSDTerrain));
            Terrain tTerrain;
            int[,] tDetails = null;
            var IntBufferX = 0;
            var IntBufferY = 0;
...
//									}
                                ...
                                if (TTD.DetailLayersSkip.Contains(i) || TTD.MainDetailsX == null ||
                                    TTD.MainDetailsX.Count < 1) continue;
                                tDetails = tTerrain.terrainData.GetDetailLayer(0, 0, TTD.DetailMaxIndex,
                                    TTD.DetailMaxIndex, i);
        ...
        private static void TerrainsReset_Do(ref GSDRoad tRoad)
        {
            ...
            int[,] tDetails;
            int ArrayCount;
            foreach (var TH in tRoad.TerrainHistory)
            {
                Terrain tTerrain = null;
                foreach (GSDTerrain TID in TIDs)
                    if (TID.GSDID == TH.TID)
                        tTerrain = TID.tTerrain;
                if (!tTerrain) continue;

                //Heights:
                if (TH.x1 != null)
                {
                    heights = tTerrain.terrainData.GetHeights(0, 0, tTerrain.terrainData.heightmapResolution,
                        tTerrain.terrainData.heightmapResolution);
                    ArrayCount = TH.cI;
                    for (var i = 0; i < ArrayCount; i++) heights[TH.x1[i], TH.y1[i]] = TH.h[i];
                ...
                {
                    var RealLayerCount = tTerrain.terrainData.detailPrototypes.Length;
                    var StartIndex = 0;
                    var EndIndex = 0;
                    for (var i = 0; i < TH.DetailLayersCount; i++)
                    {
                        if (i >= RealLayerCount) break;
                        if (TH.DetailsX.Length <= i) break;
                        if (TH.DetailsY.Length <= i) break;
                        if (TH.DetailsX == null || TH.DetailsY == null || TH.DetailsI == null ||
                            TH.DetailsX.Length < 1) continue;

                        tDetails = tTerrain.terrainData.GetDetailLayer(0, 0, tTerrain.terrainData.detailWidth,
                            tTerrain.terrainData.detailHeight, i);
                ...
                if (xTress != null)
                {
                    ...
                    {
                        var TerrainTreeCount = tTerrain.terrainData.treeInstances.Length;
                        ;
    ...
    public class RoadConstructorBufferMaker
    {
        ...
        public RoadConstructorBufferMaker(GSDRoad _tRoad, RoadUpdateTypeEnum _tUpdateType)
        {
            ...
            bRoadOn = tUpdateType == RoadUpdateTypeEnum.Full || tUpdateType == RoadUpdateTypeEnum.Intersection ||
                      tUpdateType == RoadUpdateTypeEnum.Bridges;
            bTerrainOn = tUpdateType == RoadUpdateTypeEnum.Full || tUpdateType == RoadUpdateTypeEnum.Intersection ||
                         tUpdateType == RoadUpdateTypeEnum.Bridges;
        ...
        private void InitGameObjects()
        {
            ...
            if (tRoad.MainMeshes != null)
            {
                ...
                for (var i = MFArrayCount - 1; i > -1; i--) MFArray[i].sharedMesh = null;
                for (var i = MCArrayCount - 1; i > -1; i--) MCArray[i].sharedMesh = null;
        ...
        private void MeshSetup1_Do()
        {
            ...
            if (bInterseOn) MeshSetup1_IntersectionObjectsSetup();
            ...
            if (bRoadOn)
            {
                ...
                if (ShoulderR_Vectors.Count < 64000)
                {
                    ...
                    tMesh_SR = MeshSetup1_Helper(ref tMesh_SR, ShoulderR_Vectors.ToArray(), ref tris_ShoulderR,
                ...
                if (ShoulderL_Vectors.Count < 64000)
                {
                    ...
                    tMesh_SL = MeshSetup1_Helper(ref tMesh_SL, ShoulderL_Vectors.ToArray(), ref tris_ShoulderL,
                ...
                {
                    Mesh qMesh = null;
                    for (var i = 0; i < RoadConnections_verts.Count; i++)
                    {
                        qMesh = new Mesh();
                ...
                if ((tRoad.opt_bRoadCuts || tRoad.opt_bDynamicCuts) && RoadCuts.Count > 0)
                {
                    ...
                    for (var i = 0; i < cCount; i++)
                    {
                        ...
                        MeshBuffer = new Mesh();
                        tMesh_RoadCuts.Add(MeshSetup1_Helper(ref MeshBuffer, cut_RoadVectors[i].ToArray(), ref tTris,
                            ref tNormals));
                        MeshBuffer = new Mesh();
                        tMesh_RoadCuts_world.Add(MeshSetup1_Helper(ref MeshBuffer, cut_RoadVectors[i].ToArray(),
                ...
                if (tRoad.opt_bShoulderCuts || tRoad.opt_bDynamicCuts)
                {
                    ...
                    for (var i = 0; i < rCount; i++)
                    {
                        ...
                        MeshBuffer = new Mesh();
                        tMesh_SRCuts.Add(MeshSetup1_Helper(ref MeshBuffer, cut_ShoulderR_Vectors[i].ToArray(),
                            ref tTris, ref tNormals));
                        MeshBuffer = new Mesh();
                        tMesh_SRCuts_world.Add(MeshSetup1_Helper(ref MeshBuffer, cut_ShoulderR_Vectors[i].ToArray(),
                            ref tTris, ref tNormals));
                    ...
                    for (var i = 0; i < lCount; i++)
                    {
                        ...
                        MeshBuffer = new Mesh();
                        tMesh_SLCuts.Add(MeshSetup1_Helper(ref MeshBuffer, cut_ShoulderL_Vectors[i].ToArray(),
                            ref tTris, ref tNormals));
                        MeshBuffer = new Mesh();
                        tMesh_SLCuts_world.Add(MeshSetup1_Helper(ref MeshBuffer, cut_ShoulderL_Vectors[i].ToArray(),
                            ref tTris, ref tNormals));
            ...
            if (bInterseOn) MeshSetup1_IntersectionParts();
            ...
            if (bRoadOn)
            {
                ...
                if (!tRoad.opt_bRoadCuts && !tRoad.opt_bDynamicCuts || RoadCuts == null || RoadCuts.Count <= 0)
                {
                    if (tMesh != null)
                    {
                        ...
                        gObj.transform.parent =
                            tRoad.MeshRoad
                                .transform; //Road markers stored on parent "MeshRoad" game object, with a "Pavement" child game object storing the asphalt.
...
//					Vector2[] tUV;
                    ...
                    for (var i = 0; i < cCount; i++)
                    {
                        ...
                        {
                            MeshSetup2_Helper_RoadCuts(i, ref MeshMainBuffer, cut_uv_world[i], cut_tangents_world[i],
                                ref tRoad.MeshRoad, false, out CreatedMainObj);
                        ...
                        {
                            bHasMats = MeshSetup2_Helper_RoadCuts(i, ref MeshMarkerBuffer, cut_uv[i], cut_tangents[i],
                                ref CreatedMainObj, true, out CreatedMarkerObj);
                            if (bHasMats)
                            {
                                SaveMesh(SaveMeshTypeEnum.RoadCut, MeshMarkerBuffer, tRoad,
                                    "RoadCutMarker" + i.ToString());
                            }
                            else
                            {
                ...
                {
                    if (!tRoad.opt_bShoulderCuts && !tRoad.opt_bDynamicCuts || ShoulderCutsL == null ||
                        cut_ShoulderL_Vectors.Count <= 0)
                    {
                        //Right road shoulder:
                        if (tMesh_SR != null)
                        {
                            tMesh_SR = MeshSetup2_Helper(ref tMesh_SR, uv_SR, tangents_SR, ref tRoad.MeshShoR, false,
                                true);
                        ...
                        if (tMesh_SL != null)
                        {
                            tMesh_SL = MeshSetup2_Helper(ref tMesh_SL, uv_SL, tangents_SL, ref tRoad.MeshShoL, false,
                                true);
                    ...
                    else
                    {
                        ...
                        for (var i = 0; i < rCount; i++)
                        {
                            ...
                            if (MeshMainBuffer != null)
                            {
                                MeshSetup2_Helper_CutsShoulder(i, ref MeshMainBuffer, cut_uv_SR_world[i],
                                    cut_tangents_SR_world[i], ref tRoad.MeshShoR, false, false, out CreatedMainObj);
                            ...
                            if (MeshMarkerBuffer != null)
                            {
                                bHasMats = MeshSetup2_Helper_CutsShoulder(i, ref MeshMarkerBuffer, cut_uv_SR[i],
                                    cut_tangents_SR[i], ref CreatedMainObj, false, true, out CreatedMarkerObj);
                                if (bHasMats)
                                {
                                    SaveMesh(SaveMeshTypeEnum.SCut, MeshMarkerBuffer, tRoad,
                                        "SCutRMarker" + i.ToString());
                                }
                                else
                                {
                                    //Destroy if no marker materials:
                        ...
                        for (var i = 0; i < lCount; i++)
                        {
                            ...
                            if (MeshMainBuffer != null)
                            {
                                MeshSetup2_Helper_CutsShoulder(i, ref MeshMainBuffer, cut_uv_SL_world[i],
                                    cut_tangents_SL_world[i], ref tRoad.MeshShoL, true, false, out CreatedMainObj);
                            ...
                            if (MeshMarkerBuffer != null)
                            {
                                bHasMats = MeshSetup2_Helper_CutsShoulder(i, ref MeshMarkerBuffer, cut_uv_SL[i],
                                    cut_tangents_SL[i], ref CreatedMainObj, true, true, out CreatedMarkerObj);
                                if (bHasMats)
                                {
                                    SaveMesh(SaveMeshTypeEnum.SCut, MeshMarkerBuffer, tRoad,
                                        "SCutLMarker" + i.ToString());
                                }
                                else
                                {
                                    //Destroy if no marker materials:
                ...
                else
                {
                    ...
                    if (tRoad.MeshShoR != null) Object.DestroyImmediate(tRoad.MeshShoR);

                    if (tRoad.MeshShoL != null) Object.DestroyImmediate(tRoad.MeshShoL);
                }


                for (var i = 0; i < RoadConnections_tangents.Count; i++)
                {
                    ...
                    var fDist = Vector3.Distance(RoadConnections_verts[i][2], RoadConnections_verts[i][3]);
                    fDist = Mathf.Round(fDist);
                    if (tRoad.opt_Lanes == 2)
                    {
                        if (fDist == Mathf.Round(tRoad.RoadWidth() * 2f))
                            GSDRoadUtilityEditor.SetRoadMaterial(
                                "Assets/RoadArchitect/Materials/Markers/GSDRoadConn-4L.mat", MR);
                        else if (fDist == Mathf.Round(tRoad.RoadWidth() * 3f))
                            GSDRoadUtilityEditor.SetRoadMaterial(
                                "Assets/RoadArchitect/Materials/Markers/GSDRoadConn-6L-2L.mat", MR);
                    }
                    else if (tRoad.opt_Lanes == 4)
                    {
                        if (fDist == Mathf.Round(tRoad.RoadWidth() * 1.5f))
                            GSDRoadUtilityEditor.SetRoadMaterial(
                                "Assets/RoadArchitect/Materials/Markers/GSDRoadConn-6L-4L.mat", MR);
                    }
                    ...
                    vMesh.tangents =
                        GSDRootUtil.ProcessTangents(vMesh.triangles, vMesh.normals, vMesh.uv, vMesh.vertices);
                    ...
                    GSDRoadUtilityEditor.SetRoadMaterial("Assets/RoadArchitect/Materials/GSDRoad6.mat", MR);
                    tObj.transform.parent = tRoad.MeshRoad.transform;
            ...
            if (bInterseOn) MeshSetup2_Intersections();
            ...
            {
                var mCount = tRoad.GSDSpline.GetNodeCount();
                for (var i = 0; i < mCount; i++) tRoad.GSDSpline.mNodes[i].UpdateCuts();
            }
        }
...
//		}
        ...
        private void MeshSetup2_Intersections()
        {
            ...
            for (var i = 0; i < vCount; i++)
            {
                ...
                MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes0, "Lane0B",
                    "Assets/RoadArchitect/Materials/Markers/GSDInterWhiteLYellowR.mat");
            ...
            for (var i = 0; i < vCount; i++)
            {
                var bPrimaryNode = iBLane1s_tID[i].Node1 == iBLane1s_nID[i];
                if (!bPrimaryNode && iBLane1s_tID[i].iType == GSDRoadIntersection.IntersectionTypeEnum.ThreeWay &&
                    (iBLane1s_tID[i].rType == GSDRoadIntersection.RoadTypeEnum.TurnLane ||
                     iBLane1s_tID[i].rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes) &&
                    !iBLane1s_tID[i].bNode2B_LeftTurnLane)
                {
                    tUV = iBLane1s_uv[i];
                    tTangents = iBLane1s_tangents[i];
                    xMesh = tMesh_iBLanes1[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes1,
                        "LaneD1B", "Assets/RoadArchitect/Materials/Markers/GSDInterLaneDisabled.mat");
                ...
                else if (bPrimaryNode && iBLane1s_tID[i].iType == GSDRoadIntersection.IntersectionTypeEnum.ThreeWay &&
                         iBLane1s_tID[i].rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                {
                    tUV = iBLane1s_uv[i];
                    tTangents = iBLane1s_tangents[i];
                    xMesh = tMesh_iBLanes1[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes1,
                        "LaneDA1B", "Assets/RoadArchitect/Materials/Markers/GSDInterLaneDisabledOuter.mat");
                ...
                else
                {
                    ...
                    xMesh = tMesh_iBLanes1[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes1,
                        "Lane1B", "Assets/RoadArchitect/Materials/Markers/GSDInterYellowLWhiteR.mat");
            ...
            for (var i = 0; i < vCount; i++)
            {
                var bPrimaryNode = iBLane2s_tID[i].Node1 == iBLane2s_nID[i];
                if (!bPrimaryNode && iBLane2s_tID[i].iType == GSDRoadIntersection.IntersectionTypeEnum.ThreeWay &&
                    (iBLane2s_tID[i].rType == GSDRoadIntersection.RoadTypeEnum.TurnLane ||
                     iBLane2s_tID[i].rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes) &&
                    !iBLane2s_tID[i].bNode2B_LeftTurnLane)
                {
                    tUV = iBLane2s_uv[i];
                    tTangents = iBLane2s_tangents[i];
                    xMesh = tMesh_iBLanes2[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes2,
                        "LaneDA2B", "Assets/RoadArchitect/Materials/Markers/GSDInterLaneDisabledOuter.mat");
                ...
                else if (!bPrimaryNode && iBLane2s_tID[i].iType == GSDRoadIntersection.IntersectionTypeEnum.ThreeWay &&
                         iBLane2s_tID[i].rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes &&
                         !iBLane2s_tID[i].bNode2B_RightTurnLane)
                {
                    tUV = iBLane2s_uv[i];
                    tTangents = iBLane2s_tangents[i];
                    xMesh = tMesh_iBLanes2[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes2,
                        "LaneDA2B", "Assets/RoadArchitect/Materials/Markers/GSDInterLaneDisabledOuterR.mat");
                         ...
                         iBLane2s_tID[i].rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                {
                    ...
                    xMesh = tMesh_iBLanes2[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes2,
                        "LaneD2B", "Assets/RoadArchitect/Materials/Markers/GSDInterLaneDisabled.mat");
                ...
                else
                {
                    ...
                    xMesh = tMesh_iBLanes2[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes2,
                        "Lane2B", "Assets/RoadArchitect/Materials/Markers/GSDInterWhiteR.mat");
            ...
            for (var i = 0; i < vCount; i++)
            {
                var bPrimaryNode = iBLane3s_tID[i].Node1 == iBLane3s_nID[i];
                if (!bPrimaryNode && iBLane3s_tID[i].iType == GSDRoadIntersection.IntersectionTypeEnum.ThreeWay &&
                    iBLane3s_tID[i].rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes &&
                    !iBLane3s_tID[i].bNode2B_RightTurnLane)
                {
                    tUV = iBLane3s_uv[i];
                    tTangents = iBLane3s_tangents[i];
                    xMesh = tMesh_iBLanes3[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes3,
                        "LaneD3B", "Assets/RoadArchitect/Materials/Markers/GSDInterLaneDisabled.mat");
                ...
                else
                {
                    ...
                    xMesh = tMesh_iBLanes3[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes3,
                        "Lane3B", "Assets/RoadArchitect/Materials/Markers/GSDInterWhiteR.mat");
            ...
            for (var i = 0; i < vCount; i++)
            {
                ...
                MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes0, "Lane0F",
                    "Assets/RoadArchitect/Materials/Markers/GSDInterWhiteLYellowR.mat");
            ...
            for (var i = 0; i < vCount; i++)
            {
                var bPrimaryNode = iFLane1s_tID[i].Node1 == iFLane1s_nID[i];
                if (!bPrimaryNode && iFLane1s_tID[i].iType == GSDRoadIntersection.IntersectionTypeEnum.ThreeWay &&
                    (iFLane1s_tID[i].rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes ||
                     iFLane1s_tID[i].rType == GSDRoadIntersection.RoadTypeEnum.TurnLane) &&
                    !iFLane1s_tID[i].bNode2F_LeftTurnLane)
                {
                    tUV = iFLane1s_uv[i];
                    tTangents = iFLane1s_tangents[i];
                    xMesh = tMesh_iFLanes1[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes1,
                        "LaneD1F", "Assets/RoadArchitect/Materials/Markers/GSDInterLaneDisabled.mat");
                ...
                else if (bPrimaryNode && iFLane1s_tID[i].iType == GSDRoadIntersection.IntersectionTypeEnum.ThreeWay &&
                         iFLane1s_tID[i].rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                {
                    tUV = iFLane1s_uv[i];
                    tTangents = iFLane1s_tangents[i];
                    xMesh = tMesh_iFLanes1[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes1,
                        "LaneDAR1F", "Assets/RoadArchitect/Materials/Markers/GSDInterLaneDisabledOuterR.mat");
                ...
                else
                {
                    ...
                    xMesh = tMesh_iFLanes1[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes1,
                        "Lane1F", "Assets/RoadArchitect/Materials/Markers/GSDInterYellowLWhiteR.mat");
            ...
            for (var i = 0; i < vCount; i++)
            {
                var bPrimaryNode = iFLane2s_tID[i].Node1 == iFLane2s_nID[i];
                if (!bPrimaryNode && iFLane2s_tID[i].iType == GSDRoadIntersection.IntersectionTypeEnum.ThreeWay &&
                    (iFLane2s_tID[i].rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes ||
                     iFLane2s_tID[i].rType == GSDRoadIntersection.RoadTypeEnum.TurnLane) &&
                    !iFLane2s_tID[i].bNode2F_LeftTurnLane)
                {
                    tUV = iFLane2s_uv[i];
                    tTangents = iFLane2s_tangents[i];
                    xMesh = tMesh_iFLanes2[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes2,
                        "LaneDA2F", "Assets/RoadArchitect/Materials/Markers/GSDInterLaneDisabledOuter.mat");
                ...
                else if (!bPrimaryNode && iFLane2s_tID[i].iType == GSDRoadIntersection.IntersectionTypeEnum.ThreeWay &&
                         iFLane2s_tID[i].rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes &&
                         !iFLane2s_tID[i].bNode2F_RightTurnLane)
                {
                    tUV = iFLane2s_uv[i];
                    tTangents = iFLane2s_tangents[i];
                    xMesh = tMesh_iFLanes2[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes2,
                        "LaneDAR2F", "Assets/RoadArchitect/Materials/Markers/GSDInterLaneDisabledOuterR.mat");
                         ...
                         iFLane2s_tID[i].rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes)
                {
                    ...
                    xMesh = tMesh_iFLanes2[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes2,
                        "LaneD2F", "Assets/RoadArchitect/Materials/Markers/GSDInterLaneDisabled.mat");
                ...
                else
                {
                    ...
                    xMesh = tMesh_iFLanes2[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes2,
                        "Lane2F", "Assets/RoadArchitect/Materials/Markers/GSDInterWhiteR.mat");
            ...
            for (var i = 0; i < vCount; i++)
            {
                var bPrimaryNode = iFLane3s_tID[i].Node1 == iFLane3s_nID[i];
                if (!bPrimaryNode && iFLane3s_tID[i].iType == GSDRoadIntersection.IntersectionTypeEnum.ThreeWay &&
                    iFLane3s_tID[i].rType == GSDRoadIntersection.RoadTypeEnum.BothTurnLanes &&
                    !iFLane3s_tID[i].bNode2F_RightTurnLane)
                {
                    tUV = iFLane3s_uv[i];
                    tTangents = iFLane3s_tangents[i];
                    xMesh = tMesh_iFLanes3[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes3,
                        "LaneD3F", "Assets/RoadArchitect/Materials/Markers/GSDInterWhiteR.mat");
                ...
                else
                {
                    ...
                    xMesh = tMesh_iFLanes3[i];
                    MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiLanes3,
                        "Lane3F", "Assets/RoadArchitect/Materials/Markers/GSDInterWhiteR.mat");
            ...
            for (var i = 0; i < vCount; i++)
            {
                ...
                MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiMainPlates,
                    "MainPlateB", "Assets/RoadArchitect/Materials/GSDRoad6.mat", false);
                ...
                MF = MeshSetup2_Intersection_Helper(ref fMesh, ref tUV, ref tTangents, ref tRoad.MeshiMainPlates,
                    "MainPlateBM", "Assets/RoadArchitect/Materials/GSDInterMainPlate1.mat");
            ...
            for (var i = 0; i < vCount; i++)
            {
                ...
                MF = MeshSetup2_Intersection_Helper(ref xMesh, ref tUV, ref tTangents, ref tRoad.MeshiMainPlates,
                    "MainPlateFM", "Assets/RoadArchitect/Materials/GSDRoad6.mat", false);
                ...
                MF = MeshSetup2_Intersection_Helper(ref tMesh, ref tUV, ref tTangents, ref tRoad.MeshiMainPlates,
                    "MainPlateFM", "Assets/RoadArchitect/Materials/GSDInterMainPlate1.mat");
            ...
            for (var i = 0; i < vCount; i++)
            {
                ...
                if (tNode.bIsIntersection && tNode.GSDRI != null && tNode.GSDRI.Node1 == tNode)
                {
                    ...
                    var cCount = tNode.GSDRI.transform.childCount;
                    ...
                    if (tRoad.opt_bIsStatic) tCenter.isStatic = true;
                    ...
                    if (tRoad.opt_bIsLightmapped)
                        UnityEditor.GameObjectUtility.SetStaticEditorFlags(tMarker,
                            UnityEditor.StaticEditorFlags.ContributeGI);
                    if (tRoad.opt_bIsStatic) tMarker.isStatic = true;

                    bVerts = MF.sharedMesh.vertices;
                    for (var j = 0; j < 4; j++) bVerts[j].y = tNode.GSDRI.SignHeight;
                    ...
                    SaveMesh(SaveMeshTypeEnum.Intersection, MF.sharedMesh, tRoad,
                        tNode.GSDRI.transform.name + "-" + "CenterMarkers");
                }
            }
...
//			List<Mesh> MeshToDelete = new List<Mesh>();
            ...
            foreach (var GSDRI in UniqueGSDRI) GSDRI.UpdateMaterials();
        }

        private void MeshSetup2_CombineIntersections(KeyValuePair<GSDRoadIntersection, List<MeshFilter>> KVP,
            string tName, bool bMainPlates = false)
        {
            var vCount = KVP.Value.Count;
            if (vCount < 1) return;
            tName = tRoad.name + "-" + tName;
            GameObject tCenter = null;
            var cCount = KVP.Key.transform.childCount;
            var GOToDelete = new List<GameObject>();
            for (var i = 0; i < cCount; i++)
            {
            ...
            for (var i = GOToDelete.Count - 1; i >= 0; i--) Object.DestroyImmediate(GOToDelete[i]);
            ...
            if (tCenter != null)
            {
                for (var i = 0; i < CombineCount - 1; i++) SpecialVertCount += combine[i].mesh.vertexCount;
                ...
                for (var i = 0; i < xVerts.Length; i++) xVerts[i].y = xHeight;
...
//			float tHeight = 0f;
            for (var i = 0; i < tVerts.Length; i++) tVerts[i] += tVect;
            MF.sharedMesh.vertices = tVerts;
            tObj.transform.localPosition = new Vector3(0f, 0f, 0f);
            MF.sharedMesh.RecalculateBounds();
            MF.sharedMesh.RecalculateNormals();
            MF.sharedMesh.tangents = GSDRootUtil.ProcessTangents(MF.sharedMesh.triangles, MF.sharedMesh.normals,
                MF.sharedMesh.uv, MF.sharedMesh.vertices);
            if (tRoad.opt_bIsLightmapped) UnityEditor.Unwrapping.GenerateSecondaryUVSet(MF.sharedMesh);
            if (tRoad.opt_bIsLightmapped)
                UnityEditor.GameObjectUtility.SetStaticEditorFlags(tObj, UnityEditor.StaticEditorFlags.ContributeGI);
            if (tRoad.opt_bIsStatic) tObj.isStatic = true;
            ...
            if (bMainPlates && tCenter != null) Object.DestroyImmediate(tCenter);

            SaveMesh(SaveMeshTypeEnum.Intersection, MF.sharedMesh, tRoad, tName);
        }

        private MeshFilter MeshSetup2_Intersection_Helper(ref Mesh xMesh, ref Vector2[] uv, ref Vector4[] tangents,
            ref GameObject MasterObj, string tName, string tMat, bool bCollider = false)
        {
            if (xMesh == null) return null;
            xMesh.uv = uv;
            xMesh.tangents = tangents;
            if (tRoad.opt_bIsLightmapped) UnityEditor.Unwrapping.GenerateSecondaryUVSet(xMesh);
            ...
            var MR = tObj.AddComponent<MeshRenderer>();
            MR.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
            GSDRoadUtilityEditor.SetRoadMaterial(tMat, MR);
            if (tRoad.opt_bIsLightmapped)
                UnityEditor.GameObjectUtility.SetStaticEditorFlags(tObj, UnityEditor.StaticEditorFlags.ContributeGI);
            if (tRoad.opt_bIsStatic) tObj.isStatic = true;
        ...
        private Mesh MeshSetup2_Helper(ref Mesh xMesh, Vector2[] uv, Vector4[] tangents, ref GameObject tObj,
            bool bMarker, bool bShoulder = false, bool bBridge = false)
        {
            xMesh.uv = uv;
            xMesh.tangents = tangents;
            if (tRoad.opt_bIsLightmapped) UnityEditor.Unwrapping.GenerateSecondaryUVSet(xMesh);
            ...
            if (bShoulder)
            {
                ...
                if (tRoad.ShoulderMaterial1 != null)
                {
                    ...
                    if (tRoad.ShoulderMaterial2 != null)
                    {
                        ...
                        {
                            mCounter += 1;
                            if (tRoad.ShoulderMaterial4 != null) mCounter += 1;
                        }
                    }
                }
                    ...
                    if (tRoad.ShoulderMaterial1 != null)
                    {
                        ...
                        if (tRoad.ShoulderMaterial2 != null)
                        {
                            ...
                            if (tRoad.ShoulderMaterial3 != null)
                            {
                                ...
                                if (tRoad.ShoulderMaterial4 != null) tMats[3] = tRoad.ShoulderMaterial4;
                            }
                        }
                    }
                ...
                if (bMarker)
                {
                    ...
                    if (tRoad.RoadMaterialMarker1 != null)
                    {
                        ...
                        if (tRoad.RoadMaterialMarker2 != null)
                        {
                            ...
                            {
                                mCounter += 1;
                                if (tRoad.RoadMaterialMarker4 != null) mCounter += 1;
                            }
                        }
                    }
                        ...
                        if (tRoad.RoadMaterialMarker1 != null)
                        {
                            ...
                            if (tRoad.RoadMaterialMarker2 != null)
                            {
                                ...
                                if (tRoad.RoadMaterialMarker3 != null)
                                {
                                    ...
                                    if (tRoad.RoadMaterialMarker4 != null) tMats[3] = tRoad.RoadMaterialMarker4;
                                }
                            }
                        }
                ...
                else
                {
                    ...
                    if (tRoad.RoadMaterial1 != null)
                    {
                        ...
                        if (tRoad.RoadMaterial2 != null)
                        {
                            ...
                            {
                                mCounter += 1;
                                if (tRoad.RoadMaterial4 != null) mCounter += 1;
                            }
                        }
                    }
                        ...
                        if (tRoad.RoadMaterial1 != null)
                        {
                            ...
                            if (tRoad.RoadMaterial2 != null)
                            {
                                ...
                                if (tRoad.RoadMaterial3 != null)
                                {
                                    ...
                                    if (tRoad.RoadMaterial4 != null) tMats[3] = tRoad.RoadMaterial4;
                                }
                            }
                        }
        ...
        private bool MeshSetup2_Helper_RoadCuts(int i, ref Mesh zMesh, Vector2[] uv, Vector4[] tangents,
            ref GameObject MasterObj, bool bIsMarkers, out GameObject CreatedObj)
        {
            var tName = "RoadCut" + i.ToString();
            if (bIsMarkers) tName = "Markers" + i.ToString();
            ...
            if (tRoad.opt_bIsLightmapped) UnityEditor.Unwrapping.GenerateSecondaryUVSet(zMesh);
            ...
            if (tRoad.opt_bIsLightmapped)
                UnityEditor.GameObjectUtility.SetStaticEditorFlags(CreatedObj,
                    UnityEditor.StaticEditorFlags.ContributeGI);
            if (tRoad.opt_bIsStatic) CreatedObj.isStatic = true;

            return bHasMats;
        }

        private bool MeshSetup2_Helper_CutsShoulder(int i, ref Mesh zMesh, Vector2[] uv, Vector4[] tangents,
            ref GameObject MasterObj, bool bIsLeft, bool bIsMarkers, out GameObject CreatedObj)
        {
            var tName = "";
            if (bIsMarkers)
            {
            ...
            MeshCollider MC = null;
            if (tRoad.opt_bUseMeshColliders) MC = CreatedObj.AddComponent<MeshCollider>();
            ...
            if (bIsMarkers)
            {
                if (tRoad.ShoulderMaterialMarker1 != null)
                {
                    ...
                    if (tRoad.ShoulderMaterialMarker2 != null)
                    {
                        ...
                        {
                            mCounter += 1;
                            if (tRoad.ShoulderMaterialMarker4 != null) mCounter += 1;
                        }
                    }
                }
                    ...
                    if (tRoad.ShoulderMaterialMarker1 != null)
                    {
                        ...
                        if (tRoad.ShoulderMaterialMarker2 != null)
                        {
                            ...
                            if (tRoad.ShoulderMaterialMarker3 != null)
                            {
                                ...
                                if (tRoad.ShoulderMaterialMarker4 != null) tMats[3] = tRoad.ShoulderMaterialMarker4;
                            }
                        }
                    }
        ...
        private static void SaveMesh(SaveMeshTypeEnum SaveType, Mesh tMesh, GSDRoad tRoad, string tName)
        {
            ...
            var tSceneName = SceneManager.GetActiveScene().name;
...
//			}
        ...
        private static string GetRoadTHFilename(ref GSDRoad tRoad)
        {
            ...
            tSceneName = tSceneName.Replace("/", "");
            tSceneName = tSceneName.Replace(".", "");
            var tRoadName = tRoad.GSDRS.transform.name.Replace("RoadArchitectSystem", "RAS") + "-" +
                            tRoad.transform.name;
    ...
    public static class GSDRoadUtil
    {
        ...
        private static Terrain GetTerrain_Do(ref Vector3 tVect)
        {
            ...
            if (tColliders != null)
            {
                var tCollidersLength = tColliders.Length;
                for (var i = 0; i < tCollidersLength; i++)
                {
        ...
        private static void ConstructRoad_DoStoreTerrainHistory(ref GSDRoad tRoad)
        {
            ...
            if (tRoad.TerrainHistory != null && tRoad.TerrainHistory.Count > 0)
            {
                ...
                var hCount = tRoad.TerrainHistory.Count;
            ...
            foreach (var TTD in tRoad.EditorTTDList)
            {
                ...
                var THCount = tRoad.TerrainHistory.Count;
                var bContainsTID = false;
                ...
                {
                    var TotalSize = 0;
                    for (var i = 0; i < TTD.DetailLayersCount; i++) TotalSize += TTD.DetailsI[i];
...
//				pMiddle = ((tMiddle2-tMiddle1)*0.5f)+tMiddle1;
            ...
            public Vector3 LinePlaneIntersection(ref Vector3 F1)
            {
                ...
                var dotNumerator = Vector3.Dot(pMiddle - F1, normal);
...
//			}
            public bool Contains2D(ref Vector2 p)
            {
                if (Vector2.SqrMagnitude(p - poly2D[0]) > MaxDistanceSq) return false;
...
//				if(poly2D.Length != 3){ return false; }
                ...
                for (var i = 0; i < 3; i++)
                {
                    ...
                    if (newPoint.x < p.x == p.x <= oldPoint.x &&
                        (p.y - x1.y) * (x2.x - x1.x) < (x2.y - x1.y) * (p.x - x1.x)) inside = !inside;
                    oldPoint = newPoint;
                }
...
//				if(Vector3.Distance(tVect,P3) < NearDist){
        ...
        public class Construction2DRect
        {
            ...
            private void Construction2DRect_Do(ref Vector2 _P1, ref Vector2 _P2, ref Vector2 _P3, ref Vector2 _P4,
                ref float tHeight)
            {
                P1 = _P1;
                P2 = _P2;
                ...
                if (IsApproximately(P1.x, P2.x, 0.0001f)) P2.x += 0.0002f;
                if (IsApproximately(P1.x, P3.x, 0.0001f)) P3.x += 0.0002f;
                if (IsApproximately(P1.x, P4.x, 0.0001f)) P4.x += 0.0002f;
                if (IsApproximately(P2.x, P3.x, 0.0001f)) P3.x += 0.0002f;
                if (IsApproximately(P2.x, P4.x, 0.0001f)) P4.x += 0.0002f;
                if (IsApproximately(P3.x, P4.x, 0.0001f)) P4.x += 0.0002f;

                if (IsApproximately(P1.y, P2.y, 0.0001f)) P2.y += 0.0002f;
                if (IsApproximately(P1.y, P3.y, 0.0001f)) P3.y += 0.0002f;
                if (IsApproximately(P1.y, P4.y, 0.0001f)) P4.y += 0.0002f;
                if (IsApproximately(P2.y, P3.y, 0.0001f)) P3.y += 0.0002f;
                if (IsApproximately(P2.y, P4.y, 0.0001f)) P4.y += 0.0002f;
                if (IsApproximately(P3.y, P4.y, 0.0001f)) P4.y += 0.0002f;
...
//					if(Vector2.Distance(tVect,P4) < NearDist){
        ...
        public static Threaded.GSDRoadCreationT.RoadTerrainInfo[] GetRoadTerrainInfos()
        {
            var tTerrainsObj = Object.FindObjectsOfType(typeof(Terrain));
            Threaded.GSDRoadCreationT.RoadTerrainInfo tInfo;
            var tInfos = new List<Threaded.GSDRoadCreationT.RoadTerrainInfo>();
            foreach (Terrain tTerrain in tTerrainsObj)
            {
                tInfo = new Threaded.GSDRoadCreationT.RoadTerrainInfo();
                tInfo.GSDID = tTerrain.transform.gameObject.GetComponent<GSDTerrain>().GSDID;
                tInfo.tBounds = new Rect(tTerrain.transform.position.x, tTerrain.transform.position.z,
            ...
            var fInfos = new Threaded.GSDRoadCreationT.RoadTerrainInfo[tInfos.Count];
            var fInfosLength = fInfos.Length;
            for (var i = 0; i < fInfosLength; i++) fInfos[i] = tInfos[i];
            tInfos = null;
            return fInfos;
        }
        ...
        public static void SaveNodeObjects(ref Splination.SplinatedMeshMaker[] tSplinatedObjects,
            ref EdgeObjects.EdgeObjectMaker[] tEdgeObjects, ref WizardObject WO)
        {
            SaveNodeObjects_DO(ref tSplinatedObjects, ref tEdgeObjects, ref WO);
        }

        private static void SaveNodeObjects_DO(ref Splination.SplinatedMeshMaker[] tSplinatedObjects,
            ref EdgeObjects.EdgeObjectMaker[] tEdgeObjects, ref WizardObject WO)
        {
            var sCount = tSplinatedObjects.Length;
            var eCount = tEdgeObjects.Length;
            ...
            if (WO.bIsDefault) tPath = xPath + "B/W/" + WO.FileName + ".gsd";
        ...
        public static void LoadNodeObjects(string tFileName, GSDSplineN tNode, bool bIsDefault = false,
            bool bIsBridge = false)
        {
            ...
            var tData = File.ReadAllText(tPath);
            ...
            for (var i = 0; i < tSplitCount; i++)
            {
                ...
                if (SLM != null)
                {
                    ...
                    SMM.bToggle = false;
                    if (bIsBridge && tNode.bIsBridgeStart && tNode.bIsBridgeMatched &&
                        tNode.BridgeCounterpartNode != null)
                    {
                ...
                if (ELM != null)
                {
                    ...
                    EOM.bToggle = false;
                    if (!EOM.bSingle && bIsBridge && tNode.bIsBridgeStart && tNode.bIsBridgeMatched &&
                        tNode.BridgeCounterpartNode != null)
                    {
                    ...
                    {
                        var tDist = EOM.SingleOnlyBridgePercent * (tNode.BridgeCounterpartNode.tDist - tNode.tDist) +
                                    tNode.tDist;
        ...
        public static byte[] MakeSplatMap(Terrain tTerrain, Color tBG, Color tFG, int tWidth, int tHeight,
            float SplatWidth, bool bSkipBridge, bool bSkipTunnel, string xRoadUID = "")
        {
            return MakeSplatMapDo(tTerrain, tBG, tFG, tWidth, tHeight, SplatWidth, bSkipBridge, bSkipTunnel, xRoadUID);
        }

        private static byte[] MakeSplatMapDo(Terrain tTerrain, Color tBG, Color tFG, int tWidth, int tHeight,
            float SplatWidth, bool bSkipBridge, bool bSkipTunnel, string xRoadUID)
        {
            ...
            for (var i = 0; i < tBGCount; i++) tColorsBG[i] = tBG;
            ...
            {
                tRoads = new Object[1];
                var bRoads = Object.FindObjectsOfType(typeof(GSDRoad));
            ...
            {
                tRoads = Object.FindObjectsOfType(typeof(GSDRoad));
            }
            ...
            foreach (GSDRoad tRoad in tRoads)
            {
                ...
                for (var i = 0; i < tCount - 1; i++)
                {
                    ...
                    for (var j = 0; j < cCount; j++) tColors[j] = tFG;
        ...
        private static void TranslateWorldVectToCustom(int tWidth, int tHeight, Vector3 tVect, ref Vector3 tPos,
            ref Vector3 tSize, out int x1, out int y1)
        {
    ...
    public static class GSDIntersectionObjects
    {
        ...
        {
            var mCount = MasterGameObj.transform.childCount;
            if (mCount == 0) return;
            ...
            for (var i = tObjtoDelete.Count - 1; i >= 0; i--) Object.DestroyImmediate(tObjtoDelete[i]);
        }
        ...
        private static void CreateStopSignsAllWay_Do(ref GameObject MasterGameObj, bool bIsRB)
        {
            var prefab =
                UnityEditor.AssetDatabase.LoadAssetAtPath(
                    "Assets/RoadArchitect/Mesh/RoadObj/Signs/GSDSignStopAllway.prefab", typeof(GameObject));
...
//			float LaneWidth = tSpline.tRoad.opt_LaneWidth;
            var ShoulderWidth = tSpline.tRoad.opt_ShoulderWidth;
...
//			xDir = (GSDRI.CornerLR - GSDRI.transform.position).normalized;
        ...
        {
            var tDist = (Vector3.Distance(GSDRI.CornerRL, GSDRI.CornerRR) / 2f +
                         0.025f * Vector3.Distance(GSDRI.CornerLL, GSDRI.CornerRR)) / tSpline.distance;
            ;
            var p = Mathf.Clamp(GSDRI.Node1.tTime - tDist, 0f, 1f);
        ...
        {
            var tDist = (Vector3.Distance(GSDRI.CornerLR, GSDRI.CornerLL) / 2f +
                         0.025f * Vector3.Distance(GSDRI.CornerLL, GSDRI.CornerRR)) / tSpline.distance;
            ;
            var p = Mathf.Clamp(GSDRI.Node1.tTime + tDist, 0f, 1f);
            var POS = tSpline.GetSplineValue(p, true);
        ...
        {
            var tDist = (Vector3.Distance(GSDRI.CornerLL, GSDRI.CornerRL) / 2f +
                         0.025f * Vector3.Distance(GSDRI.CornerLR, GSDRI.CornerRL)) / tSpline.distance;
            ;
            var p = -1f;
            if (GSDRI.bFlipped)
        ...
        {
            var tDist = (Vector3.Distance(GSDRI.CornerRR, GSDRI.CornerLR) / 2f +
                         0.025f * Vector3.Distance(GSDRI.CornerLR, GSDRI.CornerRL)) / tSpline.distance;
            ;
            var p = -1f;
            if (GSDRI.bFlipped)
...
//			float RoadWidth = tSpline.tRoad.RoadWidth();
            var LaneWidth = tSpline.tRoad.opt_LaneWidth;
            var ShoulderWidth = tSpline.tRoad.opt_ShoulderWidth;

            var Lanes = tSpline.tRoad.opt_Lanes;
...
//			Vector3[] tVerts = null;
            ...
            var OrigPoleAlignment = GSDRI.bRegularPoleAlignment;

            //Node1:
            //RL:
            tObjRL = CreateTrafficLight(TLDistance, true, true, MaxDistanceStart, GSDRI.bTrafficPoleStreetLight,
                tSpline.tRoad.GSDRS.opt_bSaveMeshes);
//			xDir = (GSDRI.CornerRL - GSDRI.transform.position).normalized;
...
//				xDir = (GSDRI.CornerRL - GSDRI.transform.position).normalized;
            ...
            //LR:
            tObjLR = CreateTrafficLight(TLDistance, true, true, MaxDistanceStart, GSDRI.bTrafficPoleStreetLight,
                tSpline.tRoad.GSDRS.opt_bSaveMeshes);
//			xDir = (GSDRI.CornerLR - GSDRI.transform.position).normalized;
...
//				xDir = (GSDRI.CornerLR - GSDRI.transform.position).normalized;
            ...
            //Node2:
            //RR:
            tObjRR = CreateTrafficLight(TLDistance, true, true, MaxDistanceStart, GSDRI.bTrafficPoleStreetLight,
                tSpline.tRoad.GSDRS.opt_bSaveMeshes);
//			xDir = (GSDRI.CornerRR - GSDRI.transform.position).normalized;
...
//				xDir = (GSDRI.CornerRR - GSDRI.transform.position).normalized;
            ...
            //LL:
            tObjLL = CreateTrafficLight(TLDistance, true, true, MaxDistanceStart, GSDRI.bTrafficPoleStreetLight,
                tSpline.tRoad.GSDRS.opt_bSaveMeshes);
//			xDir = (GSDRI.CornerLL - GSDRI.transform.position).normalized;
...
//				xDir = (GSDRI.CornerLL - GSDRI.transform.position).normalized;
        ...
        private static bool CreateTrafficLightBase_IsInIntersection(GSDRoadIntersection GSDRI, ref Vector3 StartVec,
            ref Vector3 EndVec)
        {
            return GSDRI.ContainsLine(StartVec, EndVec);
        }

        private static GameObject CreateTrafficLight(float tDistance, bool bIsTrafficLight1, bool bOptionalCollider,
            float xDistance, bool bLight, bool bSaveAsset)
        {
            GameObject tObj = null;
            var tTrafficLightNumber = "1";
            if (!bIsTrafficLight1) tTrafficLightNumber = "2";
...
//			string assetName = "GSDInterTLB" + tTrafficLightNumber + "_" + tDistance.ToString("F0") + "_" + xDistance.ToString("F0") + ".prefab";
            var assetNameAsset = "GSDInterTLB" + tTrafficLightNumber + "_" + tDistance.ToString("F0") + "_" +
                                 xDistance.ToString("F0") + ".asset";
            var BackupFBX = "GSDInterTLB" + tTrafficLightNumber + ".FBX";
            var tMod = tDistance / 5f;
            var hMod = tDistance / 10f * 0.7f;
            var xMod = (xDistance / 20f + 2f) * 0.3334f;
            ...
            var xMesh = (Mesh) UnityEditor.AssetDatabase.LoadAssetAtPath(
                "Assets/RoadArchitect/Mesh/RoadObj/Signs/TrafficLightBases/" + assetNameAsset, typeof(Mesh));
            if (xMesh == null)
            {
                xMesh = (Mesh) UnityEditor.AssetDatabase.LoadAssetAtPath(
                    "Assets/RoadArchitect/Mesh/RoadObj/Signs/TrafficLightBases/" + BackupFBX, typeof(Mesh));
                bDoCustom = true;
            }

            tObj = new GameObject("TempTrafficLight");
            var MF = tObj.GetComponent<MeshFilter>();
            if (MF == null) MF = tObj.AddComponent<MeshFilter>();
            var MR = tObj.GetComponent<MeshRenderer>();
            if (MR == null) MR = tObj.AddComponent<MeshRenderer>();
            GSDRoadUtilityEditor.SetRoadMaterial(
                "Assets/RoadArchitect/Materials/Signs/GSDInterTLB" + tTrafficLightNumber + ".mat", MR);
...
//				List<int> tUVInts = new List<int>();
                for (var i = 0; i < mCount; i++)
                {
                    ...
                    {
                        tVerts[i].z *= hMod;
                        if (uv[i].y > 3.8f) uv[i].y *= hMod;
                    }

                    if (bXMod && tVerts[i].z > 4.8f && tVerts[i].z < 6.2f) tVerts[i].z += xValue;
                }
                ...
                if (bSaveAsset)
                    UnityEditor.AssetDatabase.CreateAsset(tMesh,
                        "Assets/RoadArchitect/Mesh/RoadObj/Signs/TrafficLightBases/" + assetNameAsset);
            }

            var BC = tObj.AddComponent<BoxCollider>();
            var MaxHeight = MF.sharedMesh.vertices[447].z;
            BC.size = new Vector3(0.35f, 0.35f, MaxHeight);
            BC.center = new Vector3(0f, 0f, MaxHeight / 2f);

            if (bOptionalCollider)
            {
                var MaxWidth = MF.sharedMesh.vertices[497].y;
            ...
            if (bLight)
            {
                var yObj = (GameObject) UnityEditor.AssetDatabase.LoadAssetAtPath(
                    "Assets/RoadArchitect/Mesh/RoadObj/Signs/GSDStreetLight_TrafficLight.prefab", typeof(GameObject));
                var kObj = (GameObject) Object.Instantiate(yObj);
...
//				kObj.name = "StreetLight";
            ...
            for (var i = 0; i < tMeshes.Length; i++) tMeshes[i].sharedMesh.RecalculateBounds();

            return tObj;
        }

        private static Vector3 TrafficLightBase_GetRot_RL(GSDRoadIntersection GSDRI, GSDSplineC tSpline,
            float DistFromCorner, bool bOverrideRegular = false)
        {
...
//				float tDist = ((Vector3.Distance(GSDRI.CornerRR,GSDRI.CornerRL) / 2f) + DistFromCorner) / tSpline.distance;;
                var p = Mathf.Clamp(GSDRI.Node1.tTime, 0f, 1f);
        ...
        private static Vector3 TrafficLightBase_GetRot_LR(GSDRoadIntersection GSDRI, GSDSplineC tSpline,
            float DistFromCorner, bool bOverrideRegular = false)
        {
            var POS = default(Vector3);
            if (!GSDRI.bRegularPoleAlignment && !bOverrideRegular)
...
//				float tDist = ((Vector3.Distance(GSDRI.CornerLR,GSDRI.CornerLL) / 2f) + DistFromCorner) / tSpline.distance;;
        ...
        private static Vector3 TrafficLightBase_GetRot_RR(GSDRoadIntersection GSDRI, GSDSplineC tSpline,
            float DistFromCorner, bool bOverrideRegular = false)
        {
            var POS = default(Vector3);
            if (!GSDRI.bRegularPoleAlignment && !bOverrideRegular)
...
//				float tDist = ((Vector3.Distance(GSDRI.CornerRR,GSDRI.CornerLR) / 2f) + DistFromCorner) / tSpline.distance;;
                var p = Mathf.Clamp(GSDRI.Node2.tTime, 0f, 1f);
                POS = tSpline.GetSplineValue(p, true);
                POS = Vector3.Cross(POS, Vector3.up);
                if (GSDRI.bFlipped) POS = POS * -1;
            }
            else
            {
        ...
        private static Vector3 TrafficLightBase_GetRot_LL(GSDRoadIntersection GSDRI, GSDSplineC tSpline,
            float DistFromCorner, bool bOverrideRegular = false)
        {
            var POS = default(Vector3);
            if (!GSDRI.bRegularPoleAlignment && !bOverrideRegular)
...
//				float tDist = ((Vector3.Distance(GSDRI.CornerLL,GSDRI.CornerRL) / 2f) + DistFromCorner) / tSpline.distance;;
        ...
        private static void CreateTrafficLightMains(GameObject MasterGameObj, GameObject tRR, GameObject tRL,
            GameObject tLL, GameObject tLR)
        {
            var GSDRI = MasterGameObj.GetComponent<GSDRoadIntersection>();
            var tSpline = GSDRI.Node1.GSDSpline;

            var tDist = Vector3.Distance(GSDRI.CornerRL, GSDRI.CornerRR) / 2f / tSpline.distance;
            ...
            var InterDist = Vector3.Distance(GSDRI.CornerRL, GSDRI.CornerLL);
            tDist = InterDist / 2f / tSpline.distance;
            tan = tSpline.GetSplineValue(GSDRI.Node1.tTime + tDist, true);

            var fTime1 = GSDRI.Node2.tTime + tDist;
            var fTime2neg = GSDRI.Node2.tTime - tDist;
        ...
        private static void ProcessPole(GameObject MasterGameObj, GameObject tObj, Vector3 tan, int tCorner,
            float InterDist)
        {
            var GSDRI = MasterGameObj.GetComponent<GSDRoadIntersection>();
            var tSpline = GSDRI.Node1.GSDSpline;
...
//			float RoadWidth = tSpline.tRoad.RoadWidth();
            ...
            var tScaleSense = (200f - GSDRI.ScalingSense) / 200f * 200f;
            ...
            var bScale = true;
            if (IsApproximately(ScaleMod, 1f, 0.001f)) bScale = false;
            ...
            {
                if (GSDRI.bLeftTurnYieldOnGreen)
                    prefab = UnityEditor.AssetDatabase.LoadAssetAtPath(
                        "Assets/RoadArchitect/Mesh/RoadObj/Signs/GSDTrafficLightLeftYield.prefab", typeof(GameObject));
                else
                    prefab = UnityEditor.AssetDatabase.LoadAssetAtPath(
                        "Assets/RoadArchitect/Mesh/RoadObj/Signs/GSDTrafficLightLeft.prefab", typeof(GameObject));
                ...
                {
                    prefab = UnityEditor.AssetDatabase.LoadAssetAtPath(
                        "Assets/RoadArchitect/Mesh/RoadObj/Signs/GSDSignYieldOnGreen.prefab", typeof(GameObject));
            ...
            {
                prefab = UnityEditor.AssetDatabase.LoadAssetAtPath(
                    "Assets/RoadArchitect/Mesh/RoadObj/Signs/GSDTrafficLightRight.prefab", typeof(GameObject));
            ...
            {
                prefab = UnityEditor.AssetDatabase.LoadAssetAtPath(
                    "Assets/RoadArchitect/Mesh/RoadObj/Signs/GSDTrafficLightMain.prefab", typeof(GameObject));
                tLanes[i] = (GameObject) Object.Instantiate(prefab, Vector3.zero, Quaternion.identity);
            ...
            var LM = new GSDTrafficLightController(ref tLeft, ref tRight, ref tLanes, ref MR_Left, ref MR_Right,
                ref MR_Mains);
            if (tCorner == 0)
            {
                GSDRI.LightsRR = null;
        ...
        public static void GetFourPoints(GSDRoadIntersection GSDRI, out Vector3 tPosRR, out Vector3 tPosRL,
            out Vector3 tPosLL, out Vector3 tPosLR, float DistFromCorner)
        {
            GetFourPoints_Do(ref GSDRI, out tPosRR, out tPosRL, out tPosLL, out tPosLR, DistFromCorner);
        }

        private static void GetFourPoints_Do(ref GSDRoadIntersection GSDRI, out Vector3 tPosRR, out Vector3 tPosRL,
            out Vector3 tPosLL, out Vector3 tPosLR, float DistFromCorner)
        {
            //Get four points:
            var tPos1 = 0f;
            ...
            var tDir = default(Vector3);
            var ShoulderWidth1 = GSDRI.Node1.GSDSpline.tRoad.opt_ShoulderWidth;
            var ShoulderWidth2 = GSDRI.Node2.GSDSpline.tRoad.opt_ShoulderWidth;

            if (!GSDRI.bFlipped)
            {
                //RR:
                Node1Width = (Vector3.Distance(GSDRI.CornerRR, GSDRI.Node1.pos) + ShoulderWidth1) /
                             GSDRI.Node1.GSDSpline.distance;
                Node2Width = (Vector3.Distance(GSDRI.CornerRR, GSDRI.Node2.pos) + ShoulderWidth2) /
                             GSDRI.Node2.GSDSpline.distance;
                ...
                //RL:
                Node1Width = (Vector3.Distance(GSDRI.CornerRL, GSDRI.Node1.pos) + ShoulderWidth1) /
                             GSDRI.Node1.GSDSpline.distance;
                Node2Width = (Vector3.Distance(GSDRI.CornerRL, GSDRI.Node2.pos) + ShoulderWidth2) /
                             GSDRI.Node2.GSDSpline.distance;
                tPos1 = GSDRI.Node1.tTime + Node1Width;
                ...
                //LL:
                Node1Width = (Vector3.Distance(GSDRI.CornerLL, GSDRI.Node1.pos) + ShoulderWidth1) /
                             GSDRI.Node1.GSDSpline.distance;
                Node2Width = (Vector3.Distance(GSDRI.CornerLL, GSDRI.Node2.pos) + ShoulderWidth2) /
                             GSDRI.Node2.GSDSpline.distance;
                tPos1 = GSDRI.Node1.tTime + Node1Width;
                if (GSDRI.iType == GSDRoadIntersection.IntersectionTypeEnum.ThreeWay) tPos1 = GSDRI.Node1.tTime;
                tTan1 = GSDRI.Node1.GSDSpline.GetSplineValue(tPos1, true);
                ...
                //LR:
                Node1Width = (Vector3.Distance(GSDRI.CornerLR, GSDRI.Node1.pos) + ShoulderWidth1) /
                             GSDRI.Node1.GSDSpline.distance;
                Node2Width = (Vector3.Distance(GSDRI.CornerLR, GSDRI.Node2.pos) + ShoulderWidth2) /
                             GSDRI.Node2.GSDSpline.distance;
                tPos1 = GSDRI.Node1.tTime - Node1Width;
                tTan1 = GSDRI.Node1.GSDSpline.GetSplineValue(tPos1, true) * -1f;
                tPos2 = GSDRI.Node2.tTime - Node2Width;
    ...
    public static class GSDIntersections
    {
        ...
        private static GameObject CreateIntersection_Do(GSDSplineN tNode, GSDSplineN xNode)
        {
            ...
            if (tNode1.bIsEndPoint)
            {
                ...
                var bAlreadyNode = false;
                if (tNode1.idOnSpline == 1 || tNode1.idOnSpline == 0) bFirstNode = true;
                if (bFirstNode && tNode1.idOnSpline == 1 && tNode1.GSDSpline.mNodes[0].bSpecialEndNode_IsStart)
                    bAlreadyNode = true;
                else if (!bFirstNode && tNode1.idOnSpline == tNode1.GSDSpline.GetNodeCount() - 2 &&
                         tNode1.GSDSpline.mNodes[tNode1.GSDSpline.GetNodeCount() - 1].bSpecialEndNode_IsEnd)
                    bAlreadyNode = true;

                var tPos = default(Vector3);
                if (bFirstNode)
                    tPos = (tNode1.tangent * -1f).normalized * (tNode1.GSDSpline.tRoad.RoadWidth() * RoadMod) +
                           tNode1.pos;
                else
                    tPos = tNode1.GSDSpline.GetSplineValue(0.999f, true).normalized *
                        (tNode1.GSDSpline.tRoad.RoadWidth() * RoadMod) + tNode1.pos;

                if (!bAlreadyNode)
                {
                    if (bFirstNode)
                    {
                        zNode = GSDConstruction.InsertNode(tNode1.GSDSpline.tRoad, true, tPos, false, 0, true, true);
                    ...
                    {
                        zNode = GSDConstruction.CreateNode(tNode1.GSDSpline.tRoad, true, tPos, true);
            ...
            else if (tNode2.bIsEndPoint)
            {
                var bFirstNode = false;
                var bAlreadyNode = false;
                if (tNode2.idOnSpline == 1 || tNode2.idOnSpline == 0) bFirstNode = true;
                if (bFirstNode && tNode2.idOnSpline == 1 && tNode2.GSDSpline.mNodes[0].bSpecialEndNode_IsStart)
                    bAlreadyNode = true;
                else if (!bFirstNode && tNode2.idOnSpline == tNode2.GSDSpline.GetNodeCount() - 2 &&
                         tNode2.GSDSpline.mNodes[tNode2.GSDSpline.GetNodeCount() - 1].bSpecialEndNode_IsEnd)
                    bAlreadyNode = true;

                var tPos = default(Vector3);
                if (bFirstNode)
                    tPos = (tNode2.tangent * -1f).normalized * (tNode2.GSDSpline.tRoad.RoadWidth() * RoadMod) +
                           tNode2.pos;
                else
                    tPos = tNode2.GSDSpline.GetSplineValue(0.999f, true).normalized *
                        (tNode2.GSDSpline.tRoad.RoadWidth() * RoadMod) + tNode2.pos;

                if (!bAlreadyNode)
                {
                    if (bFirstNode)
                    {
                        zNode = GSDConstruction.InsertNode(tNode2.GSDSpline.tRoad, true, tPos, false, 0, true, true);
                        zNode.bSpecialEndNode_IsStart = true;
                        zNode.bSpecialIntersection = true;
                    ...
                    {
                        zNode = GSDConstruction.CreateNode(tNode2.GSDSpline.tRoad, true, tPos, true);
                        zNode.bSpecialEndNode_IsEnd = true;
                        zNode.bSpecialIntersection = true;
                        zNode.tangent = tNode2.tangent;
                    }
...
//					GSDRI.Node1.GSDSpline.tRoad.PiggyBacks[4] = GSDRI.Node2.GSDSpline;
        ...
        public static Vector3[] GetCornerVectors_Test(GSDRoadIntersection GSDRI)
        {
            ...
            var tOffset = 5f;
            var tPos1 = tNode.tTime - tOffset / tSpline.distance;
            var tPos2 = tNode.tTime + tOffset / tSpline.distance;
        ...
        public static void CreateIntersection(GSDRoadIntersection GSDRI)
        {
            ...
            var iWidth = GSDRI.IntersectionWidth * 1.25f;
            ...
            foreach (var tCollider in tColliders)
        ...
        private static void FlattenIntersectionArea(ref List<GSDRoad> tRoads, GSDRoadIntersection GSDRI, float iWidth,
            out float tHeight)
        {
        ...
        private static Vector3[] GetExtendedVectors(GSDRoadIntersection GSDRI, bool bPrimary = true)
        {
            ...
            var tOffset = tNode.GSDSpline.tRoad.RoadWidth();
        ...
        //to each other. This function finds those two points. If the lines are not parallel, the function 
        //outputs true, otherwise false.
        private static bool ClosestPointsOnTwoLines(out Vector3 closestPointLine1, out Vector3 closestPointLine2,
            Vector3 linePoint1, Vector3 lineVec1, Vector3 linePoint2, Vector3 lineVec2)
        {
            closestPointLine1 = Vector3.zero;
            closestPointLine2 = Vector3.zero;
        ...
        private static void CreateIntersectionMesh_Main(GSDRoadIntersection GSDRI, float tHeight, out Vector3[] tVects,
            ref string tName)
        {
            //Get four points:
...
//			tObj = GameObject.Find("tInter4"); tObj.transform.position = tVerts[3];
            ...
            for (var i = 0; i < MVL; i++) tVerts[i] -= iObj.transform.position;
            tMesh.vertices = tVerts;
            tMesh.RecalculateBounds();
...
//			MeshToFile(MF,"Assets/RoadArchitect/Mesh/Intersections/" + tName +".obj");
            ...
            GSDRoadUtilityEditor.SetRoadMaterial("Assets/RoadArchitect/Materials/GSDRoadIntersection.mat", MR);
        }
        ...
        private static void CreateIntersectionMesh_Outer(GSDRoadIntersection GSDRI, Vector3[] tVects, ref string tName)
        {
            ...
            var cCount = GSDRI.transform.childCount;
//			bool bOuter = false;
...
//			tObj = GameObject.Find("tInter4"); tObj.transform.position = bVects2[3];
        ...
        private static void CreateIntersectionMesh_OuterInternal(Vector3[] tVerts, GameObject iObj, Vector3 vOffset,
            ref string tName)
        {
            var tMesh = new Mesh();
            var MVL = 16;
...
//			MC.sharedMesh = MF.sharedMesh;
            ...
            MR.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
            MR.receiveShadows = true;
            GSDRoadUtilityEditor.SetRoadMaterial("Assets/RoadArchitect/Materials/GSDInterText.mat", MR);
        }
        ...
        private static void InitializeIntersectionObjects(GSDRoadIntersection tGSDRI)
        {
            ...
            {
                var iObjects = Object.FindObjectsOfType(typeof(GSDRoadIntersection));
                //Add intersection components, if necessary:
                foreach (GSDRoadIntersection GSDRI in iObjects) InitializeIntersectionObjects_Internal(GSDRI);
            }
        }
        ...
        private static void InitializeIntersectionObjects_Internal(GSDRoadIntersection GSDRI)
        {
            ...
            var cCount = GSDRI.Node1.id_connected.Count;
            GSDSplineN tNode;
            for (var i = 0; i < cCount; i++)
            {
            ...
            if (1 == 1 && GSDRI.iType == GSDRoadIntersection.IntersectionTypeEnum.ThreeWay)
            {
                ...
                if (tList.Count != 3)
                {
                    Debug.LogError(
                        ...
                        tList.Count + " on intersection: " + GSDRI.transform.name);
                    return;
                }

                ProcessFourCorners(ref n1Point, ref n2Point, GSDRI.transform.gameObject,
                    GetLongestSplineDistance(n1Spline, n2Spline));
                ProcessFourCorners(ref n1Point, ref n3Point, GSDRI.transform.gameObject,
                    GetLongestSplineDistance(n1Spline, n3Spline));
                ProcessFourCorners(ref n2Point, ref n3Point, GSDRI.transform.gameObject,
            ...
            else if (GSDRI.iType == GSDRoadIntersection.IntersectionTypeEnum.FourWay)
            {
                ...
                ProcessFourCorners(ref n1Point, ref n3Point, GSDRI.transform.gameObject,
                    GetLongestSplineDistance(n1Spline, n3Spline));
                ProcessFourCorners(ref n1Point, ref n4Point, GSDRI.transform.gameObject,
                    GetLongestSplineDistance(n1Spline, n4Spline));
                ProcessFourCorners(ref n2Point, ref n3Point, GSDRI.transform.gameObject,
                    GetLongestSplineDistance(n2Spline, n3Spline));
                ProcessFourCorners(ref n2Point, ref n4Point, GSDRI.transform.gameObject,
                    GetLongestSplineDistance(n2Spline, n4Spline));
                ProcessFourCorners(ref n3Point, ref n4Point, GSDRI.transform.gameObject,
        ...
        {
            var SplineNodeObjects = Object.FindObjectsOfType(typeof(GSDSplineN));
        ...
        private static Vector3 GetFourCornerPoint(ref GSDSplineC tSpline, ref GSDSplineN tNode,
            GSDRoadIntersection GSDRI)
        {
            GSDSplineN iNode;
            ...
            var Pos1 = tNode.tTime;
            var iPos = iNode.tTime;
        ...
        private static void ProcessFourCorners(ref Vector3 n1, ref Vector3 n2, GameObject tIntersectionObject,
        ...
        {
            var cAng = (a * a + b * b - c * c) / (2 * a * b);
            var rad = Mathf.Acos(cAng);
            var tFloat = Mathf.Rad2Deg * rad;
            return tFloat;
        }

        private static void ProcessTwoCorners(ref GameObject tIntersectionObject, ref Vector3 n1, ref Vector3 n2,
            float SplineDistance)
        {
            ...
            var tStep = 1.25f / SplineDistance;
            var bSuccess = MoveCorner(tStep, 3f, ref tCorner, ref tIntersectionObject, v3);
            if (!bSuccess)
            {
        ...
        private static bool MoveCorner(float tStep, float tDist, ref GameObject tCorner,
            ref GameObject tIntersectionObject, Vector3 v3)
        {
            var tStart = 0.05f;
            var bDone = false;
            var spamguard = 0;
    ...
    public static class GSDNavigation
    {
        public static void UpdateConnectedNodes()
        {
            ...
            var SplineObjects = Object.FindObjectsOfType(typeof(GSDSplineC));

            //Store connected spline nodes on each other:
            GSDSplineN xNode;
            foreach (GSDSplineC tSpline in SplineObjects)
            {
                var mNodeCount = tSpline.mNodes.Count;
                for (var i = 0; i < mNodeCount; i++)
                {
        ...
        {
            var tSplines = Object.FindObjectsOfType(typeof(GSDSplineC));
            var SplineCount = 0;
            var NodeCount = 0;
            foreach (GSDSplineC tSpline in tSplines)
            {
                SplineCount += 1;
                foreach (var tNode in tSpline.mNodes)
                {
                    NodeCount += 1;
