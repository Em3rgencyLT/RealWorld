...
namespace GSD
{
    ...
    public static class GSDRootUtil
    {
        ...
        public static float Ease(float t, float k1, float k2)
        {
            ...
            if (t < k1)
                ...
                s = 2 * k1 / Mathf.PI + k2 - k1 +
                    (1 - k2) * (2 / Mathf.PI) * Mathf.Sin((t - k2) / (1.0f - k2) * Mathf.PI / 2);
        ...
        /// <returns></returns>
        public static bool Intersects2D(ref Vector2 Line1S, ref Vector2 Line1E, ref Vector2 Line2S, ref Vector2 Line2E,
            out Vector2 intersectionPoint)
        {
            ...
            if (s >= 0 && s <= 1 && t >= 0 && t <= 1)
            {
                var intersectionPointX = Line1S.x + t * firstLineSlopeX;
                var intersectionPointY = Line1S.y + t * firstLineSlopeY;
        ...
        {
            var MVL = verts.Length;
            if (MVL == 0) return new Vector4[0];
            var triangleCount = tris.Length; // mesh.triangles.Length / 3;
...
#if UNITY_EDITOR
            System.GC.Collect();
            if (bWait) System.GC.WaitForPendingFinalizers();
